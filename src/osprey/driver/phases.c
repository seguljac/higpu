...
/** DAVID CODE BEGIN **/
    case P_hc_cpp:
        // Running a custom version of "cc1".
        if (abi == ABI_N32) add_string(args, "-m32");
        add_string(args, "-D_BSD_SOURCE");
        add_string(args, "-std=c99");
        add_string(args, "-D_POSIX_C_SOURCE=200112L");
        add_string(args, "-quiet");
        add_string(args, "-E");
        if (input_source == NULL) input_source = string_copy(the_file);
        add_string(args, input_source);
        if (last_phase != P_any_cpp)
        {
            add_string(args, ">");
...
/** DAVID CODE BEGIN **/
            if (first_phase == P_any_cpp)
            {
                ...
                {
                    add_phase(source_lang == L_CC ? P_gcpp_plus : P_gcpp);
                }
            }
