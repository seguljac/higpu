...
/** DAVID CODE BEGIN **/
    ...
    {
        // Enable debugging trace in gfec.
        add_phase_for_option(O_vhc, P_c_gfe);
        // Enable debugging trace in ipl and ipa.
        flag = add_new_option("-td1024");
        add_phase_for_option(flag, P_ipl);
        add_phase_for_option(flag, P_ipa_link);
        prepend_option_seen(flag);
    }
    if (option_was_seen(O_keep_all_types))
    {
        add_phase_for_option(O_keep_all_types, P_c_gfe);
    }
#endif  // HICUDA
/*** DAVID CODE END ***/
