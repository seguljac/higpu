 ...
 * malloc/free memory allocation, and is MUCH more effecient for a particular
 * style of memory allocation common in the compiler. In this style,
 * individual memory blocks are not freed one at a time. Rather all the
 * outstanding memory in a particular memory pool is freed as a whole. Doing
 * this allow us to replace large numbers of calls to alloc/free with only a
 * few calls which allocate and deallocate large blocks.
 *
 * Each MEM_POOL has a current allocation level. MEM_POOLs may be Push'ed onto
 * a new allocation level or Pop'ed back to their previous allocation level.
 * When a MEM_POOL is pop'ed, all the memory allocated from it since it was
 * pushed to its current level is freed.
 *
 ...
 * possible to see a lot of information about the callsites where memory is
 * allocated. In particular, it is possible to see how much memory allocated
 * from each callsite is outstanding.
 *
 * Before you begin
 * ================
 *
 ...
 *
 *	    If this MEM_POOL is passed as an argument to an alloc or free routine,
 *	    use malloc instead of the mempools. Note that it causes the use of
 *	    malloc and not calloc. Memory from this pool will not be automatically
 *	    zeroed.
 *
 ...
 *
 *  void MEM_POOL_Initialize(MEM_POOL *pool, char *name, BOOL bz)
 *
 *      Must be called on a pool before it is used for the other operations.
 *
 *      pool - is a pointer to the MEM_POOL to initialize.
 *      name - is a name to use when printing information about it
 *      bz   - If TRUE, memory allocated from the pool will always be zero'ed
 *
 *      After a MEM_POOL has been initialized, it is important that the
 *      structure it points to at no point go out of scope, so MEM_POOLs
 *      initialized this way should generally statically allocated.
 *
 ...
 *
 *
 *  void MEM_POOL_Push(MEM_POOL *pool)
 *
 *      Check that the pool is not frozen. If not, push a pool to a new
 *      allocation level.
 *
 *      pool - is a pointer to the MEM_POOL to push
 *
 *  void MEM_POOL_Push_Freeze(MEM_POOL *pool)
 *      
 *      Same as MEM_POOL_Push, except that after being pushed, the pool is
 *      frozen for subsequent push/pop operations until the pool is unfrozen.
 * 
 *      pool - is a pointer to the MEM_POOL to push
 *
 *  void MEM_POOL_Pop(MEM_POOL *pool)
 * 
 *      Check that the pool is not frozen. If not, then pop a MEM_POOL to a
 *      previous allcation level.  All memory allocated since the matching
 *      call to MEM_POOL_Push is freed.  As a special case, unmatched pop's
 *      just free ALL outstanding storage in t.
 *
 *      pool - is a pointer to the MEM_POOL to pop
 *
 *  void MEM_POOL_Pop_Unfreeze(MEM_POOL *pool)
 * 
 *      Same as MEM_POOL_Pop, except that the memory pool is first unfrozen.
 *      Must be called on a frozen memory pool.
 *
 *      pool - is a pointer to the MEM_POOL to pop
 *
 *
 *  MEM_PTR MEM_POOL_Alloc(MEM_POOL *pool, size_t size)
 *
 ...
 *
 * 	    Free all the memory associated with <pool>. Currently, deleted pools
 * 	    are not reported by MEM_Trace (but this can be changed). Note that
 * 	    <*pool> itself is not freed. A deleted pool cannot be used unless it
 * 	    is first initialized again with MEM_POOL_Initialize.
 *
 *
