...
/* Interfaces that support multiple string tables. A string table is
 * represented as an opaque handler (void*).
 *
 * NOTE: I don't understand the heap management of class STR_TAB. It does
 * not deallocate the buffer allocated in 'init_str_tab'. If the buffer is
 * external, it does reallocation on it. */

extern void* init_str_tab(UINT32 size);

extern void* init_str_tab(const char *buf, UINT32 size);

extern STR_IDX get_str_idx(void *str_tab_handle, const char *str);

extern const char* get_str(void *str_tab_handle, STR_IDX str_idx);

extern STR_IDX last_str_idx(void *str_tab_handle);

extern void* init_tcon_str_tab(UINT32 size);

extern void* init_tcon_str_tab(const char *buf, UINT32 size);

extern const char* get_tcon_str(void *tcon_str_tab_handle, STR_IDX str_idx);

extern STR_IDX last_tcon_str_idx(void *tcon_str_tab_handle);
 ...
 * If `str' is NULL, return STR_IDX_ZERO.
 */
extern STR_IDX Save_Str(const char *str, bool *is_new = NULL);
 ...
 */
extern STR_IDX Save_Str2(const char *s1, const char *s2, bool *is_new = NULL);
 ...
 * If both `s1' and 's2' are NULL, return STR_IDX_ZERO.
 */
extern STR_IDX Save_Str2i(const char *s1, const char *s2,
        UINT i, bool *is_new = NULL);

extern STR_IDX Get_Str(const char *str);
