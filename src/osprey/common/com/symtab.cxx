...
New_Scope(SYMTAB_IDX level, MEM_POOL *pool, BOOL reserve_index_zero)
{
    if (level >= Max_scope) {
        ...
        Max_scope *= 2;
        Scope_tab = (SCOPE *)MEM_POOL_Realloc(Malloc_Mem_Pool,
                Scope_tab, size, size*2);
    }

    // What CURRENT_SYMTAB refers to
    Current_scope = level;
...
/**
 * Initialize all symbol tables.  This function must be called before
 * any symbol table operation is performed.
 *
 * If reading the symtab from a file, then reserve_index_zero should
 * be FALSE.
 */
void
Initialize_Symbol_Tables(BOOL reserve_index_zero) {
    if (Scope_tab != NULL) return;

    Scope_tab = TYPE_MEM_POOL_ALLOC_N(SCOPE, Malloc_Mem_Pool, Max_scope);
    ...
    if (reserve_index_zero) {
        // The string table starts with 4KB.
        Initialize_Strtab(0x1000);

        // For producer, we reserve the 1st entry for all global tables.
        UINT dummy_idx;
        ...
        bzero(&New_EELIST((EELIST_IDX&)dummy_idx), sizeof(EELIST));
...
ST_ATTR*
find_st_attr(ST_IDX st_idx, bool create_if_not_exist) {
    SYMTAB_IDX level = ST_IDX_level(st_idx);
    ST_ATTR_TAB *st_attr_tab = Scope_tab[level].st_attr_tab;

    UINT size = st_attr_tab->size();
    for (UINT i = 0; i < size; ++i) {
        ST_ATTR &st_attr = st_attr_tab->Entry(i);
        if (ST_ATTR_st_idx(st_attr) == st_idx) return &st_attr;
    }
    ...
    if (create_if_not_exist) {
        ...
        ST_ATTR_Init(st_attr, st_idx, ST_ATTR_EXTRA_FLAGS, 0);
        return &st_attr;
    }
