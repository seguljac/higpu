...
//----------------------------------------------------------------------
inline EELIST&
New_EELIST(EELIST_IDX& eelist) { return EElist_Table.New_entry(eelist); }

inline UINT32
EELIST_Table_Size() { return EElist_Table.Size(); }
...
inline void ST_ATTR_Init(ST_ATTR& st_attr,
        ST_IDX st_idx, ST_ATTR_KIND akind, UINT64 val)
{
...
// This version clones the kind and value from <orig_st_attr>.
inline void ST_ATTR_Init(ST_ATTR& st_attr,
        ST_IDX st_idx, ST_ATTR& orig_st_attr)
{
    st_attr.st_idx = st_idx;
    st_attr.kind = orig_st_attr.kind;
    st_attr.Set_u(orig_st_attr.Get_flags());
}
...
/* THE FOLLOWING FUNCTIONS SHOULD NOT BE PUT HERE. */

/**
 * Find if 'st_idx' has attributes in the corresponding ST_ATTR_TAB.
 * If create_if_not_exit is true, a new ST_ATTR will be created if it does
 * not exist.
 *
 * Return the entry or NULL if it does not exist (and create_if_not_exist is
 * false).
 */
ST_ATTR* find_st_attr(ST_IDX st_idx, bool create_if_not_exist = true);

/**
 * Retrive the extra flags for 'st_idx' from the corresponding ST_ATTR_TAB.
 * If 'st_idx' is not in ST_ATTR_TAB, return 0.
 */
inline UINT32 get_st_attr_flags(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? 0 : ST_ATTR_flags(*st_attr);
}

inline void set_st_attr_flags(ST_IDX st_idx, UINT32 flags)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_flags(*st_attr, flags);
}

inline bool st_attr_is_cuda_runtime(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_cuda_runtime(*st_attr);
}

inline void set_st_attr_is_cuda_runtime(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_cuda_runtime(*st_attr);
}

inline void clear_st_attr_is_cuda_runtime(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_cuda_runtime(*st_attr);
}

inline bool st_attr_is_global_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_global_var(*st_attr);
}

inline void set_st_attr_is_global_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_global_var(*st_attr);
}

inline void clear_st_attr_is_global_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_global_var(*st_attr);
}

inline bool st_attr_is_shared_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_shared_var(*st_attr);
}

inline void set_st_attr_is_shared_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_shared_var(*st_attr);
}

inline void clear_st_attr_is_shared_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_shared_var(*st_attr);
}

inline bool st_attr_is_const_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_const_var(*st_attr);
}

inline void set_st_attr_is_const_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_const_var(*st_attr);
}

inline void clear_st_attr_is_const_var(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_const_var(*st_attr);
}

inline bool st_attr_is_kernel(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_kernel(*st_attr);
}

inline void set_st_attr_is_kernel(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_kernel(*st_attr);
}

inline void clear_st_attr_is_kernel(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_kernel(*st_attr);
}

inline bool st_attr_is_device(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_device(*st_attr);
}

inline void set_st_attr_is_device(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_device(*st_attr);
}

inline void clear_st_attr_is_device(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_device(*st_attr);
}

inline bool st_attr_is_hicuda_data(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx, false);
    return (st_attr == NULL) ? false : ST_ATTR_is_hicuda_data(*st_attr);
}

inline void set_st_attr_is_hicuda_data(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Set_ST_ATTR_is_hicuda_data(*st_attr);
}

inline void clear_st_attr_is_hicuda_data(ST_IDX st_idx)
{
    ST_ATTR *st_attr = find_st_attr(st_idx);
    Clear_ST_ATTR_is_hicuda_data(*st_attr);
}
