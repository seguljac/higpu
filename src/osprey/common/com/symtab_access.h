...
inline BOOL
PU_is_kernel(const PU& pu)      { return (pu.flags & PU_IS_KERNEL) != 0; }
inline void
Set_PU_is_kernel(PU& pu)        { pu.flags |= PU_IS_KERNEL; }
inline void
Clear_PU_is_kernel(PU& pu)      { pu.flags &= ~PU_IS_KERNEL; }

inline BOOL
PU_is_device(const PU& pu)      { return (pu.flags & PU_IS_DEVICE) != 0; }
inline void
Set_PU_is_device(PU& pu)        { pu.flags |= PU_IS_DEVICE; }
inline void
Clear_PU_is_device(PU& pu)      { pu.flags &= ~PU_IS_DEVICE; }
...
/** DAVID CODE BEGIN **/
inline EELIST_IDX
TY_eelist(const TY& ty) { return ty.EElist(); }
inline void
Set_TY_eelist(TY& ty, EELIST_IDX idx) { ty.Set_eelist(idx); }
inline EELIST_IDX
TY_eelist(const TY_IDX ty_idx) { return TY_eelist(Ty_Table[ty_idx]); }
inline void
Set_TY_eelist(TY_IDX ty_idx, EELIST_IDX idx) {
    Set_TY_eelist(Ty_Table[ty_idx], idx);
}
/*** DAVID CODE END ***/
...
inline BOOL
TY_is_cuda_runtime(const TY& ty) {
    return ty.flags & TY_IS_CUDA_RUNTIME;
}

inline void
Set_TY_is_cuda_runtime(TY& ty) {
    ty.flags |= TY_IS_CUDA_RUNTIME;
}

inline void
Clear_TY_is_cuda_runtime(TY& ty) {
    ty.flags &= ~TY_IS_CUDA_RUNTIME;
}

inline BOOL
TY_is_cuda_runtime(const TY_IDX tyi) {
    return TY_is_cuda_runtime(Ty_Table[tyi]);
}

inline void
Set_TY_is_cuda_runtime(TY_IDX tyi) {
    Set_TY_is_cuda_runtime(Ty_Table[tyi]);
}

inline void
Clear_TY_is_cuda_runtime(TY_IDX tyi) {
...
inline BOOL
TY_is_enum(const TY& ty) {
    return ty.Scalar_flags() & TY_IS_ENUM;
}

inline BOOL
TY_is_enum(const TY_IDX ty_idx) {
    return TY_is_enum(Ty_Table[ty_idx]);
}

inline void
Set_TY_is_enum(TY& ty) {
    ty.Set_scalar_flag(TY_IS_ENUM);
}

inline void
Set_TY_is_enum(TY_IDX ty_idx) {
    Set_TY_is_enum(Ty_Table[ty_idx]);
}

inline void
Clear_TY_is_enum(TY& ty) {
    ty.Clear_scalar_flag(TY_IS_ENUM);
}

inline void
Clear_TY_is_enum(TY_IDX ty_idx) {
    Clear_TY_is_enum(Ty_Table[ty_idx]);
}

inline BOOL
TY_is_long_int(const TY& ty) {
    return ty.Scalar_flags() & TY_IS_LONG_INT;
}

inline BOOL
TY_is_long_int(const TY_IDX ty_idx) {
    return TY_is_long_int(Ty_Table[ty_idx]);
}

inline void
Set_TY_is_long_int(TY& ty) {
    ty.Set_scalar_flag(TY_IS_LONG_INT);
}

inline void
Set_TY_is_long_int(TY_IDX ty_idx) {
    Set_TY_is_long_int(Ty_Table[ty_idx]);
}

inline void
Clear_TY_is_long_int(TY& ty) {
    ty.Clear_scalar_flag(TY_IS_LONG_INT);
}

inline void
Clear_TY_is_long_int(TY_IDX ty_idx) {
    Clear_TY_is_long_int(Ty_Table[ty_idx]);
}

inline BOOL
TY_is_char(const TY& ty) {
    return ty.Scalar_flags() & TY_IS_CHAR;
}

inline BOOL
TY_is_char(const TY_IDX ty_idx) {
    return TY_is_char(Ty_Table[ty_idx]);
}

inline void
Set_TY_is_char(TY& ty) {
    ty.Set_scalar_flag(TY_IS_CHAR);
}

inline void
Set_TY_is_char(TY_IDX ty_idx) {
    Set_TY_is_char(Ty_Table[ty_idx]);
}

inline void
Clear_TY_is_char(TY& ty) {
    ty.Clear_scalar_flag(TY_IS_CHAR);
}

inline void
Clear_TY_is_char(TY_IDX ty_idx) {
    Clear_TY_is_char(Ty_Table[ty_idx]);
}
...
inline BOOL
LABEL_is_not_used(const LABEL& l) { return l.flags & LABEL_IS_NOT_USED; }
inline void
Set_LABEL_is_not_used(LABEL& l) { l.flags |= LABEL_IS_NOT_USED; }
inline void
Clear_LABEL_is_not_used(LABEL& l) { l.flags &= ~LABEL_IS_NOT_USED; }
...
inline mUINT32 ST_ATTR_flags(const ST_ATTR &st_attr)
{
    Is_True(st_attr.kind == ST_ATTR_EXTRA_FLAGS,
            ("attribute is not for extra flags"));
    return st_attr.Get_flags();
}

inline void Set_ST_ATTR_flags(ST_ATTR &st_attr, mUINT32 flags)
{
    st_attr.kind = ST_ATTR_EXTRA_FLAGS;
    st_attr.Set_flags(flags);
}

inline bool ST_ATTR_is_cuda_runtime(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_CUDA_RUNTIME;
}

inline void Set_ST_ATTR_is_cuda_runtime(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_CUDA_RUNTIME);
}

inline void Clear_ST_ATTR_is_cuda_runtime(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_CUDA_RUNTIME);
}

inline bool ST_ATTR_is_global_var(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_GLOBAL_VAR;
}

inline void Set_ST_ATTR_is_global_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_GLOBAL_VAR);
}

inline void Clear_ST_ATTR_is_global_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_GLOBAL_VAR);
}

inline bool ST_ATTR_is_shared_var(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_SHARED_VAR;
}

inline void Set_ST_ATTR_is_shared_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_SHARED_VAR);
}

inline void Clear_ST_ATTR_is_shared_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_SHARED_VAR);
}

inline bool ST_ATTR_is_const_var(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_CONST_VAR;
}

inline void Set_ST_ATTR_is_const_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_CONST_VAR);
}

inline void Clear_ST_ATTR_is_const_var(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_CONST_VAR);
}

inline bool ST_ATTR_is_kernel(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_KERNEL;
}

inline void Set_ST_ATTR_is_kernel(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_KERNEL);
}

inline void Clear_ST_ATTR_is_kernel(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_KERNEL);
}

inline bool ST_ATTR_is_device(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_DEVICE;
}

inline void Set_ST_ATTR_is_device(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_DEVICE);
}

inline void Clear_ST_ATTR_is_device(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_DEVICE);
}

inline bool ST_ATTR_is_hicuda_data(const ST_ATTR &st_attr)
{
    return ST_ATTR_flags(st_attr) & ST_ATTR_IS_HICUDA_DATA;
}

inline void Set_ST_ATTR_is_hicuda_data(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags | ST_ATTR_IS_HICUDA_DATA);
}

inline void Clear_ST_ATTR_is_hicuda_data(ST_ATTR &st_attr)
{
    mUINT32 flags = ST_ATTR_flags(st_attr);
    st_attr.Set_flags(flags & ~ST_ATTR_IS_HICUDA_DATA);
}
