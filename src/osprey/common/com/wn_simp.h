...
/*---------------------------------------------------------------------------
 ...
 Description:
  ...
  WN * WN_SimplifyCvtl(OPCODE opc, INT16 cvtl_bits, WN *k0)
      ...
      Simplify binary and unary operations. opc is the opcode we are trying to
      build, and k0 and k1 are the operands of the WHIRL node being
      constructed. If simplification occurred, the functions return a
      simplified node. If no simplifications occurred, the function return
      NULL. These functions delete unused whirl nodes and trees during
      simplification.
  ...
  INT32 WN_Simp_Compare_Trees(WN *t1, WN *t2)
	...
	3) Special cases:
       ...
       g) Symbol table entries are compared by examining id first, then index.
  ...
  WN * WN_Simplify_Rebuild_Expr_Tree(WN * tree)
      Assumes that all the children of tree are already simplified, rebuild
      the tree with simplification. This is useful to catch cases such as
      when an LDID of a constant is replaced with a CONST node, and the tree
      can now be folded.

  BOOL WN_Simplifier_Enable(BOOL enable)
      Turn on the simplifier, and return the old state of the simplifier.

      The simplifier is controlled by a few variables set in config.c. Most of
      these are the same as the old cfold control variables. The variables
      which affect the simplifier are:
      ...
      Enable_Cfold_Aggressive: Turn on "aggressive" optimizations.
                               On by default at -O1.

      Enable_Cfold_Reassociate: Allow optimizations on floating point
                                quantities which require reassociation.
      ...
      Simp_Multiply_To_Shift: Allow conversion of multiplies by powers of 2 to
                              shifts
      ...
      WN_SimpAlias_Manager: set to an alias manager. If set to non-null, causes
                            the simplifier to copy the alias informations on
                            loads and stores it rebuilds.
