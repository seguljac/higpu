...
extern WN* WN_CreateTriplet(WN *start_idx, WN *stride, WN *end_idx);

extern WN* WN_CreateArrSection(WN *base_addr, int ndims);
...
/** DAVID CODE BEGIN **/
extern WN* WN_LdidScalar(ST_IDX sym);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
extern WN* WN_StidScalar(ST *sym, WN *value);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
extern WN* WN_Enum(UINT64 value, TY_IDX enum_ty_idx);
/*** DAVID CODE END ***/
...
inline WN*
WN_LdaZeroOffset(ST *sym) {
    return WN_Lda(Pointer_type, 0, sym);
}

inline WN*
WN_LdaZeroOffset(ST_IDX st_idx) {
    return WN_LdaZeroOffset(ST_ptr(st_idx));
}

/**
 * ty_idx is the address type of 'st'.
 */
inline WN*
WN_LdaZeroOffset(ST_IDX st_idx, TY_IDX ty_idx) {
    return WN_CreateLda(OPR_LDA, Pointer_type, MTYPE_V, 0, ty_idx, st_idx);
}
...
// Default stride = 1
inline WN*
WN_CreateTriplet(WN *start_idx, WN *end_idx) {
    return WN_CreateTriplet(start_idx, WN_Intconst(MTYPE_U4, 1), end_idx);
}
