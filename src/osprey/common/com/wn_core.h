...
// Due to the addition of REGION_KIND_HICUDA, we need to change the way to
// determine if a region is EH.
...
typedef enum REGION_KIND {
    ...
    REGION_KIND_MP	        = 0x4,  // for OpenMP
    REGION_KIND_HICUDA      = 0x5,  // for HiCUDA
    REGION_KIND_RPI	        = 0x6,
    REGION_KIND_COLD	    = 0x7,
    REGION_KIND_SWP	        = 0x8,

    REGION_KIND_EH          = 0x9,	/* any kind >= 0x9 is EH */
    REGION_KIND_TRY         = 0x9,
    REGION_KIND_CLEANUP     = 0xa,
    REGION_KIND_EXC_SPEC    = 0xb,
    REGION_KIND_MASK        = 0xc,
    REGION_KIND_GUARD       = 0xd,
    REGION_KIND_NULL_CLEANUP = 0xe  /* cleanup with empty destructor */
} REGION_KIND;
...
/*REFERENCED*/
inline ST* WN_st(const WN *x)
{
    Is_True(WN_has_sym(x), ("WN_st: wn doesn't have ST field"));
    Is_True(ST_IDX_index(WN_st_idx(x)) != 0 || WN_st_idx(x) == 0,
            ("WN_st: zero index in nonzero level disallowed"));
    return (WN_st_idx(x) != 0 ? &St_Table[WN_st_idx(x)] : NULL);
}
...
/** DAVID CODE BEGIN **/
#define WN_pragma_hicuda(x) ((WN_pragma_flags(x)) & WN_PRAGMA_HICUDA)
#define WN_set_pragma_hicuda(x) ((WN_pragma_flags(x)) |= WN_PRAGMA_HICUDA)
#define WN_reset_pragma_hicuda(x) ((WN_pragma_flags(x)) &= ~(WN_PRAGMA_HICUDA))
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
#define WN_CALL_IS_KERNEL       0x4000  /* it is an execution configuration for a CUDA kernel */
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
#define WN_Call_Is_Kernel(x)        (WN_call_flag(x) & WN_CALL_IS_KERNEL)
#define WN_Set_Call_Is_Kernel(x)    (WN_call_flag(x) |= WN_CALL_IS_KERNEL)
#define WN_Reset_Call_Is_Kernel(x)  (WN_call_flag(x) &= ~WN_CALL_IS_KERNEL)
/*** DAVID CODE END ***/
