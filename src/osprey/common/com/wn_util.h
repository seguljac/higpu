...
extern WN_ITER* WN_WALK_TreeIter(WN*);
extern WN_ITER* WN_WALK_TreeNext(WN_ITER*);

extern WN_ITER* WN_WALK_StmtIter(WN*);
extern WN_ITER* WN_WALK_StmtNext(WN_ITER*);

extern WN_ITER* WN_WALK_SCFIter(WN *wn);
extern WN_ITER* WN_WALK_SCFNext(WN_ITER*);

extern void WN_WALK_Abort(WN_ITER*);

extern void WN_INSERT_BlockBefore(WN* b, WN* wn, WN* in);

#define WN_INSERT_BlockFirst(b, in) WN_INSERT_BlockBefore(b, WN_first(b), in)

extern void WN_INSERT_BlockAfter(WN* b, WN* wn, WN* in);

#define WN_INSERT_BlockLast(b, in) WN_INSERT_BlockAfter(b, WN_last(b), in)

extern WN* WN_COPY_Tree(WN *tree_node);

extern WN* WN_COPY_Tree_With_Map(WN *tree_node);

extern void WN_COPY_All_Maps(WN *dst, WN *src);

extern void WN_DELETE_Tree(WN* tree);

extern void WN_DELETE_FromBlock(WN *blck, WN *wn);
...
WN *WN_LOOP_UpperBound(const WN *loop, OPCODE *compare,
#ifdef KEY
        BOOL  enhanced = FALSE
#endif
        );
WN *WN_LOOP_Increment(const WN *loop, BOOL *is_incr);
...
extern "C" void Add_Pragma_To_MP_Regions(WN_VECTOR *wnv, 
        WN_PRAGMA_ID pragma_id, ST *st, WN_OFFSET ofst, WN_MAP parent_map,
        BOOL make_compiler_generated);
