...
void Configure_Source(char *filename)
{
    ...
    if (Mcmodel_Name == NULL) {
        ...
        if( strcmp( Mcmodel_Name, "small" ) == 0 )
            ...
            FmtAssert(FALSE,
                    ("code model `%s' not supported in the 64-bit mode",
                     Mcmodel_Name));
    }
#endif // TARG_X8664
    ...
    /* Since there seems to be little compile time reason not to be
     * aggressive, make the folder aggressive by default.
     */
    if (! Cfold_Aggr_Set) Enable_Cfold_Aggressive = TRUE;

    if (!Enable_CVT_Opt_Set) Enable_CVT_Opt = (Opt_Level > 0);
