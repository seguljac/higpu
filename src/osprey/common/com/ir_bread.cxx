...
/** DAVID CODE BEGIN **/
    case SHDR_EELIST:
	    EElist_Table.Transfer((EELIST*)addr, hdr.size/hdr.entsize);
	    break;
/*** DAVID CODE END ***/
...
WN_get_symtab(void *handle, PU_Info *pu)
{
    ...
    if (st == Subsect_InMem) return 0;
    if (st != Subsect_Exists) return ERROR_RETURN;
    ...
    if (lsymtab->size < sizeof(lsymtab)
            || lsymtab->entries < LOCAL_SYMTAB_TABLES
            || lsymtab->size > size) {
        errno = EINVAL;
        return ERROR_RETURN;
    }
...
Read_Local_Info(MEM_POOL *pool, PU_Info *pu)
{
    ...
    // Init them from the input WHIRL file.
