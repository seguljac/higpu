...
WN* WN_Create(OPERATOR opr, TYPE_ID rtype, TYPE_ID desc, mINT16 kid_count)
{
    ...
    /* Find the memory. First try a free list, then go to the MEM_POOL. For
     * safety, we use a bzero'd MEM_POOL and bzero new nodes popped from a
     * free list, but clients should not be relying on uninitialized fields
     * being zeroed.
     */
    WN_FREE_LIST *free_list = use_free_list ? Which_WN_FREE_LIST(size) : NULL;
    if (free_list != NULL && !WN_FREE_LIST_Empty(free_list)) {
        wn = WN_FREE_LIST_Pop(free_list);
    } else {
        // Init the mem pool if necessary.
        if (WN_mem_pool_ptr == &WN_mem_pool && !WN_mem_pool_initialized) {
            MEM_POOL_Initialize(WN_mem_pool_ptr, "WHIRL Nodes", TRUE);
            MEM_POOL_Push(WN_mem_pool_ptr);
            WN_mem_pool_initialized = TRUE;
...
WN* WN_CreateTriplet(WN *start_idx, WN *stride, WN *end_idx)
{
    WN *wn = WN_Create(OPC_U4TRIPLET, 3);
    
    WN_kid0(wn) = start_idx;
    WN_kid1(wn) = stride;
    WN_kid2(wn) = end_idx;
...
WN*
WN_CreateArrSection(WN *base_addr, int ndims) {
    BOOL valid_base = (base_addr != NULL);
    if (valid_base) {
        OPERATOR opr = WN_operator(base_addr);
        valid_base = (opr == OPR_LDA || opr == OPR_LDID);
    }

    Is_True(valid_base, ("Bad LDA node in an ARRSECTION!"));

    WN *wn = WN_Create(OPR_ARRSECTION, Pointer_type, MTYPE_V, ndims*2+1);
    WN_kid0(wn) = base_addr;
...
/** DAVID CODE BEGIN **/
    // Just to make sure this field is clear.
    WN_set_ty(wn, TY_IDX_ZERO);
/*** DAVID CODE END ***/
...
WN* WN_Enum(UINT64 value, TY_IDX enum_ty_idx) {
    /* The enum node uses the same opcode INTCONST because it is really just
     * an unsigned integer. It is not desirable to create another opcode for
     * enum.
     *
     * Therefore, the only way is to slip an extra type index here. However,
     * it relies on the assumption that all unused fields in the WHIRL spec
     * are zeroed, which is true in this implementation.
     */
    WN *wn = WN_Intconst(MTYPE_U4, value);
    WN_set_ty(wn, enum_ty_idx);
    return wn;
}

WN*
WN_LdidScalar(ST_IDX sym) {
    TY_IDX sym_ty_idx = ST_type(sym);
    return WN_Ldid(TY_mtype(sym_ty_idx), 0, sym, sym_ty_idx, 0);
}

WN*
WN_Ldid(TYPE_ID desc,
        WN_OFFSET offset, ST_IDX sym, TY_IDX align, UINT field_id) {
...
WN* WN_StidScalar(ST *sym, WN *value)
{
    TY_IDX sym_ty_idx = ST_type(sym);
    return WN_Stid(TY_mtype(sym_ty_idx), 0, sym, sym_ty_idx, value, 0);
}
...
WN_Lda(TYPE_ID rtype, WN_OFFSET ldaOffset, ST* sym, UINT field_id) {
    ...
    return WN_CreateLda(OPR_LDA, rtype, MTYPE_V,
                        ldaOffset, pty, sym, field_id);
}
