...
/** DAVID CODE BEGIN **/
    // We allow the combination of ST_IS_TEMP_VAR and SCLASS_EXTERN,
    // to indicate that it is a CUDA symbol declared in cuda_runtime.h.
    // NOTE: NOW WE DO NOT NEED IT ANYMORE!
    if (ST_is_temp_var(s)) {
        ST_SCLASS sc = ST_sclass(s);
        Is_True(sc == SCLASS_AUTO
                || sc == SCLASS_FORMAL
                || sc == SCLASS_FORMAL_REF,
                // || sc == SCLASS_EXTERN,
            (msg, "Storage class", Sclass_Name(sc), "ST_IS_TEMP_VAR")
        );
    }
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
        /* The 'fld' and 'etype' fields are used for EELIST and extra flags. */
