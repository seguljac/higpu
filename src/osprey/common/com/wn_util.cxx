...
/*****************************************************************************
 *
 * Insert node "in" (which could be a BLOCK) into BLOCK node "blck", before
 * node "wn". If "wn" is NULL, "in" is inserted at the end.
 *
 ****************************************************************************/
...
{
    /* Sanity checks */
    FmtAssert(in != NULL, ("Bad tree node"));
    OPCODE opc = WN_opcode(in);
    FmtAssert(OPCODE_is_stmt(opc) || OPCODE_is_scf(opc),
            ("Expecting a structured control flow node or a statement node"));

    FmtAssert(blck != NULL && WN_operator(blck) == OPR_BLOCK,
            ("Expecting a BLOCK operator"));

    /* Make sure that "wn" is in the block if it is non-NULL. The if statement
     * speeds up the check because an insertion near the end is often.
     */
    if (wn != NULL && wn != WN_last(blck)) {
        WN *node = WN_first(blck);
        while (node != NULL && node != wn) node = WN_next(node);
        /* BUG FIX: it is unsafe to assert node != NULL, because it would be
         * wrong if wn == NULL.
         */
        FmtAssert(node == wn, ("Illegal insert block operation"));
    }

    /* Get the first and the last node of "in", which are different only if
     * "in" is a BLOCK.
     */
    WN *first = NULL, *last = NULL;
    if (opc == OPC_BLOCK) {
        first = WN_first(in);
        // Handle the case if 'in' is an empty block.
        if (first == NULL) {
            // Just delete it.
            WN_Delete(in);
            return;
        }
        // Remove the skeleton block 'in'.
        last = WN_last(in);
    ...
    WN *prev = (wn == NULL) ? WN_last(blck) : WN_prev(wn);

    // Adjust previous and next pointers of 'in'.
    WN_prev(first) = prev;
    WN_next(last) = wn;

    // Adjust other pointers pointing to 'in'.
    if (prev == NULL) {
        WN_first(blck) = first;
    } else {
        WN_next(prev) = first;
...
/*****************************************************************************
 *
 * Recursively walk through <wn> and fills the given map that stores the
 * parent of each WN node.
 *
 ****************************************************************************/

void WN_Parentize(WN *wn, WN_MAP parent_map, WN_MAP_TAB *map_tab)
{
    ...
    if (WN_opcode(wn) == OPC_BLOCK)
    {
        WN* kid = WN_first(wn);
        while (kid != NULL) {
            WN_MAP_Set(parent_map, kid, (void*)wn);
...
#ifdef KEY
// Bug 10011
static BOOL type_ok (const TY_IDX ty)
{
    INT fld_count = 0;
