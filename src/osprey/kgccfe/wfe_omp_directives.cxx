...
WN* WFE_region(REGION_KIND kind)
{
    ...
    region  = WN_CreateRegion (kind, body, pragmas, exits, -1, 0);
...
/**
 * Check for 'for' directive node (for_wn).
 * Not to check the clause part of the directive, only the current block.
 */
void
WFE_check_for(WN *for_wn) {
    ...
    if (WN_operator(for_wn) != OPR_BLOCK) {
        ...
        WN *first_wn = WN_first(for_wn);
        if (first_wn && WN_operator(first_wn) == OPR_DO_LOOP) {
            // check that loop iteration variable is not threadprivate
            ST *index_st = WN_st(WN_index(first_wn));
            if (ST_is_thread_private(index_st)) {
                msg = "A for loop iteration variable may not appear \
...
WFE_expand_start_for(struct For_clause_wn_type *for_clause_wn) {
    ...
    WN *wn = WN_CreatePragma(WN_PRAGMA_PDO_BEGIN, (ST_IDX)NULL, 0, 0);
    WN_set_pragma_omp(wn);
    // The current block is the pragma block of 'region'.
    WFE_Stmt_Append(wn, Get_Srcpos());

    // Push the pragma onto the omp check stack.
    SRCPOS srcpos = Get_Srcpos();
    WFE_CS_push(wfe_omp_for, SRCPOS_linenum(srcpos), SRCPOS_filenum(srcpos));
    // Link it with the pragma block and the enclosing region.
    WFE_Set_Prag(WFE_Stmt_Top());
    WFE_Set_Region(region);
    ...
    // For private_clause
    st_list = for_clause_wn->private_clause;
    if (st_list) WFE_Set_Cflag(clause_private); 
    for ( ; st_list != NULL; st_list = st_list->next) {
        wn = WN_CreatePragma(WN_PRAGMA_LOCAL, st_list->st, 0, 0);
        WN_set_pragma_omp(wn);
        WFE_Stmt_Append(wn, Get_Srcpos());
    }

    // For lastprivate_clause
    st_list = for_clause_wn->lastprivate_clause;
    if (st_list) WFE_Set_Cflag(clause_lastprivate); 
    for ( ; st_list != NULL; st_list = st_list->next) {
        wn = WN_CreatePragma(WN_PRAGMA_LASTLOCAL, st_list->st, 0, 0);
        WN_set_pragma_omp(wn);
        WFE_Stmt_Append(wn, Get_Srcpos());
    }

    // For firstprivate_clause
    st_list = for_clause_wn->firstprivate_clause;
    if (st_list) WFE_Set_Cflag(clause_firstprivate); 
    for ( ; st_list != NULL; st_list = st_list->next) {
        wn = WN_CreatePragma(WN_PRAGMA_FIRSTPRIVATE, st_list->st, 0, 0);
        WN_set_pragma_omp(wn);
        WFE_Stmt_Append(wn, Get_Srcpos());
    }

    // For reduction_clause
    wn_list = for_clause_wn->reduction_clause;
    if (wn_list) WFE_Set_Cflag(clause_reduction);
    for ( ; wn_list != NULL; wn_list = wn_list->next) {
        wn = wn_list->wn;       // it's already a pragma
        WFE_Stmt_Append(wn, Get_Srcpos());
    }
    ...
    if (for_clause_wn->schedule_2_clause.schedule_2_kind != SK_NONE) { 
        ...
        wn = WN_CreateXpragma(WN_PRAGMA_CHUNKSIZE, (ST_IDX)NULL, 1);
    ...
    // We are done with the pragma block of this region.
    WFE_Stmt_Pop(wfe_stmk_region_pragmas);
}

void
WFE_expand_end_for() {
    // The top of the statement stack should be the body block of the region.

    // First do some checking on the for directive.
    WFE_check_for(WFE_Stmt_Top());
