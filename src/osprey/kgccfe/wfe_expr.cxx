...
/** DAVID CODE BEGIN **/
  HICUDA_MARKER_STMT,      "hicuda_marker_stmt",      'e', 0,  OPERATOR_UNKNOWN,
/*** DAVID CODE END ***/
#endif // KEY
...
#endif
        Is_True(! DECL_BIT_FIELD(arg1), ("WFE_Array_Expr: "
            "address arithmetic cannot use bitfield addresses"));
        INT64 ofst = (BITSPERBYTE * Get_Integer_Value(DECL_FIELD_OFFSET(arg1))
            + Get_Integer_Value(DECL_FIELD_BIT_OFFSET(arg1))) / BITSPERBYTE;
...
/** DAVID CODE BEGIN **/
            wn = WN_Lda (Pointer_Mtype, ST_ofst(st)+component_offset, st, 0);
/*** DAVID CODE END ***/
        if (component_ty_idx == 0)
            *ty_idx = ST_type(st);
        else {
...
/** DAVID CODE BEGIN **/
        // type associated with field_id
        TY_IDX hi_ty_idx = Get_TY(TREE_TYPE(lhs));
        if (hi_ty_idx != TY_IDX_ZERO && TY_kind(hi_ty_idx) == KIND_STRUCT
                && field_id > 0)
        {
            if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
            {
                fprintf(TFile, "STID: %s is a struct, field_id = %u\n",
                        ST_name(Get_ST(lhs)), field_id);
                fprintf(TFile, " hi_ty_idx: 0x%X => ", hi_ty_idx);
            }
            hi_ty_idx = get_field_type(hi_ty_idx, field_id);
            if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
            {
                fprintf(TFile, "0x%X\n", hi_ty_idx);
            }
...
/*** DAVID CODE END ***/
        ...
        Is_True(!is_bit_field || field_id <= MAX_FIELD_ID,
                ("WFE_Lhs_Of_Modify_Expr: "
                 "field id for bit-field exceeds limit"));
        ...
        if (rhs_wn == NULL) {
            ...
            if (MTYPE_byte_size(desc) < 4) {
                ...
                rhs_wn = WN_CreateCvtl(
                        !MTYPE_signed(desc) ? OPC_U4CVTL : OPC_I4CVTL,
                        MTYPE_bit_size(desc),
                        rhs_wn);
            }
#endif
...
/** DAVID CODE BEGIN **/
        ...
        if (hi_ty_idx != TY_IDX_ZERO
                && TY_kind(hi_ty_idx) == KIND_STRUCT
                && field_id > 0)
        {
...
/*** DAVID CODE END ***/
    ...
    else if (code == ARRAY_REF)
    {
        ...
        if (TY_is_volatile(elem_ty_idx)) volt = TRUE;

        TY_IDX desc_ty_idx = (component_ty_idx != TY_IDX_ZERO) ?
            component_ty_idx : Get_TY(TREE_TYPE(lhs));
        if (TY_is_volatile(desc_ty_idx)) {
            Clear_TY_is_volatile(desc_ty_idx);
            volt = TRUE;
        }
...
/** DAVID CODE BEGIN **/
            if (is_realpart || is_imagpart)
            {
                if (assign_code == MODIFY_EXPR)
                {
#if 0
                    WN *iload_wn = WN_CreateIload(OPR_ILOAD, rtype, desc,
                            component_offset,
                            ...
                            field_id);
#else
                    TY_IDX load_ty_idx = (field_id == 0) ?
                            elem_ty_idx : desc_ty_idx;
                    WN *iload_wn = WN_CreateIload(OPR_ILOAD, rtype, desc,
                            component_offset,
                            load_ty_idx, Make_Pointer_Type(load_ty_idx),
                            WN_COPY_Tree(addr_wn), 0);
#endif

                    OPERATOR opr = is_realpart ? OPR_IMAGPART : OPR_REALPART;
                    rhs_wn = WN_Binary(OPR_COMPLEX, rtype, rhs_wn,
                            WN_Unary(opr,
                                Mtype_complex_to_real(rtype), iload_wn));
                }
                else
                {
                    WN *fzero = WN_Floatconst(
                            Mtype_complex_to_real(rtype), 0.0);

                    rhs_wn = is_realpart ?
                            WN_Binary(OPR_COMPLEX, rtype, rhs_wn, fzero) :
                            WN_Binary(OPR_COMPLEX, rtype, fzero, rhs_wn);
                }
            }
/*** DAVID CODE END ***/
...
/*** DAVID CODE END ***/
        ...
        // the assignment
        if (!WFE_Keep_Zero_Length_Structs && desc == MTYPE_M
...
/** DAVID CODE BEGIN **/
            wn = WN_CreateIstore(OPR_ISTORE, MTYPE_V, desc,
                    ...
                    // Make_Pointer_Type(elem_ty_idx, FALSE),
                    Make_Pointer_Type(
                        field_id != 0 ? desc_ty_idx : elem_ty_idx),
                    rhs_wn, addr_wn,
                    // field_id);
                    0);
/*** DAVID CODE END ***/
            ...
            if (need_result) {
                if (result_in_temp) {
                    wn = WN_Ldid(rtype,
                            result_preg, result_preg_st, desc_ty_idx, 0);
                } else {
/** DAVID CODE BEGIN **/
#if 0
                    wn = WN_CreateIload(OPR_ILOAD, rtype, desc,
...
/*** DAVID CODE END ***/
                ...
                if (is_realpart)
                    wn = WN_Unary(OPR_REALPART,
                            Mtype_complex_to_real(rtype), wn);
                else if (is_imagpart)
                    wn = WN_Unary(OPR_IMAGPART,
                            Mtype_complex_to_real (rtype), wn);
            }
            else wn = NULL;
        }
...
/** DAVID CODE BEGIN **/
                ...
                case FUNCTION_DECL:
                {
                    ...
                    ty_idx = ST_type(st);
#ifdef KEY
                    // Taking the address of a nested function requires the
                    // use of a trampoline dynamically allocated on the stack
                    if (code0 == FUNCTION_DECL
                            && PU_is_nested_func(Pu_Table[ST_pu(st)])) {
                        Set_PU_has_alloca(Get_Current_PU());
                    }
#endif
                    if (code0 == VAR_DECL && st != ST_base(st))
                    {
                        // for VLAs, use the base_st instead of st
                        FmtAssert(ST_ofst(st) == 0,
                                ("Variable Length Arrays within struct "
                                 "not currently implemented"));
                        wn = WN_Ldid(Pointer_Mtype, 0,
                                ST_base(st), ST_type(ST_base(st)));
                    }
                            ...
                            && TY_size(ty_idx) == 0)
                    {
                        ...
                        DevWarn("taking address of zero length struct %s "
                                "at line %d", ST_name(st), lineno);
...
/** DAVID CODE BEGIN **/
            ...
            if (hi_ty_idx != TY_IDX_ZERO && TY_kind(hi_ty_idx) == KIND_STRUCT)
            {
                if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
                {
                    fprintf(TFile, "LDID: %s is a struct, field_id = %u\n",
                            ST_name(Get_ST(exp)), field_id);
                }
                if (field_id > 0)
                {
                    is_struct_field = TRUE;
            ...
            if (desc_ty_idx == TY_IDX_ZERO) desc_ty_idx = hi_ty_idx;

            ty_idx = desc_ty_idx;
            if (nop_ty_idx != TY_IDX_ZERO
                    && MTYPE_is_integral(TY_mtype(desc_ty_idx))
                    /* For regular integer conversion, we don't do it here. */
                    && is_bit_field) {
                ty_idx = nop_ty_idx;
            }

            // if non-zero, need to generate CVTL with this size
            UINT cvtl_size = 0;
            if (! is_bit_field) {
                ...
                if (TY_size(desc_ty_idx) > TY_size(ty_idx)) ty_idx = desc_ty_idx;
            }
            ...
            if (MTYPE_is_integral(desc)) {
                // 'rtype' and 'desc' must have the same sign.
...
/*** DAVID CODE END ***/
        ...
        case COMPONENT_REF:
        {
            ...
            ty_idx = (component_ty_idx == TY_IDX_ZERO) ?
                Get_TY(TREE_TYPE(exp)) : component_ty_idx;

            if (DECL_BIT_FIELD(arg1)) is_bit_field = TRUE;

            if (!is_bit_field && component_ty_idx == TY_IDX_ZERO)
            {
                // only for top-level COMPONENT_REF
...
/** DAVID CODE BEGIN **/
            ...
            // Type of the actual object being loaded
            desc_ty_idx = component_ty_idx;
            if (desc_ty_idx == TY_IDX_ZERO) desc_ty_idx = hi_ty_idx;
            ...
            if (! MTYPE_is_integral(TY_mtype(desc_ty_idx))) {
                ...
                ty_idx = nop_ty_idx;
                if (ty_idx == TY_IDX_ZERO) ty_idx = desc_ty_idx;
            }
/*** DAVID CODE END ***/
            ...
            else if (TY_size(desc_ty_idx) > TY_size(ty_idx))
            {
                ty_idx = desc_ty_idx;
            }
...
/** DAVID CODE BEGIN **/
                /* Even for structs, the two types we give are the field's,
                 * not the entire struct's. Also, we give field_id 0 because
                 * it does not appear to be used in whirl2c. I don't know
                 * the "standard" way of specifying an ILOAD. */
                wn = WN_CreateIload(OPR_ILOAD, rtype,
                        ...
                        iload_ty_idx,
                        Make_Pointer_Type(iload_ty_idx),
#endif
                        // Make_Pointer_Type(hi_ty_idx, FALSE),
                        wn0,
                        // field_id
...
/*** DAVID CODE END ***/
        ...
        case POSTINCREMENT_EXPR:
        {
            ...
            wn  = WFE_Lhs_Of_Modify_Expr(code, TREE_OPERAND(exp, 0),
                    need_result, 0, 0, 0, FALSE, wn1, 0, FALSE, FALSE);
        }
        break;
        ...
        case ARRAY_REF:
        {
            ...
            desc_ty_idx = (component_ty_idx == TY_IDX_ZERO) ?
                hi_ty_idx : component_ty_idx;

            ty_idx = (MTYPE_is_integral(TY_mtype(desc_ty_idx))
                    && (nop_ty_idx != TY_IDX_ZERO)) ?
                nop_ty_idx : desc_ty_idx;
...
/** DAVID CODE BEGIN **/
            ...
            wn = WN_CreateIload(OPR_ILOAD, rtype,
                    ...
                    ty_idx,
                    // Make_Pointer_Type(elem_ty_idx, FALSE),
                    Make_Pointer_Type(field_id != 0 ? ty_idx : elem_ty_idx),
                    wn0,
                    // field_id);
                    0);
