...
void extern
WFE_CS_push(WFE_CHECK_KIND kind, int lnum, int fnum) {
    int new_stack_size;

    if (omp_check_sp == omp_check_last) {
        new_stack_size = ENLARGE(omp_check_size);
        omp_check_stack = (CHECK_STMT*)realloc(
            omp_check_stack, new_stack_size*sizeof(CHECK_STMT));
...
extern WFE_CHECK_KIND
WFE_CS_pop(WFE_CHECK_KIND kind) {
