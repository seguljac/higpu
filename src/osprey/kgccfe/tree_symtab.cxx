...
/** DAVID CODE BEGIN **/
static TY_IDX long_ty_idx = TY_IDX_ZERO;
static TY_IDX ulong_ty_idx = TY_IDX_ZERO;
static TY_IDX char_ty_idx = TY_IDX_ZERO;
/*** DAVID CODE END ***/
...
{
    if (TREE_CODE(type_tree) == ERROR_MARK) exit(RC_USER_ERROR);
...
/** DAVID CODE BEGIN **/
        ...
        if (TYPE_READONLY(type_tree)) Set_TY_is_const(idx);
        if (TYPE_VOLATILE(type_tree)) Set_TY_is_volatile(idx);

#ifdef KEY
        if (TYPE_RESTRICT(type_tree)) Set_TY_is_restrict(idx);
...
#endif
#else
            Fail_FmtAssertion("Variable-length array (at line %d) "
                    "is not supported\n", lineno);
#endif
/*** DAVID CODE END ***/
			variable_size = TRUE;
	...
	switch (TREE_CODE(type_tree))
    {
        ...
        case INTEGER_TYPE:
            ...
            if (TREE_UNSIGNED(type_tree)) mtype = MTYPE_complement(mtype);
...
/** DAVID CODE BEGIN **/
            ...
            /* The 'long int' type has an arch-dependent size; we need this
             * info to distinguish between 'long' and 'long long'; otherwise,
             * code generation for function signatures will be incorrect.
             */
            if (TYPE_IS_LONG_INT(type_tree)) {
                if (MTYPE_is_unsigned(mtype)) {
                    if (ulong_ty_idx == TY_IDX_ZERO) {
                        TY &ty = New_TY(ulong_ty_idx);
                        TY_Init(ty, tsize, KIND_SCALAR, mtype,
                                Save_Str(".special_ulong"));
                        Set_TY_is_long_int(ty);
                    }
                    idx = ulong_ty_idx;
                } else {
                    if (long_ty_idx == TY_IDX_ZERO) {
                        TY &ty = New_TY(long_ty_idx);
                        TY_Init(ty, tsize, KIND_SCALAR, mtype,
                                Save_Str(".special_long"));
                        Set_TY_is_long_int(ty);
                    }
                    idx = long_ty_idx;
                }
            } else if (TYPE_IS_CHAR(type_tree)) {
                // This is 'char' with no sign specification.
                mtype = MTYPE_I1;
                if (char_ty_idx == TY_IDX_ZERO) {
                    TY &ty = New_TY(char_ty_idx);
                    TY_Init(ty, tsize, KIND_SCALAR, mtype,
                            Save_Str(".special_char"));
                    Set_TY_is_char(ty);
                }
                idx = char_ty_idx;
            } else {
...
/*** DAVID CODE END ***/
        ...
        case REAL_TYPE:
            switch (tsize) {
                ...
                default:
                    FmtAssert(FALSE,
                             ("Get_TY unexpected REAL_TYPE size %d", tsize));
            }
            idx = MTYPE_To_TY(mtype);	// use predefined type
            break;
        ...
        case ARRAY_TYPE:
		{
            ...
            TY_Init(ty, tsize, KIND_ARRAY, MTYPE_M, 
                    Save_Str(Get_Name(TYPE_NAME(type_tree))));
            if(TYPE_NAME(type_tree) == NULL) Set_TY_anonymous(ty);

// #define MULTI_DIM_ARB

#ifdef MULTI_DIM_ARB
            UINT16 ndims = 0;
            tree tmp_type_tree = type_tree;
            tree tmp_type_size = type_size;
            ...
            while (1)
            {
                ...
                tree elem_type_tree = TREE_TYPE(tmp_type_tree);
                tree elem_sz_tree = TYPE_SIZE(elem_type_tree);
                ...
                if(elem_sz_tree == NULL_TREE) goto skip_set_arb;
#endif
                if(TREE_CODE(elem_sz_tree) == INTEGER_CST) {
                    Set_ARB_const_stride(arb);
                    Set_ARB_stride_val(arb, 
                            Get_Integer_Value(elem_sz_tree) / BITSPERBYTE);
                }
                else {
                    WN *swn = WFE_Expand_Expr(elem_sz_tree);
                    ...
                    if(WN_operator(swn) != OPR_LDID) {
                        TY_IDX ty_idx = Get_TY(TREE_TYPE(tmp_type_size));
                ...
                if(tmp_type_size != NULL_TREE) {
                    tree ubnd_tree = TYPE_MAX_VALUE(TYPE_DOMAIN(tmp_type_tree));
#ifdef KEY
                    // For Zero-length arrays, TYPE_MAX_VALUE tree is NULL
                    if(ubnd_tree == NULL_TREE) {
                        Set_ARB_const_ubnd(arb);
                        Set_ARB_ubnd_val(arb, 0xffffffff);
                    } else
#endif /* KEY */
                    if(TREE_CODE(ubnd_tree) == INTEGER_CST) {
                        Set_ARB_const_ubnd(arb);
                        Set_ARB_ubnd_val(arb, Get_Integer_Value(ubnd_tree));
                    }
#ifdef KEY
                    // bug 4086: see comments "throw away any variable
                    // type sizes ..." in finish_decl().
                    else if(!TYPE_DEFER_EXPANSION(tmp_type_tree))
#else
                    else
#endif
                    {
                        WN *uwn = WFE_Expand_Expr(ubnd_tree);
...
#ifdef KEY
                        // bug 4086
                        && !TYPE_DEFER_EXPANSION(tmp_type_tree)
#endif
                   )
                {
                    WN *swn, *wn;
                    swn = WFE_Expand_Expr(tmp_type_size);
...
skip_set_arb:
                // Work on the element type if it is an array.
                tmp_type_tree = elem_type_tree;
                tmp_type_size = elem_sz_tree;
                if (TREE_CODE(tmp_type_tree) != ARRAY_TYPE) {
                    Set_ARB_last_dimen(arb);
                    break;
                }
            }
            // Now 'tmp_type_tree' is the actual element type.

            TY_IDX elem_ty_idx = Get_TY(tmp_type_tree);
            Set_TY_etype(ty, elem_ty_idx);
            Set_TY_align(idx, TY_align(elem_ty_idx));

            // Update the dimension field in each ARB.
            ...
            tree elem_type_tree = TREE_TYPE(type_tree);
            TY_IDX elem_ty_idx = Get_TY(elem_type_tree);
            Set_TY_etype(ty, elem_ty_idx);
            Set_TY_align(idx, TY_align(elem_ty_idx));

            // We assume 1-D. A multi-dim array is an array of array.
            ARB_HANDLE arb = New_ARB();
            ARB_Init(arb, 0, 0, 0);
            Set_TY_arb(ty, arb);
            Set_ARB_first_dimen(arb);
            ...
            // For example, extern struct bar_t bar[]; 
            //(don't know size of struct bar_t)
            if(elem_sz_tree == NULL_TREE) break;
#endif
            if(TREE_CODE(elem_sz_tree) == INTEGER_CST) {
                Set_ARB_const_stride(arb);
                Set_ARB_stride_val(arb,
            ...
            if(type_size != NULL_TREE) {
                tree ubnd_tree = TYPE_MAX_VALUE(TYPE_DOMAIN(type_tree));
#ifdef KEY
                // For Zero-length arrays, TYPE_MAX_VALUE tree is NULL
                if(ubnd_tree == NULL_TREE) {
                    Set_ARB_const_ubnd(arb);
...
// inlined.
            ...
            {
                Set_TY_size(ty_idx,
                        TY_size(Get_TY(TREE_TYPE(TREE_TYPE (decl_node)))));
            }
#ifndef KEY
            ...
            if (TREE_READONLY(decl_node)) Set_TY_is_const(ty_idx);
            if (TREE_THIS_VOLATILE(decl_node)) Set_TY_is_volatile(ty_idx);
