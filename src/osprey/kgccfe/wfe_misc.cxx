...
// defined in gnu/toplev.c
extern int flag_debug_hicuda;
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
            Set_Trace_File(Trc_File_Name, FALSE);   // overwrite
            if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
            {
                fprintf(TFile, "\n%s%s\tHICUDA TRACES IN GFEC\n%s%s\n",
...
/** DAVID CODE BEGIN **/
    // Set the trace flag.
    if (flag_debug_hicuda)
    {
        Tracing_Enabled = TRUE;
        Set_Trace(TKIND_DEBUG, TDEBUG_HICUDA);
    }
/*** DAVID CODE END ***/
} /* WFE_Init */
...
WFE_Stmt_Push(WN* wn, WFE_STMT_KIND kind, SRCPOS srcpos) {
    ...
    // Reallocate the statement stack if too small.
    if (wn_stmt_sp == wn_stmt_stack_last) {
        new_stack_size = ENLARGE(wn_stmt_stack_size);
        wn_stmt_stack = (WN_STMT*)realloc(
            wn_stmt_stack, new_stack_size*sizeof(WN_STMT));
    ...
    if (srcpos) WN_Set_Linenum(wn, srcpos);
} /* WFE_Stmt_Push */
...
WFE_Stmt_Append(WN* wn, SRCPOS srcpos) {
    ...
    if (srcpos) {
        ...
        if (WN_operator(wn) == OPR_BLOCK) {
            WN *first_wn = WN_first(wn);
            if (first_wn) WN_Set_Linenum(first_wn, srcpos);
        }
    }
...
/** DAVID CODE BEGIN **/
    if (wn_stmt_sp->kind != kind)
    {
        // More informative error message for a hiCUDA region.
        if (wn_stmt_sp->kind == wfe_stmk_hc_region_body)
        {
            // Go to the parent REGION node and see what type of hiCUDA region
            // is this.
            WN_STMT *parent_stmt_sp = wn_stmt_sp - 1;
            FmtAssert(parent_stmt_sp >= wn_stmt_stack, (""));
            WN *region_wn = parent_stmt_sp->wn;
            FmtAssert(region_wn != NULL
                    && WN_operator(region_wn) == OPR_REGION
                    && WN_region_kind(region_wn) == REGION_KIND_HICUDA, (""));
            WN *pragma_blk_wn = WN_region_pragmas(region_wn);
            FmtAssert(pragma_blk_wn != NULL
                    && WN_operator(pragma_blk_wn) == OPR_BLOCK, (""));
            WN *pragma_wn = WN_first(pragma_blk_wn);
            // We only look at the first pragma.
            FmtAssert(pragma_wn != NULL && WN_pragma_hicuda(pragma_wn), (""));
            WN_PRAGMA_ID pid = (WN_PRAGMA_ID)WN_pragma(pragma_wn);
            // For now, the only thing we need to handle is a kernel region.
            if (pid == WN_PRAGMA_HC_KERNEL)
            {
                SRCPOS sp = WN_Get_Linenum(pragma_wn);
                HC_assert(FALSE,
                        ("Missing the kernel_end directive for kernel <%s> "
                         "(line %d)",
                         ST_name(WN_st_idx(pragma_wn)), SRCPOS_linenum(sp)));
            }
        }
