...
Get_Pre_ST(tree name) {
    if (name != NULL) return WN_st(WFE_Expand_Expr(name));

    SRCPOS srcpos = Get_Srcpos();
    // Do not understand this line.
    WFE_CS_push(wfe_omp_master,
...
/**
 * Converting a list of for clause tree nodes into ST's.
 */
void
prepare_com_clause(tree clause_tree, ST_list **clause_st) {
    register tree t = NULL;
    ST_list *stlist = NULL;

    for (t = clause_tree; t != NULL; t = TREE_CHAIN(t)) {      
       	stlist = (struct ST_list*)malloc(sizeof(struct ST_list));
        stlist->st = Get_Pre_ST(TREE_VALUE(t));
...
/**
 * Convert a list of reduction-clause tree nodes into WHIRL pragmas.
 */
void
prepare_reduction_clause(struct reduction_list *reduction_clause_list,
    ...
    while (reduction_clause_list) {   
        ...
        for (t = reduction_clause_list->node.var_list; t; t = TREE_CHAIN(t)) {
            ...
            // One pragma for each variable (in all reduction clauses)
            // This is not put in WFE_expand_start_for because of 'opcode'.
            // is st of type 'ST_IDX'?
...
/**
 * Sanity check for a for directive.
 * Make sure that there is at most one SCHEDULE, ORDERED or NOWAIT clause.
 */
void
check_for_directive(struct for_clause_list *clause_list) {
    ...
    if (count_schedule > 1 || count_ordered > 1 || count_nowait > 1) {
        if (count_schedule > 1) printf("Too many SCHEDULE clausees.\n");
        if (count_ordered > 1) printf("Too many ORDERED clausees.\n");
        if (count_nowait > 1) printf("Too many NOWAIT clausees.\n");
          
        SRCPOS srcpos = Get_Srcpos();
        Fail_FmtAssertion("Invalid syntax in the #PRAGMA OMP FOR directive \
            at line: %d, file number: %d.!\n",
            SRCPOS_linenum(srcpos), SRCPOS_filenum(srcpos));
    }
}

/**
 * Convert 'clause_list' to For_clause_wn_type and call WFE_expand_start_for.
 */
void
expand_start_for(struct for_clause_list *clause_list) {
    struct For_clause_wn_type *for_clause_wn;
 
    /* GNU IR data structure */
    ...
    struct schedule_2 schedule_2_clause_value = { SK_NONE, NULL };
    bool nowait_clause_value = false;

    struct for_clause_list *cl = NULL;
    struct reduction_list *rl = NULL;

    /* WHIRL data structure */
    ST_list *private_clause_st = NULL;
    ST_list *firstprivate_clause_st = NULL;
    ST_list *lastprivate_clause_st = NULL;
    WN_list *reduction_clause_wn = NULL;
    struct schedule_2_wn schedule_2_clause_wn = { SK_NONE, NULL };
    ...
    for (cl = clause_list; cl != NULL; cl = cl->next) {
        switch (cl->type) {
            case f_private:
                private_clause_tree = chainon(private_clause_tree,
                    cl->node.var_list);
                break;
            case f_firstprivate:
                firstprivate_clause_tree = chainon(firstprivate_clause_tree,
                    cl->node.var_list);
                break;
            case f_lastprivate:
                lastprivate_clause_tree = chainon(lastprivate_clause_tree,
                    cl->node.var_list);
                break;
            case f_reduction:
           	    rl = (struct reduction_list*)malloc(
                    sizeof(struct reduction_list));
                rl->node = cl->node.reduction_node;
                rl->next = reduction_clause_list;
                reduction_clause_list = rl;
                break;
            ...
            case f_schedule_2:
                schedule_2_clause_value.schedule_kind =
                    cl->node.schedule_node.schedule_kind;
                schedule_2_clause_value.chunk_size =
                    cl->node.schedule_node.chunk_size;
                break;
            case f_nowait:
    ...
    if (schedule_2_clause_value.schedule_kind != SK_NONE) {
        schedule_2_clause_wn.schedule_2_kind =
            schedule_2_clause_value.schedule_kind;
        schedule_2_clause_wn.chunk_size_wn = 
            WFE_Expand_Expr(schedule_2_clause_value.chunk_size);
    }

    for_clause_wn = (struct For_clause_wn_type*)malloc(
        sizeof(For_clause_wn_type));
