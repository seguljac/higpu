...
/** DAVID CODE BEGIN **/
    struct hicuda_kernel_part *hc_kernel_part_type;
    enum kernel_part_distr_type hc_kernel_part_distr_type_type;
    struct virtual_space *hc_virtual_space_type;
    /* struct hicuda_loopblock *hc_loopblock_dir_type; */

    struct hicuda_global_shared *hc_global_dir_type;
    struct hicuda_global_shared *hc_shared_dir_type;
    struct hicuda_const *hc_const_dir_type;
    struct hicuda_shape *hc_shape_type;
    struct copy_arr_region *hc_copy_arr_region_type;
    struct arr_region *hc_arr_region_type;
    struct idx_range_list *hc_idx_range_type;
    struct free_data_list *hc_free_data_type;
    struct dim_sz_list *hc_shape_dim_type;
/*** DAVID CODE END ***/
	}
...
/** DAVID CODE BEGIN **/
%token PRAGMA_HICUDA
%token HC_KERNEL_START HC_KERNEL_END HC_BLOCK HC_THREAD
%token HC_LOOP_PART HC_OVER_TBLOCK HC_OVER_THREAD
%token HC_DISTR_BLOCK HC_DISTR_CYCLIC
%token HC_LOOPBLOCK
%token HC_GLOBAL HC_DECL HC_COPYIN HC_CLEAR HC_COPYOUT HC_NOBNDCHECK
%token HC_SHARED HC_CONST HC_REMOVE HC_FREE
%token HC_SHAPE
%token HC_BARRIER
%token HC_IDENTIFIER
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
%type <ttype> HC_IDENTIFIER
%type <ttype> hicuda_construct hicuda_directive hicuda_directive_body
%type <ttype> hc_kernel_begin_directive hc_kernel_end_directive
%type <ttype> hc_kernel_part_construct hc_kernel_part_directive
/* %type <ttype> hc_loopblock_construct */
%type <ttype> hc_global_directive hc_global_free_directive
%type <ttype> hc_shared_directive hc_shared_remove_directive
%type <ttype> hc_const_directive hc_const_remove_directive
%type <ttype> hc_shape_directive
%type <ttype> hc_barrier_directive

%type <hc_kernel_part_type> hc_kernel_part_dir_body
%type <hc_kernel_part_distr_type_type> hc_distr_type hc_explicit_distr_type

%type <hc_virtual_space_type> hc_virtual_space
%type <hc_virtual_space_type> hc_block_clause hc_thread_clause

/* %type <hc_loopblock_dir_type> hc_loopblock_directive */

%type <hc_global_dir_type> hc_global_clause

%type <hc_shared_dir_type> hc_shared_clause

%type <hc_arr_region_type> hc_arr_region hc_opt_arr_region hc_arr_region_list
%type <hc_arr_region_type> hc_decl_clause
%type <hc_copy_arr_region_type> hc_global_copyin_clause
%type <hc_copy_arr_region_type> hc_global_copyout_clause
%type <hc_copy_arr_region_type> hc_shared_copyin_clause
%type <hc_copy_arr_region_type> hc_shared_copyout_clause
%type <itype> hc_shared_nobndcheck_clause
/* %type <hc_arr_region_type> hc_loopblock_copyin_clause */

%type <hc_idx_range_type> hc_idx_range hc_opt_idx_range_list

%type <hc_free_data_type> hc_free_var hc_free_var_list

%type <hc_shape_type> hc_shape hc_shape_list
%type <hc_shape_dim_type> hc_shape_dim hc_shape_dim_list
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
// no check for HiCUDA keyword for the next token
static bool hc_no_keyword_check = FALSE;

extern bool in_hicuda_pragma;
extern bool hc_inside_parens;
extern bool hc_dollar_sign_prefix;
...
program:
    /* empty */
    {
        if (pedantic) pedwarn ("ISO C forbids an empty source file");
        finish_file();
    }
	|
    extdefs
    {
        ...
        while (! global_bindings_p()) poplevel(0, 0, 0);

        /* __FUNCTION__ is defined at file scope ("").  This call may not
         * be necessary as my tests indicate it still works without it. */
        finish_fname_decls ();

/** DAVID CODE BEGIN **/
        save_global_types_and_symbols();
        decl_all_types_and_symbols();
/*** DAVID CODE END ***/
...
/* DAVID FIX: $$ => $<ttype>$ (detected by bison 2.4) */
structsp_attr:
...
hicuda_construct:
    hc_kernel_part_construct
    /* | hc_loopblock_construct */
    ;

hicuda_directive:
    PRAGMA_HICUDA save_filename save_lineno hicuda_directive_body
    {
        finish_hicuda_stmt($4, $3, $2);
        add_stmt($4);

        // A hiCUDA tree stmt is different from a regular statement: it has
        // custom data structure that does not have tree_exp and therefore
        // STMT_LINENO field.
        // STMT_LINENO($4) = $3;

        // The semantic value must be NULL, otherwise weird things happen.
        $$ = NULL;
    }
    ;

hicuda_directive_body:
    hc_kernel_begin_directive
    | hc_kernel_end_directive
    | hc_global_directive
    | hc_global_free_directive
    | hc_shared_directive
    | hc_shared_remove_directive
    | hc_const_directive
    | hc_const_remove_directive
    | hc_shape_directive
    | hc_barrier_directive
    ;
...
hc_shape_directive:
    HC_SHAPE
    {
        hc_no_keyword_check = TRUE;
    }
    hc_shape_list '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_SHAPE_DIR, $3);
        hc_no_keyword_check = FALSE;
    }
    ;
...
hc_shape_list:
    ...
    |
    hc_shape hc_shape_list
    {
        chain_shape_list($1, $2);
        $$ = $1;
    }
    ;

hc_shape:
    IDENTIFIER hc_shape_dim_list
    {
        // Convert an identifier into a TREE node.
        // Is build_external_ref better?
        tree var = lookup_name($1);
        if (var == NULL)
        {
            error("Undefined variable %s", IDENTIFIER_POINTER($1));
        }

        $$ = build_hicuda_shape(var, $2);
    }
    ;
...
hc_shape_dim_list:
    ...
    |
    hc_shape_dim hc_shape_dim_list
    {
        chain_dim_sz_list($1, $2);
        $$ = $1;
    }
    ;
...
// NOTE: a mid-rule action is counted as a component.
hc_global_directive:
    HC_GLOBAL hc_global_clause '\n'
    {
        if ($2->decl != NULL)
        {
            // GLOBAL COPYIN
            $$ = build_hicuda_stmt(HICUDA_GLOBAL_COPYIN_DIR, $2);
        }
        else
        {
            // GLOBAL COPYOUT
            $$ = build_hicuda_stmt(HICUDA_GLOBAL_COPYOUT_DIR, $2);
        }
    }
    ;
...
hc_global_clause:
    hc_decl_clause
    {
        $$ = build_hicuda_global_shared($1, NULL, FALSE);
    }
    | hc_decl_clause hc_global_copyin_clause
    /* must separate from the prev rule */
    {
        $$ = build_hicuda_global_shared($1, $2, FALSE);
    }
    | hc_decl_clause HC_CLEAR
    {
        $$ = build_hicuda_global_shared($1, NULL, TRUE);
    }
    | hc_global_copyout_clause
    {
        $$ = build_hicuda_global_shared(NULL, $1, FALSE);
    }
    ;
...
hc_global_copyin_clause:
    HC_COPYIN
    {
        // Since the copyin clause is at the end of a directive, it is OK
        // to leave this flag TRUE when there is no array region followed.
        hc_no_keyword_check = TRUE;
    }
    hc_opt_arr_region
    {
        // nobndcheck is meaningless here.
        $$ = build_copy_arr_region($3, TRUE);
    }
    ;
...
hc_opt_idx_range_list:
    ...
    | hc_idx_range hc_opt_idx_range_list
    {
        chain_idx_range_list($1, $2);
        $$ = $1;
    }
    ;
...
hc_idx_range:
    ...
    '[' expr ']'
    {
        $$ = build_idx_range($2, NULL);
    }
    |
    '[' '*' ']'
    {
        $$ = build_idx_range(NULL, NULL);
    }
    ;
...
hc_global_free_directive:
    ...
    hc_free_var_list '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_GLOBAL_FREE_DIR, $4);
    }
    ;

hc_shared_directive:
    HC_SHARED hc_shared_clause '\n'
    {
        if ($2->decl != NULL)
        {
            // SHARED COPYIN
            $$ = build_hicuda_stmt(HICUDA_SHARED_COPYIN_DIR, $2);
        }
        else
        {
            // SHARED COPYOUT
            $$ = build_hicuda_stmt(HICUDA_SHARED_COPYOUT_DIR, $2);
        }
    }
    ;
...
hc_shared_clause:
    ...
    | hc_decl_clause hc_shared_copyin_clause
    /* must separate from the prev rule */
    {
        $$ = build_hicuda_global_shared($1, $2, FALSE);
    }
    | hc_shared_copyout_clause
    {
        $$ = build_hicuda_global_shared(NULL, $1, FALSE);
    }
    ;

hc_shared_copyin_clause:
    HC_COPYIN hc_shared_nobndcheck_clause
    {
        hc_no_keyword_check = TRUE;
    }
    hc_opt_arr_region
    {
        // Is this safe?
        hc_no_keyword_check = FALSE;
        $$ = build_copy_arr_region($4, ($2 == 1) ? TRUE : FALSE);
    }
    ;

/* 1 if the flag is present and 0 otherwise */
hc_shared_nobndcheck_clause:
    /* empty */
    {
        $$ = 0;
    }
...
hc_shared_copyout_clause:
    HC_COPYOUT hc_shared_nobndcheck_clause
    {
        hc_no_keyword_check = TRUE;
    }
    hc_arr_region
...
hc_shared_remove_directive:
    ...
    hc_free_var_list '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_SHARED_REMOVE_DIR, $4);
    }
    ;
...
hc_const_directive:
    ...
    hc_arr_region_list '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_CONST_COPYIN_DIR,
                build_hicuda_const($4));
    }
    ;

/* a non-empty list of array regions */
hc_arr_region_list:
    hc_arr_region
    {
        $$ = $1;
...
hc_const_remove_directive:
    ...
    hc_free_var_list '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_CONST_REMOVE_DIR, $4);
    }
    ;

/* A list of at least one hc_free_var. */
hc_free_var_list:
    hc_free_var
    {
        $$ = $1;
    ...
    |
    hc_free_var hc_free_var_list
    {
        chain_free_data_list($1, $2);
        $$ = $1;
    }
    ;

/* A variable identifier whose corresponding global/shared variable
 * will be freed. */
hc_free_var:
    IDENTIFIER
    {
...
hc_barrier_directive:
    HC_BARRIER '\n'
    {
        $$ = build_hicuda_stmt(HICUDA_BARRIER_DIR, NULL);
    }
    ;
...
hc_kernel_begin_directive:
    ...
    hc_block_clause hc_thread_clause '\n'
    {
        new_hc_kernel_scope();

        // The kernel name is represented as an IDENTIFIER.
        $$ = build_hicuda_stmt(HICUDA_KERNEL_DIR_BEGIN,
                build_hicuda_kernel($3, $5, $6));
    }
    ;
    
hc_block_clause:
    HC_BLOCK '(' hc_virtual_space ')'
    {
        $$ = $3;
    }
    ;
...
hc_virtual_space:
    expr_no_commas
    {
        $$ = build_virtual_space($1);
    }
    | hc_virtual_space ',' expr_no_commas
    {
        chain_virtual_space($1, build_virtual_space($3));
        $$ = $1;
    }
    ;
...
hc_kernel_end_directive:
    ...
    {
        end_hc_kernel_scope();
        $$ = build_hicuda_stmt(HICUDA_KERNEL_DIR_END, NULL);
    }
    ;

hc_kernel_part_directive:
    PRAGMA_HICUDA save_filename save_lineno hc_kernel_part_dir_body
    {
        tree t = build_hicuda_stmt(HICUDA_KERNEL_PART_DIR_BEGIN, $4);
        finish_hicuda_stmt(t, $3, $2);
        add_stmt(t);
...
hc_kernel_part_dir_body:
    HC_LOOP_PART HC_OVER_TBLOCK hc_distr_type '\n'
    {
        $$ = build_hicuda_kernel_part($3, HC_KERNEL_PART_NONE);
    }
    |
    HC_LOOP_PART HC_OVER_THREAD '\n'
    {
        $$ = build_hicuda_kernel_part(HC_KERNEL_PART_NONE,
                HC_KERNEL_PART_CYCLIC);
    }
    |
    HC_LOOP_PART HC_OVER_TBLOCK hc_distr_type HC_OVER_THREAD '\n'
    {
        $$ = build_hicuda_kernel_part($3, HC_KERNEL_PART_CYCLIC);
    }
    ;
...
hc_distr_type:
    ...
    {
        // THIS IS A HACK!
        hc_inside_parens = FALSE;
    }
    hc_explicit_distr_type ')'
    {
        $$ =  $3;
    }
    ;
...
hc_explicit_distr_type:
    ...
    ;
/*
hc_loopblock_directive:
    PRAGMA_HICUDA HC_LOOPBLOCK expr hc_loopblock_copyin_clause '\n'
    {
        $$ = build_hicuda_loopblock($3, $4);
    }
    ;
*/
/* an optional list of COPYIN array sections */
/*
hc_loopblock_copyin_clause:
    // empty
    {
        $$ = NULL;
    }
...
hc_kernel_part_construct:
    hc_kernel_part_directive iteration_statement
    {
        add_stmt(build_hicuda_stmt(HICUDA_KERNEL_PART_DIR_END, NULL));
        // This sets the semantic value for the entire rule.
        $$ = NULL;
    }
    ;
/*
hc_loopblock_construct:
    hc_loopblock_directive
    {
        add_stmt(build_hicuda_stmt(HICUDA_LOOPBLOCK_DIR_BEGIN, $1));
        // The use of $$ refers to this component's semantic value.
    }
    iteration_statement
    {
        add_stmt(build_hicuda_stmt(HICUDA_LOOPBLOCK_DIR_END, NULL));
        // This sets the semantic value for the entire rule.
        $$ = NULL;
    }
    ;
...
for_construct:
    ...
    iteration_statement
    {
        add_stmt(build_omp_stmt(for_dir_e, NULL));
        $$ = NULL;
    }
    ;
...
iteration_statement:
    FOR
    {
        $<ttype>$ = build_stmt(FOR_STMT,
            NULL_TREE, NULL_TREE, NULL_TREE, NULL_TREE);
        // Generate a DO_LOOP WHIRL node as opposed to WHILE_DO.
		TREE_ADDRESSABLE($<ttype>$) = 1;
		add_stmt($<ttype>$); 
    }
	'(' for_init_stmt
	{
        stmt_count++;
        // We need this because a stmt is already added in parsing
        // for_init_stmt.
        RECHAIN_STMTS($<ttype>2, FOR_INIT_STMT($<ttype>2));
    }
	xexpr ';'
    {
        if ($6) FOR_COND($<ttype>2) = c_common_truthvalue_conversion($6);
    }
	xexpr ')'
	{
        FOR_EXPR($<ttype>2) = $9;
...
sections_construct:
    ...
    section_scope
    {
        add_stmt(build_omp_stmt(sections_cons_e, NULL));
        $$ = NULL;
    }
    ;
...
single_construct:
    ...
    structured_block
    {
        add_stmt(build_omp_stmt(single_cons_e, NULL));
        $$ = NULL;
    }
    ;
...
master_construct:
    master_directive
    {
        add_stmt(build_omp_stmt(master_cons_b, NULL));
        // $$ = NULL;
    }
    structured_block
    {
        add_stmt(build_omp_stmt(master_cons_e, NULL));
        $$ = NULL;
    }
    ;
...
critical_construct:
    critical_directive
    {
        add_stmt(build_omp_stmt(critical_cons_b, $1));
        // $$ = NULL;
    }
    structured_block
    {
        add_stmt(build_omp_stmt(critical_cons_e, NULL));
        $$ = NULL;
    }
    ;
...
ordered_construct:
    ordered_directive
    {
        add_stmt(build_omp_stmt(ordered_cons_b, NULL));
        // $$ = NULL;
    }
    structured_block
    {
        add_stmt(build_omp_stmt(ordered_cons_e, NULL));
        $$ = NULL;
    }
    ;
...
static int check_hicuda_string(char *s, bool *status)
{
    // Indicate if 's' should be handled by this function.
    *status = true;

    if (strcmp(s, "\n") == 0)
    {
        // See the end of the hiCUDA pragma.
        in_hicuda_pragma = FALSE;
        // Reset flags.
        hc_no_keyword_check = hc_inside_parens = FALSE;
        return '\n';
    }

    // Check for HiCUDA keywords when outside [] or ().
    if (hc_inside_parens || hc_no_keyword_check)
    {
        *status = FALSE;
        return 0;
    }
    ...
    if (strcmp(s, "tblock") == 0) return HC_BLOCK;
    if (strcmp(s, "thread") == 0) return HC_THREAD;
    if (strcmp(s, "loop_partition") == 0) return HC_LOOP_PART;
    if (strcmp(s, "over_tblock") == 0) return HC_OVER_TBLOCK;
    if (strcmp(s, "over_thread") == 0) return HC_OVER_THREAD;
    if (strcmp(s, "BLOCK") == 0) return HC_DISTR_BLOCK;
    if (strcmp(s, "CYCLIC") == 0) return HC_DISTR_CYCLIC;

    if (strcmp(s, "loopblock") == 0) return HC_LOOPBLOCK;

    if (strcmp(s, "global") == 0) return HC_GLOBAL;
    if (strcmp(s, "alloc") == 0) return HC_DECL;
    if (strcmp(s, "copyin") == 0) return HC_COPYIN;
    if (strcmp(s, "clear") == 0) return HC_CLEAR;
    if (strcmp(s, "copyout") == 0) return HC_COPYOUT;

    if (strcmp(s, "shared") == 0) return HC_SHARED;
    if (strcmp(s, "nobndcheck") == 0) return HC_NOBNDCHECK;
    if (strcmp(s, "constant") == 0) return HC_CONST;
    if (strcmp(s, "shape") == 0) return HC_SHAPE;

    if (strcmp(s, "free") == 0) return HC_FREE;
    if (strcmp(s, "remove") == 0) return HC_REMOVE;

    if (strcmp(s, "barrier") == 0) return HC_BARRIER;
...
/** DAVID CODE BEGIN **/
    case CPP_NAME:
    {
        ...
        else if (ret == IDENTIFIER && in_hicuda_pragma)
        {
            if (TREE_CODE(yylval.ttype) != IDENTIFIER_NODE) abort();

            char *hc_name = IDENTIFIER_POINTER(yylval.ttype);
            bool valid = false;
            int code = check_hicuda_string(hc_name, &valid);

            // Check if it has a '$' prefix.
            TREE_WITH_DOLLAR_PREFIX(yylval.ttype) = 0;
            if (hc_dollar_sign_prefix)
            {
                if (valid) {
                    // A HiCUDA keyword can never have $ prefix.
                    valid = false;
                } else {
                    // Set the flag to indicate $ prefix.
                    TREE_WITH_DOLLAR_PREFIX(yylval.ttype) = 1;
                    // Need to change the token name.
                    code = HC_IDENTIFIER;
                    valid = true;
                }
                hc_dollar_sign_prefix = FALSE;
            }
