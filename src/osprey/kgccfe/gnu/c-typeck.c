     ...
     int fun;
{
    ...
    if (decl && TREE_DEPRECATED(decl)) warn_deprecated_use(decl);

    if (decl == NULL ||
        decl == error_mark_node || C_DECL_ANTICIPATED(decl)) {
        if (objc_ivar) {
	        ref = objc_ivar;
        } else if (fun) {
	        if (decl == NULL || decl == error_mark_node) {
                ...
                // Implicit declaration of built-in function. Don't change
                // the built-in declaration, but don't let this go by
                // silently, either.
    ...
    if (TREE_TYPE (ref) == error_mark_node) return error_mark_node;

    if (!skip_evaluation) assemble_external(ref);
    TREE_USED(ref) = 1;
