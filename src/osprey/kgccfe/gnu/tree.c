...
/** DAVID CODE BEGIN **/
            if (code == HICUDA_MARKER_STMT) {
                return sizeof(struct tree_hicuda);
            }
/*** DAVID CODE END ***/
#endif // KEY
            // <struct tree_exp> already has one kid (operand), so we need to
            // substract it.
    ...
    int type_quals;
{
     ...
     *
     * We must preserve the TYPE_NAME, since there is code that depends on
     * this. We must also preserve the following flags:
     * - TYPE_IS_LONG_INT, TYPE_IS_CHAR
     */
    for (t = TYPE_MAIN_VARIANT(type); t != NULL_TREE;
            t = TYPE_NEXT_VARIANT(t)) {
        if (TYPE_QUALS(t) == type_quals
                && TYPE_NAME(t) == TYPE_NAME(type)
                && TYPE_CONTEXT(t) == TYPE_CONTEXT(type)
                && TYPE_IS_LONG_INT(t) == TYPE_IS_LONG_INT(type)
                && TYPE_IS_CHAR(t) == TYPE_IS_CHAR(type)) return t;
    }
...
/** DAVID CODE BEGIN **/
    TYPE_IS_CHAR(char_type_node) = TRUE;
/*** DAVID CODE END ***/
