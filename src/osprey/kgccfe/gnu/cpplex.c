...
/** DAVID CODE BEGIN **/
extern bool in_hicuda_pragma;
bool hc_inside_parens = FALSE;          // inside (),[],or {}
bool hc_dollar_sign_prefix = FALSE;     // $<identifier>
/*** DAVID CODE END ***/
#endif
...
static enum pragma_type current_pragma = PT_INVALID;
bool last_token_omp_hash = FALSE;
/** DAVID CODE BEGIN **/
bool last_token_hicuda_hash = FALSE;
/*** DAVID CODE END ***/

/**
 * Look ahead and check if it's a known pragma (OpenMP or hiCUDA).
 * If it is, return the token that represents the keyword 'pragma'.
 */
static cpp_token*
_cpp_pragma_token(cpp_reader *pfile) {
    cpp_buffer *buffer = pfile->buffer;
    ...
    // Check for keyword "pragma".
    int len = strlen("pragma");
    if ((rlimit - c) < len || memcmp(c, "pragma", len)) return NULL;
    c += len;
...
/*** DAVID CODE END ***/
    ...
    // Check for keyword "omp".
    len = strlen("omp");
    if ((rlimit - c) >= len && memcmp(c, "omp", len) == 0) {
        // Found #pragma omp.
        current_pragma = PT_OMP;
        return _cpp_lex_direct(pfile);
    }

    // Check for keyword "options".
    len = strlen("options");
    if ((rlimit - c) >= len && memcmp(c, "options", len) == 0) {
        // Found #pragma options.
        current_pragma = PT_OPTIONS;
        return _cpp_lex_direct(pfile);
    }

    // Check for keyword "mips_frequency_hint" or "frequency_hint".
    bool is_exec_freq = false;
    len = strlen("mips_frequency_hint");
    if ((rlimit - c) >= len && memcmp(c, "mips_frequency_hint", len) == 0) {
        is_exec_freq = true;
    } else {
        len = strlen("frequency_hint");
        if ((rlimit - c) >= len && memcmp(c, "frequency_hint", len) == 0) {
            is_exec_freq = true;
        }
    }
    if (is_exec_freq) {
        // Found #pragma [mips_]frequency_hint.
        current_pragma = PT_EXEC_FREQ;
        return _cpp_lex_direct(pfile);
    }
...
#ifdef KEY
/** DAVID CODE BEGIN **/
            if ((in_omp_pragma || in_hicuda_pragma)
                    && *(pfile->buffer->cur-1) == '\n')
            {
                return result;
            }
...
#ifdef KEY
        // Handle the case of seeing '#' at the beginning of a line.
        if ((result->flags & BOL) && result->type == CPP_HASH
                && pfile->state.parsing_args != 1)
        {
            // Do a lookahead to find if it is a known pragma.
	        cpp_token *pragma_res = _cpp_pragma_token(pfile);
	        if (pragma_res != NULL)
            {
                // This is either an OpenMP or a hiCUDA pragma.
                switch (current_pragma)
                {
/** DAVID CODE BEGIN **/
                    case PT_HICUDA:
                        if (flag_hicuda)
                        {
                            last_token_hicuda_hash = TRUE;

                            // Why can't we update the source line?
                            if (pfile->cb.line_change != NULL
...
/*** DAVID CODE END ***/
                ...
                // Should ignore the pragma.
                skip_to_end_of_line(pfile);
                continue;
            }
            // Unknown pragmas can be handled later.
        }
#endif // KEY
        ...
        if (pfile->state.in_directive) break;
        ...
        if (!pfile->state.skipping || result->type == CPP_EOF) break;
    }
...
#ifdef KEY
    // The newline character may indicate the end of a pragma.
    // We need to deal with this special case here.
    if (c == '\n') {
/** DAVID CODE BEGIN **/
        if (in_omp_pragma || in_hicuda_pragma) {
/*** DAVID CODE END ***/
            handle_newline(pfile);
            // printf("HANDLED PRAGMA NEWLINE: now %d\n", pfile->line);
            buffer->saved_flags = BOL;
...
/** DAVID CODE BEGIN **/
        // We could have done this in yacc specification, but then we write
        // it many more times.
        if (in_hicuda_pragma) hc_inside_parens = TRUE;
/*** DAVID CODE END ***/
#endif // KEY
        break;
...
/** DAVID CODE BEGIN **/
        // The only undesirable effect of putting the code here as opposed to
        // in the yacc specification is that if an unmatching ending bracket is
        // determined, hc_inside_parens will be FALSE, but it does not matter.
        if (in_hicuda_pragma) hc_inside_parens = FALSE;
/*** DAVID CODE END ***/
        break;
...
/** DAVID CODE BEGIN **/
        if (in_hicuda_pragma) {
            hc_dollar_sign_prefix = TRUE;
            // Throw away this char.
            pfile->buffer->cur++;
            // Parse the following chars as an identifer.
            goto start_ident;
        }

        if (CPP_OPTION(pfile, dollars_in_ident)) goto start_ident;
/*** DAVID CODE END ***/
