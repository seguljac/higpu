...
/** DAVID CODE BEGIN **/
    ...
    // Save all STRUCT/UNION types in a global chain.
    if (flag_keep_all_types) tree_type_list = chainon(tree_type_list, tags);
...
void save_global_types_and_symbols()
{
    if (flag_keep_all_types)
    {
        tree_type_list = chainon(tree_type_list, global_binding_level->tags);
        // We also maintain a list of global symbols, including function
        // declarations.
        tree_sym_list = chainon(tree_sym_list, global_binding_level->names);
    }
}
	 ...
	 standard built-in type.
        ...
        if (TREE_CODE(x) == TYPE_DECL) {
            ...
            if (DECL_SOURCE_LINE(x) == 0) {
                // built-in types
                if (TYPE_NAME(tt) == NULL_TREE) TYPE_NAME(tt) = x;
            } else {
                tree orig_tt = DECL_ORIGINAL_TYPE(x);
                if (tt != error_mark_node && orig_tt == NULL_TREE) {
     ...
     int thislevel_only;
{
    ...
    for (level = binding_level; level; level = level->level_chain) {
        ...
        for (tail = level->tags; tail; tail = TREE_CHAIN(tail)) {
            if (TREE_PURPOSE(tail) == name) {
                if (TREE_CODE(TREE_VALUE(tail)) != code) {
                    ...
                    if (thislevel) pending_xref_error();
                }
        ...
        if (! level->tag_transparent) {
            if (thislevel_only) return NULL_TREE;
            thislevel = 0;
        }
    }
...
/** DAVID CODE BEGIN **/
    ...
    // DAVID EDIT:
    //
    // ASM spec is appended to some functions in <stdio.h> (e.g., sscanf and
    // fscanf) in the C99 mode. The code here causes the function name to be
    // replaced with the name in the asm spec (e.g., __iso99_sscanf and
    // __iso99_fscanf), which confuses NVCC.
    //
    // For now, we simply ignore asm spec for function decl.
    //
...
/*** DAVID CODE END ***/
  ...
  if (TREE_CODE (decl) == VAR_DECL)
    ...
    /* If this is a function and an assembler name is specified, reset
     * DECL_RTL so we can give it its new name.  Also, update built_in_decls
     * if it was a normal built-in.  */
...
/** DAVID CODE BEGIN **/
static tree special_long_integer_type_node = NULL_TREE;
static tree special_long_unsigned_type_node = NULL_TREE;
/*** DAVID CODE END ***/
     ...
     int initialized;
{
  ...
  /* Look through the decl specs and record which ones appear.
    ...
    for (spec = declspecs; spec; spec = TREE_CHAIN (spec)) {
        ...
        if (id == ridpointers[(int)RID_INT]) explicit_int = 1;
        if (id == ridpointers[(int)RID_CHAR]) explicit_char = 1;
        ...
        if (TREE_CODE(id) == TYPE_DECL) {
            ...
            if (t == NULL || TREE_CODE(t) != TYPE_DECL) {
...
found:
    ...
    typedef_type = type;
    if (type) size_varies = C_TYPE_VARIABLE_SIZE(type);
...
/** DAVID CODE BEGIN **/
        ...
        if (longlong) {
            ...
            // The front-end may reference 'long_unsigned_type_node' internally
            // for other purposes, so make a copy here.
            if (special_long_unsigned_type_node == NULL_TREE) {
                special_long_unsigned_type_node =
                    build_type_copy(long_unsigned_type_node);
                TYPE_IS_LONG_INT(special_long_unsigned_type_node) = TRUE;
            }
            type = special_long_unsigned_type_node;
            // printf("meeting UNSIGNED LONG: 0x%08X\n", (unsigned)type);
        } else if (specbits & 1 << (int)RID_SHORT) {
            type = short_unsigned_type_node;
        } else if (type == char_type_node) {
            type = unsigned_char_type_node;
        } else if (typedef_decl != NULL) {
        ...
        // The front-end may reference 'long_integer_type_node' internally
        // for other purposes, so make a copy here.
        if (special_long_integer_type_node == NULL_TREE) {
            special_long_integer_type_node =
                build_type_copy(long_integer_type_node);
            TYPE_IS_LONG_INT(special_long_integer_type_node) = TRUE;
        }
        type = special_long_integer_type_node;
        // printf("meeting SIGNED LONG: 0x%08X\n", (unsigned)type);
    } else if (specbits & 1 << (int)RID_SHORT) {
