...
/** DAVID CODE BEGIN **/
extern int flag_keep_all_types;
/*** DAVID CODE END ***/
...
build_omp_stmt(enum omp_tree_type c, void *p) {
    ...
    printf("New OpenMP statement of type %u at 0x%08X\n", 
        t->omp.choice, (unsigned)t);

    return t;
}

// natural scope the current kernel directive belongs to (if any)
static tree hc_kernel_scope = NULL;

void new_hc_kernel_scope()
{
    if (hc_kernel_scope != NULL) error("nested kernel directives");

    hc_kernel_scope = *(current_scope_stmt_stack());
    assert(hc_kernel_scope != NULL);
}

void end_hc_kernel_scope()
{
    if (hc_kernel_scope == NULL
            || hc_kernel_scope != *(current_scope_stmt_stack()))
    {
        error("unmatched kernel_end directive");
    }

    hc_kernel_scope = NULL;
}
...
/**
 * Build a hicuda-marker statement node and return it.
 */
tree build_hicuda_stmt(enum hicuda_tree_type c, void *p)
{
    tree t = build_stmt(HICUDA_MARKER_STMT);
    t->hicuda.choice = c;
    t->hicuda.hicuda_dir = p;

    return t;
}

void finish_hicuda_stmt(tree t, int lineno, const char *filename)
{
    t->hicuda.lineno = lineno;
    t->hicuda.filename = filename;

#if 0
    printf("New hiCUDA directive (%p) of type %u at line %d of <%s>\n", 
            t, t->hicuda.choice, t->hicuda.lineno, t->hicuda.filename);
#endif
}
...
tree tree_type_list = NULL;
tree tree_sym_list = NULL;

void decl_all_types_and_symbols()
{
    if (flag_keep_all_types)
    {
#if 0
        printf("Adding all types (0x%08X) and symbols (0x%08X)\n",
                (unsigned int)tree_type_list, (unsigned int)tree_sym_list);
#endif
        WFE_decl_all_types_and_symbols(tree_type_list, tree_sym_list);
    }
}
...
process_omp_stmt(tree t) {
    printf("Processing OpenMP statement at 0x%08X (type %u)\n",
        (unsigned)t, t->omp.choice);

    switch (t->omp.choice) {
        case parallel_dir_b:
            expand_start_parallel(
                (struct parallel_clause_list*)t->omp.omp_clause_list);
            break;
        case parallel_dir_e:
            expand_end_parallel();
            break;
        case for_dir_b:
            expand_start_for(
                (struct for_clause_list*)t->omp.omp_clause_list);
            break;
        case for_dir_e:
            expand_end_for();
            break;
        case sections_cons_b:
            expand_start_sections(
                (struct sections_clause_list*)t->omp.omp_clause_list);
            break;
        ...
        case single_cons_b:
            expand_start_single(
                (struct single_clause_list*)t->omp.omp_clause_list);
            break;
        case single_cons_e:
          expand_end_single ();
          break;
...
extern void process_hicuda_stmt(tree t)
{
    // For hiCUDA directives, the internal line # is incorrect. Let's
    // overwrite it here.
    lineno = t->hicuda.lineno;

#if 0
    printf("Processing hiCUDA directive (%p) of type %u "
            "at line %d of <%s>\n",
            t, t->hicuda.choice, lineno, t->hicuda.filename);
#endif

    switch (t->hicuda.choice)
    {
        /* kernel directive */
        case HICUDA_KERNEL_DIR_BEGIN:
            expand_hc_kernel_begin(
                    (struct hicuda_kernel*)t->hicuda.hicuda_dir);
            break;
        case HICUDA_KERNEL_DIR_END:
            expand_hc_kernel_end();
            break;

        /* kernel partition directive */
        case HICUDA_KERNEL_PART_DIR_BEGIN:
            expand_hc_kernel_part_begin(
                    (struct hicuda_kernel_part*)t->hicuda.hicuda_dir);
            break;
        case HICUDA_KERNEL_PART_DIR_END:
            expand_hc_kernel_part_end();
            break;

        /* loopblock directive */
        case HICUDA_LOOPBLOCK_DIR_BEGIN:
            expand_hc_loopblock_begin(
                    (struct hicuda_loopblock*)t->hicuda.hicuda_dir);
            break;
        case HICUDA_LOOPBLOCK_DIR_END:
            expand_hc_loopblock_end();
            break;

        /* global directive */
        case HICUDA_GLOBAL_COPYIN_DIR:
        case HICUDA_GLOBAL_COPYOUT_DIR:
            expand_hc_global(
                    (struct hicuda_global_shared*)t->hicuda.hicuda_dir);
            break;
        case HICUDA_GLOBAL_FREE_DIR:
            expand_hc_global_free(
                    (struct free_data_list*)(t->hicuda.hicuda_dir));
            break;

        /* shared directive */
        case HICUDA_SHARED_COPYIN_DIR:
        case HICUDA_SHARED_COPYOUT_DIR:
            expand_hc_shared(
                    (struct hicuda_global_shared*)t->hicuda.hicuda_dir);
            break;
        case HICUDA_SHARED_REMOVE_DIR:
            expand_hc_shared_remove(
                    (struct free_data_list*)(t->hicuda.hicuda_dir));
            break;

        /* const directive */
        case HICUDA_CONST_COPYIN_DIR:
            expand_hc_const(
                    (struct hicuda_const*)(t->hicuda.hicuda_dir));
            break;
        case HICUDA_CONST_REMOVE_DIR:
            expand_hc_const_remove(
                    (struct free_data_list*)(t->hicuda.hicuda_dir));
            break;

        /* shape directive */
        case HICUDA_SHAPE_DIR:
            expand_hc_shape(
                    (struct hicuda_shape*)(t->hicuda.hicuda_dir));
            break;

        /* barrier directive */
        case HICUDA_BARRIER_DIR:
            expand_hc_barrier();
            break;
...
/** DAVID CODE BEGIN **/
    case HICUDA_MARKER_STMT:
        process_hicuda_stmt(t);
        break;
/*** DAVID CODE END ***/
