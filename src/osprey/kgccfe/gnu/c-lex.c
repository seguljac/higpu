...
// to verify that the succeeding tokens are 'pragma omp|option|...'.
extern bool last_token_omp_hash;
/** DAVID CODE BEGIN **/
bool in_hicuda_pragma = FALSE;
// TRUE implies we have seen a CPP_HASH, and we have done lookahead
// to verify that the succeeding tokens are 'pragma hicuda'.
extern bool last_token_hicuda_hash;
/*** DAVID CODE END ***/
#endif
...
retry:
    ...
    if (last_token_omp_hash && tok->type == CPP_NAME) {
        ...
        if (TREE_CODE(name) == IDENTIFIER_NODE
                && strcmp(IDENTIFIER_POINTER(name), "pragma") == 0) {
            tok = cpp_get_token(parse_in);
            in_omp_pragma = TRUE;
        }
    }
...
/** DAVID CODE BEGIN **/
    if (last_token_hicuda_hash && tok->type == CPP_NAME)
    {
        last_token_hicuda_hash = FALSE;
        tree name = HT_IDENT_TO_GCC_IDENT(HT_NODE(tok->val.node));
        // Is this check necessary?
        if (TREE_CODE(name) == IDENTIFIER_NODE
                && strcmp(IDENTIFIER_POINTER(name), "pragma") == 0) {
            tok = cpp_get_token(parse_in);
            in_hicuda_pragma = TRUE;
        }
    }
/*** DAVID CODE END ***/
    ...
    // The C++ front end does horrible things with the current line number.
    // To ensure an accurate line number, we must reset it every time we
    // return a token.
    lineno = src_lineno;
