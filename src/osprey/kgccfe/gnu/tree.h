...
/** DAVID CODE BEGIN **/
    // must add it after 'id' because of HT_IDENT_TO_GCC_IDENT
    unsigned with_dollar_prefix_flag : 1;
    unsigned unused_0 : 7;
/*** DAVID CODE END ***/
};

/** DAVID CODE BEGIN **/
#define TREE_WITH_DOLLAR_PREFIX(NODE) \
    (IDENTIFIER_NODE_CHECK(NODE)->identifier.with_dollar_prefix_flag)
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
#define TYPE_IS_LONG_INT(TYPE) (TYPE_CHECK(TYPE)->type.is_long_int)
#define TYPE_IS_CHAR(TYPE) (TYPE_CHECK(TYPE)->type.is_char)
/*** DAVID CODE END ***/
...
#endif
/** DAVID CODE BEGIN **/
  unsigned is_long_int : 1; /* true if the type is unsigned long, or long */
  unsigned is_char : 1;     /* true if the type is char with no sign spec */
/*** DAVID CODE END ***/
...
struct tree_hicuda GTY(())
{
    struct tree_common common;
    enum hicuda_tree_type choice;
    void *hicuda_dir;
    int lineno;
    const char *filename;
};
...
/** DAVID CODE BEGIN **/
    struct tree_hicuda GTY ((tag ("TS_HICUDA"))) hicuda;
/*** DAVID CODE END ***/
#endif
 };

