...
void WFE_decl_all_types_and_symbols(tree ty_list, tree st_list)
{
    for ( ; ty_list != NULL; ty_list = TREE_CHAIN(ty_list))
    {
        Get_TY(TREE_VALUE(ty_list));
    }
    for ( ; st_list != NULL; st_list = TREE_CHAIN(st_list))
    {
        // For now, we only care about function declarations.
        if (TREE_CODE(st_list) == FUNCTION_DECL) Get_ST(st_list);
    }
}
...
// The static init code generated cannot be compiled by GCC, so let's
// always generate element-wise assignments.
#if 0
