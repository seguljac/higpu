...
void INTSYMB_LIST::Init(INTSYMB_LIST *il, MEM_POOL *mem_pool)
{
    ...
    for (INTSYMB_NODE *in = iter.First(); !iter.Is_Empty(); in=iter.Next()) {
        Append(CXX_NEW(INTSYMB_NODE(in), mem_pool));
    }
}
...
/** DAVID CODE BEGIN **/
    ...
    {
        fprintf(TFile, "Inside Set_Array: Du_Mgr = %p, &Du_Mgr = %p\n",
                Du_Mgr, &Du_Mgr);
    }
/*** DAVID CODE END ***/
    ...
    Is_True(_num_vec == WN_num_dim(wn),
            ("ACCESS_ARRAY::Set_Array called with"
...
INT ACCESS_ARRAY::Delinearize(DOLOOP_STACK *stack, INT dim, WN *wn)
{
    ...
    SUMPROD_LIST *nl_symb = av->Non_Lin_Symb;
     ...
     */
    SUMPROD_CONST_ITER tmp_iter(nl_symb);
            ...
            lin_node = lin_iter.Next())
    {
        if (!(lin_node->Symbol == *delin_symbol)) {
            tmp->Lin_Symb->Append(CXX_NEW(
                        INTSYMB_NODE(lin_node->Symbol,lin_node->Coeff),
                        &LNO_local_pool));
        }
    }
...
void ACCESS_VECTOR::Update_Non_Const_Loops_Nonlinear(DOLOOP_STACK *stack)
{
    ...
    for (const SUMPROD_NODE *sp_node = sp_iter.First();
            !sp_iter.Is_Empty(); sp_node = sp_iter.Next())
    {
                ...
                node=iter.Next())
        {
            ...
            INT i = 0;
            while (! (SYMBOL(WN_index(stack->Bottom_nth(i))) == symbol)) i++;
        ...
        const SYMBOL *delin_symbol)
{
    ...
    /* Create a new array of ACCESS_VECTORS, unfortunately, because it's an
    ...
    for (lin_node=lin_iter.First(); !lin_iter.Is_Empty();
        ...
        if (!(lin_node->Symbol == *delin_symbol)) {
            new_dim[dim+1].Lin_Symb->Append(
                    CXX_NEW(INTSYMB_NODE(lin_node->Symbol,lin_node->Coeff),
                        _mem_pool));
        }
    }
...
void ACCESS_VECTOR::Set(WN *wn, DOLOOP_STACK *stack,
    ...
    switch (WN_operator(wn))
    {
        ...
        case OPR_MPY:
                    ...
                    && (coeff < INT32_MAX-1) && (coeff > INT32_MIN+1))
            {
                 ...
                 * list if exist.
                 */
                SUMPROD_LIST *list = CXX_NEW(SUMPROD_LIST,
                        (Non_Lin_Symb != NULL) ? &LNO_local_pool : _mem_pool);
                SYMBOL_LIST *sl = CXX_NEW(SYMBOL_LIST, _mem_pool);
                list->Append(CXX_NEW(SUMPROD_NODE(sl, coeff), _mem_pool));
                ...
                SUMPROD_NODE *sp_prev = NULL, *sp_next = NULL;
                for (SUMPROD_NODE *sp_node = sp_iter.First();
        ...
        DOLOOP_STACK *stack, WN *wn)
{
    ...
    for (i = 0; i < stack->Elements() && !is_iv; i++) {
        SYMBOL doloop(WN_index(stack->Bottom_nth(i)));
...
SUMPROD_LIST *ACCESS_VECTOR::Add_Nonlin(WN *wn, SUMPROD_LIST *list,
        DOLOOP_STACK *stack)
{
    Is_True(list != NULL , ("Null input list in ACCESS_VECTOR::Add_Nonlin"));
        ...
        case OPR_LDID:
        {
            ...
            for (SUMPROD_NODE *node = iter.First(); !iter.Is_Empty();
                ...
                node->Prod_List->Append(
                        CXX_NEW(SYMBOL_NODE(symbol, is_iv), _mem_pool));
            }
...
ACCESS_VECTOR* ACCESS_VECTOR::divide_by_const(UINT divisor, WN *wn)
{
    if (Too_Messy) return NULL;

    // quotient
    ACCESS_VECTOR *q = CXX_NEW(
            ACCESS_VECTOR(_nest_depth, _mem_pool), _mem_pool);
    // remainder (a copy of this vector)
    ACCESS_VECTOR *r = CXX_NEW(
            ACCESS_VECTOR(_nest_depth, _mem_pool), _mem_pool);

    /* Deal with the constant term first. */
    q->Const_Offset = Const_Offset / divisor;
    r->Const_Offset = Const_Offset - q->Const_Offset * divisor;

    /* Go through the coefficients of loop index variables (IV).
     * (coeff * iv) / divisor =
     *     (coeff / divisor) * iv + (coeff % divisor)*iv
     *
    ...
    if (Lin_Symb != NULL)
    {
        ...
        for (INTSYMB_NODE* in = ii.First(); !ii.Is_Empty(); in = ii.Next()) 
        {
            ...
            if (q_coeff != 0) {
                if (q->Lin_Symb == NULL) {
                    q->Lin_Symb = CXX_NEW(INTSYMB_LIST,_mem_pool);
                }
                q->Lin_Symb->Append(CXX_NEW(
                            INTSYMB_NODE(in->Symbol, q_coeff), _mem_pool));
            }
            if (r_coeff != 0) {
                if (r->Lin_Symb == NULL) {
                    r->Lin_Symb = CXX_NEW(INTSYMB_LIST,_mem_pool);
                }
                r->Lin_Symb->Append(CXX_NEW(
                            INTSYMB_NODE(in->Symbol, r_coeff), _mem_pool));
            }
        }
    }
    ...
    if (Non_Lin_Symb != NULL)
    {
        SUMPROD_ITER sli(Non_Lin_Symb);
        for (SUMPROD_NODE *sln = sli.First(); !sli.Is_Empty();
                sln = sli.Next())
        {
            ...
            if (q_coeff != 0) {
                if (q->Non_Lin_Symb == NULL) {
                    q->Non_Lin_Symb = CXX_NEW(SUMPROD_LIST,_mem_pool);
                }
                SUMPROD_NODE *q_sn = CXX_NEW(SUMPROD_NODE(
                            CXX_NEW(SYMBOL_LIST(sln->Prod_List, _mem_pool),
                                _mem_pool), q_coeff), _mem_pool);
                q->Non_Lin_Symb->Append(q_sn);
            }
            if (r_coeff != 0) {
                if (r->Non_Lin_Symb == NULL) {
                    r->Non_Lin_Symb = CXX_NEW(SUMPROD_LIST,_mem_pool);
                }
                SUMPROD_NODE *r_sn = CXX_NEW(SUMPROD_NODE(
                            CXX_NEW(SYMBOL_LIST(sln->Prod_List, _mem_pool),
...
// TODO: Is_Loop_Var in SYMBOL_NODE
ACCESS_VECTOR* ACCESS_VECTOR::divide_by_iv(UINT iv_idx, WN *wn)
{
    if (Too_Messy) return NULL;

    Is_True(iv_idx < _nest_depth, (""));
    ...
    // The constant term goes to the remainder.
    r->Const_Offset = Const_Offset;

    // All loop IV terms go to the remainder except the given one.
    if (_lcoeff != NULL)
    {
        q->Const_Offset = _lcoeff[iv_idx];
        r->_lcoeff = CXX_NEW_ARRAY(mINT32, _nest_depth, _mem_pool);
        for (UINT i = 0; i < _nest_depth; ++i) {
            r->_lcoeff[i] = (i == iv_idx) ? 0 : _lcoeff[i];
        }
    }

    // All linear terms go to the remainder.
    if (Lin_Symb != NULL)
    {
        INTSYMB_ITER ii(Lin_Symb);
        for (INTSYMB_NODE* in = ii.First(); !ii.Is_Empty(); in = ii.Next()) 
        {
            INT32 r_coeff = in->Coeff;
            if (r_coeff == 0) continue;

            if (r->Lin_Symb == NULL) {
                r->Lin_Symb = CXX_NEW(INTSYMB_LIST,_mem_pool);
            }
            r->Lin_Symb->Append(
                    CXX_NEW(INTSYMB_NODE(in->Symbol, r_coeff), _mem_pool));
        }
    }

    // All non-linear terms go to the remainder.
    if (Non_Lin_Symb != NULL)
    {
        SUMPROD_ITER sli(Non_Lin_Symb);
        for (SUMPROD_NODE *sln = sli.First(); !sli.Is_Empty();
    ...
    /* Prove that the remainder is in the range of 0 .. (IV-1). */

    // Is it possible that r < 0 (r + 1 <= 0)?
    r->Const_Offset = -(r->Const_Offset+1);
    if (!Is_Consistent_Condition(r, wn))
    {
        // Is it possible that r >= IV (IV - r <= 0)?
        r->Const_Offset++;              // r <= 0
        r->_lcoeff[iv_idx] -= 1;        // r - IV <= 0
        r->Negate_Me();                 // IV - r <= 0
        if (!Is_Consistent_Condition(r, wn))
        {
            // Recover the original remainder.
...
ACCESS_VECTOR* ACCESS_VECTOR::divide_by_sym(SYMBOL& sym, WN *wn)
{
    if (Too_Messy) return NULL;
    ...
    // All loop IV terms go to the remainder.
    if (_lcoeff != NULL)
    {
        r->_lcoeff = CXX_NEW_ARRAY(mINT32, _nest_depth, _mem_pool);
        for (UINT i = 0; i < _nest_depth; ++i) {
            r->_lcoeff[i] = _lcoeff[i];
        }
    }

    // All linear terms go to the remainder except the ones that contain the
    // given symbol.
    if (Lin_Symb != NULL)
    {
        INTSYMB_ITER ii(Lin_Symb);
        for (INTSYMB_NODE* in = ii.First(); !ii.Is_Empty(); in = ii.Next()) 
        {
            if (in->Coeff == 0) continue;

            if (in->Symbol == sym) {
                q->Const_Offset += in->Coeff;
            } else {
                if (r->Lin_Symb == NULL) {
                    r->Lin_Symb = CXX_NEW(INTSYMB_LIST,_mem_pool);
                }
                r->Lin_Symb->Append(CXX_NEW(
                            INTSYMB_NODE(in->Symbol, in->Coeff), _mem_pool));
            }
        }
    }

    // All non-linear terms go to the remainder except the ones that contain
    // the given symbol.
    if (Non_Lin_Symb != NULL)
    {
        SUMPROD_ITER sli(Non_Lin_Symb);
        for (SUMPROD_NODE *sln = sli.First(); !sli.Is_Empty();
                sln = sli.Next())
        {
            if (sln->Coeff == 0) continue;

            SYMBOL_LIST *sl = sln->Prod_List;
            Is_True(sl != NULL, (""));

            // Divide the symbol list by <sym>.
            SYMBOL_LIST *r_sl = CXX_NEW(SYMBOL_LIST, _mem_pool);
            BOOL found_sym = FALSE;
            SYMBOL_NODE *sn = sl->Head();
            while (sn != NULL)
            {
                if (sn->Symbol == sym) {
                    found_sym = TRUE;
                } else {
                    r_sl->Append(CXX_NEW(SYMBOL_NODE(sn), _mem_pool));
                }
                sn = sn->Next();
            }
            ...
            if (found_sym)
            {
                // This new symbol list is part of the quotient.
                sn = r_sl->Head();
                Is_True(sn != NULL, (""));
                if (sn->Next() == NULL) {
                    // Promote it to a linear term.
                    if (q->Lin_Symb == NULL) {
                        q->Lin_Symb = CXX_NEW(INTSYMB_LIST,_mem_pool);
                    }
                    q->Lin_Symb->Append(
                            CXX_NEW(INTSYMB_NODE(sn->Symbol, sln->Coeff),
                                _mem_pool));
                } else {
                    if (q->Non_Lin_Symb == NULL) {
                        q->Non_Lin_Symb = CXX_NEW(SUMPROD_LIST,_mem_pool);
                    }
                    q->Non_Lin_Symb->Append(
                            CXX_NEW(SUMPROD_NODE(r_sl, sln->Coeff),
                                _mem_pool));
                }
            }
            else
            {
                // This new symbol list (identical to the original one) goes
                // to the remainder.
                if (r->Non_Lin_Symb == NULL) {
                    r->Non_Lin_Symb = CXX_NEW(SUMPROD_LIST,_mem_pool);
                }
                r->Non_Lin_Symb->Append(
                        CXX_NEW(SUMPROD_NODE(r_sl, sln->Coeff), _mem_pool));
            }
        }
    }
    ...
    /* Prove that the remainder is in the range of 0 .. (SYM-1). */

    // Is it possible that r < 0 (r + 1 <= 0)?
    r->Const_Offset = -(r->Const_Offset+1);
    if (!Is_Consistent_Condition(r, wn))
    {
        // Is it possible that r >= SYM (SYM - r <= 0)?
        r->Const_Offset++;              // r <= 0
        ...
        r->Lin_Symb->Prepend(CXX_NEW(INTSYMB_NODE(sym, -1), _mem_pool));
...
BOOL ACCESS_VECTOR::perfect_divide_by_const(UINT divisor)
{
...
WN* SYMBOL::to_wn() const
{
    Is_True(!_is_formal, ("SYMBOL::to_wn: meet a formal!\n"));

    ST *st = St();

    // Determine the type of this WN.
    // For a struct variable, we must determine the field type.
    TY_IDX ty_idx = ST_type(st);
    if (TY_kind(ty_idx) == KIND_STRUCT)
    {
        FLD_ITER fld_iter = Make_fld_iter(TY_fld(ty_idx));
        INT ofst_left = WN_Offset();
        while (ofst_left > 0)
        {
            Is_True(!FLD_last_field(fld_iter), (""));

            // Subtract the current field's size from the offset.
            FLD_HANDLE fld(fld_iter);
            TY_IDX fld_ty_idx = FLD_type(fld);
            ofst_left -= TY_size(fld_ty_idx);
...
WN* INTSYMB_NODE::to_wn() const
{
    TYPE_ID rtype = Symbol.Type;
    return WN_Mpy(rtype, WN_Intconst(rtype, Coeff), Symbol.to_wn());
}

WN* SUMPROD_NODE::to_wn() const
{
    WN *wn = WN_Intconst(Integer_type, Coeff);

    SYMBOL_ITER si(Prod_List);
    for (SYMBOL_NODE *sn = si.First(); !si.Is_Empty(); sn = si.Next()) {
...
WN* ACCESS_VECTOR::to_wn(DOLOOP_STACK *stack) const
{
    ...
    if (_lcoeff != NULL) {
        for (INT i = 0; i < _nest_depth; ++i) {
            SYMBOL dl_sym(WN_index(stack->Bottom_nth(i)));
