...
void LNO_Build_Access_Array(WN *wn, DOLOOP_STACK *stack,
        MEM_POOL *pool, INDX_RANGE_STACK *irs)
{
    ACCESS_ARRAY *array = CXX_NEW(
            ACCESS_ARRAY(WN_num_dim(wn), stack->Elements(), pool), pool);
/** DAVID CODE BEGIN **/
    if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
    {
        fprintf(TFile,
                "In LNO: Before Set_Array: Du_Mgr = %p, &Du_Mgr = %p\n",
                Du_Mgr, &Du_Mgr);
    }
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    ...
    {
        fprintf(TFile,
                "In LNO: After Set_Array: Du_Mgr = %p, &Du_Mgr = %p\n",
                Du_Mgr, &Du_Mgr);
    }
/*** DAVID CODE END ***/
    WN_MAP_Set(LNO_Info_Map, wn, (void*)array);

    if (irs != NULL) LNO_Update_Indx_Range(irs, array, wn);
}


// Build the access arrays for the bounds of the do loop
extern void LNO_Build_Do_Access(WN *wn,
        DOLOOP_STACK *stack, BOOL Hoist_Bounds)
{
    DO_LOOP_INFO *dli = (DO_LOOP_INFO*)WN_MAP_Get(LNO_Info_Map, wn);
    FmtAssert(dli != NULL, ("Unmapped DO loop in LNO_Build_Do_Access"));

    MEM_POOL *pool = dli->Pool();
    {
        // first the step
        ACCESS_VECTOR *step = CXX_NEW(
                ACCESS_VECTOR(stack->Elements(), pool), pool);
