...
static void Build_Doloop_Stack_Rec(WN *wn, WN *parent, DOLOOP_STACK *stack)
{
    ...
    if (WN_opcode(parent) == OPC_DO_LOOP && WN_do_body(parent) == wn) {
        stack->Push(parent);
    }
}
...
void Build_Doloop_Stack(WN* wn, DOLOOP_STACK* stack)
{
    ...
    if (WN_opcode(wn) == OPC_DO_LOOP) stack->Push(wn);
}
...
/** DAVID CODE BEGIN **/
  copy = Pre_Optimizer(PREOPT_DUONLY_PHASE, copy, Du_Mgr, Alias_Mgr, NULL);
/*** DAVID CODE END ***/
