...
// Description:
//
//     Loop level canonicalization. Mark loops with all sorts of information.
//
//
...
//
//      Mark all the DO_LOOPs and IFs in the function using the DO_LOOP_INFO
//      data structure and the LNO_Info_Map map. Use LNO_default_pool to store
//      the annotations.
//
//		Get rid of all the labels inside "Good" do loops.  These have to be
//		dead.
//
//		For all do loops with indirect bounds, copy the bounds into pregs.
//
...
static void Mark_Code(WN *, WN *,
        DOLOOP_STACK *,DLI_STACK *,
        II_STACK *, STACK_OF_WN *,
        HASH_TABLE<INT, WN*> *, mUINT8 depth, INT *inner_depth,
        BOOL promote_pointers, BOOL inside_bound, BOOL strict_limit=TRUE);
...
extern BOOL Mark_Code(WN *func_nd, BOOL promote_pointers, BOOL strict_limit)
{
    ...
    II_STACK *if_stack =
        CXX_NEW(II_STACK(&LNO_local_pool),&LNO_local_pool); 
...
static void Patch_Loop_Statement_Expression(WN* wn_exp, WN* wn_loop)
{
...
static void Mark_Code(WN *wn, WN *func_nd,
        DOLOOP_STACK *stack, DLI_STACK *dlistack,
        II_STACK *if_stack, STACK_OF_WN *label_stack, 
        HASH_TABLE<INT32,WN*> *label_hash, mUINT8 depth, INT *inner_depth,
        BOOL promote_pointers, BOOL inside_bound, BOOL strict_limit)
{
    WN *kid;
    DO_LOOP_INFO *dli;

    Is_True(wn != NULL, ("Null wn in Mark_Code"));
...
{
    OPCODE opcode = WN_opcode(wn);
    BOOL is_ld_st = OPCODE_is_load(opcode) || OPCODE_is_store(opcode);

    INT addr_num = kid_num;
    WN *addr = WN_kid(wn,addr_num);
    ...
    /* Figure out the load size if not provided. */
    if (load_size == 0)
    {
        if (!is_ld_st) return;
        ...
        {
            case MTYPE_I1:
            case MTYPE_U1: load_size = 1; break;

            case MTYPE_I2:
            case MTYPE_U2: load_size = 2; break;

            case MTYPE_I4:
            case MTYPE_U4:
            case MTYPE_F4: load_size = 4; break;

            case MTYPE_I8:
            case MTYPE_U8:
            case MTYPE_F8:
            case MTYPE_C4: load_size = 8; break;
#ifdef PATHSCALE_MERGE
#ifdef TARG_IA64
...
#endif
            case MTYPE_C8:
            case MTYPE_FQ: load_size = 16; break;
#ifdef PATHSCALE_MERGE
            case MTYPE_C10:
#endif
            case MTYPE_CQ: load_size = 32; break;
    ...
    if (is_ld_st && (addr_oper == OPR_LDID || addr_oper == OPR_LDA))
    {
        // Create a 1D ARRAY node.
        WN *array = WN_Create(
                OPCODE_make_op(OPR_ARRAY, Pointer_type, MTYPE_V), 3);
    ...
    if (is_ld_st && (addr_oper == OPR_ADD || addr_oper == OPR_SUB))
    {
        if (WN_operator(WN_kid0(addr)) == OPR_INTCONST) {
            ...
            Fold_Intconst(wn,WN_kid1(addr),
                    (addr_oper == OPR_ADD ? FALSE : TRUE));
            addr = WN_kid(wn,addr_num);
            addr_oper = WN_operator(addr);
        }
    }
...
/*****************************************************************************
 *
 * Fold the offset of "wn" into the given ARRAY node "array".
 *
 ****************************************************************************/
...
static void Fold_Offset(WN *wn, WN *array) 
{
    ...
    WN_ESIZE elem_sz = WN_element_size(array);

#ifdef KEY
    if (elem_sz == 0) return;
#endif
    // The offset must be a multiple of the array element size.
    if (offset == 0 || (abs(offset) % elem_sz) == 0) return;
    ...
    if ((Pointer_type == MTYPE_A4 || Pointer_type == MTYPE_U4) &&
            // << WHIRL 0.30: Added MTYPE_A4
            (offset <= INT32_MAX) && (offset >= INT32_MIN)) {
        rtype = MTYPE_I4;
    } else {
        rtype = MTYPE_I8;
    }
    ...
    WN_array_index(array,0) = LWN_CreateExp2(add_op,
            WN_array_index(array,0),
            LWN_Make_Icon(rtype, offset/elem_sz));
    LWN_Set_Parent(WN_array_index(array,0), array);
