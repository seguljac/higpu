 ...
 * 
 * Set proved_inconsistent to TRUE if in the process the system is proved
 * inconsistent. Return FALSE on uncorrectable failure (oveflow).
 *
 ****************************************************************************/
...
BOOL SYSTEM_OF_EQUATIONS::Sub_In_Equal(BOOL *proved_inconsistent) 
{
    *proved_inconsistent = FALSE;
    ...
    // Copy to the work space.
    if (!Copy_To_Work_Eq()) return TRUE;
...
/*****************************************************************************
 *
 * Is the first constraint or the second constraint redundant? One of them is
 * considered redundant iff their coefficient vector (except the constant) are
 * equivalent (i.e. multiples of each other). The one that has a "looser"
 * constant bound is redundant.
 *
 * If first is redundant return 1; if second is redundant return 2; otherwise
 * return 0.
 *
 * All cols < min_var assumed to be 0.
 *
...
INT SYSTEM_OF_EQUATIONS::Is_Simple_Redundant(
        const mINT32 *row1, const mINT32 *row2,
        INT64 row1c, INT64 row2c, INT min_var, INT cols)
{
    BOOL has_multiple = FALSE;
    INT32 mult1 = 1, mult2 = 1;     // must be positive
    ...
    for (INT col = min_var; col < cols; col++)
    {
        if (row1[col] == 0) {
            ...
            if ((INT64)mult1*row2[col] != (INT64)mult2*row1[col]) return 0;
        } else {
            has_multiple = TRUE;
            ...
            // Make sure they are of the same sign and become positive.
            if (mult1 < 0) {
                if (mult2 > 0) return 0;
                mult1 = -mult1;
                mult2 = -mult2;
    ...
    /* Find out which constraint is tigher; the other one is redundant. */
    return (mult2*row1c <= mult1*row2c) ? 2 : 1;
}
...
/*****************************************************************************
 *
 * Copy all the 'le' constraints from 'from' to 'to' (inclusive) into the work
 * space. Return FALSE if not enough space.
 *
 ****************************************************************************/
...
BOOL SYSTEM_OF_EQUATIONS::Copy_To_Work(const INT32 from, const INT32 to)
{
    ...
    for (INT32 i = from; i <= to; ++i, ++_work_rows) {
        for (INT32 j = 0; j < _work_cols; ++j) {
...
/*****************************************************************************
 *
 * Copy the inverse of constraint i into the work space. Return FALSE if not
 * enough space.
 *
 * The inverse of A*X <= b is (-A)*X <= -b-1
 *
 ****************************************************************************/
...
BOOL SYSTEM_OF_EQUATIONS::Copy_Inverse_To_Work(const INT32 i)
{
    ...
    if (_work_rows == 0) {
        ...
        FmtAssert(_vars == _work_cols,
                ("Inconsistency in Copy_Inverse_To_Work"));
    }
...
/*****************************************************************************
 *
 * Is the systm of inequalities in the work array consistent? Can always
 * conservatively return TRUE. Use the SVPC test followed by Acyclic test
 * followed by Fourier Motzkin.
 *
 ****************************************************************************/
 ...
 *
 * Simple tests to improve efficiency. We consider the two constraints
 * redundant if one constraint (ignoring the constant term) is a multiple of
 * the other. The redundant constraint is the one that is less tight.
 *
...
INT SYSTEM_OF_EQUATIONS::Mark_Simple_Redundant(BOOL *is_redundant)
{
    ...
    /* last_non_zero holds the column position of the last non-zero element in
     * each row (constraint). This is an efficiency aid, and redundant
     * constraints must have the same value for this.
     */
    static INT16 last_non_zero[SOE_MAX_WORK_ROWS];

    /* Init the is_redundant array and fill the last_non_zero array. */
    for (i = 0; i < _work_rows; i++) {
    ...
    for (i = 0; i < _work_rows; ++i)
    {
        ...
        for (j = i+1; j < _work_rows; ++j)
        {
            if (is_redundant[j]
                    || last_non_zero[i] != last_non_zero[j]) continue;
            
            INT redun = Is_Simple_Redundant(&_work[i][0], &_work[j][0],
                    _work_const[i], _work_const[j], 0, _work_cols);
...
/*****************************************************************************
 *
 * Mark redundant new constraints that have not been set in 'is_redundant'.
 *
 * Copy the non-redundant of constraints[0..i-1,i+1...] and the inverse of
 * constraint[i] into the work array, constraint[i] is redundant if the system
 * is inconsistent.
 *
 * This process may still be conservative.
 *
 ****************************************************************************/
...
INT SYSTEM_OF_EQUATIONS::Mark_New_Redundant(BOOL *is_redundant)
{
    ...
    for (i = 0; i < _eqns_le; ++i)
    {
        ...
        // Copy non-redundant constraints 0..i-1 into the work array.
        for (j = 0; j < i; j++) {
            if (!is_redundant[j]) if (!Copy_To_Work(j,j)) return counter;
        }

        // Copy non-redundant constraints i.._eqns_le-1 into the work array.
        for (j = i+1; j < _eqns_le; j++) {
            if (!is_redundant[j]) if (!Copy_To_Work(j,j)) return counter;
        }
