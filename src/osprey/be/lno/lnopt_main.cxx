...
extern WN* Lnoptimizer(PU_Info* current_pu, WN *func_nd,
        DU_MANAGER *du_mgr, ALIAS_MANAGER *alias_mgr)
{
    ...
    if (!Du_Built(du_mgr))
    {
        ...
        Prompf_Init(); 
        if (Run_prompf) Print_Prompf_Transaction_Log(FALSE);
        Prompf_Dump(current_pu, func_nd); 
        if (LNO_Prompl) Print_Prompl_Msgs(current_pu, func_nd); 
        Prompf_Finish(); 

#ifdef KEY
        // Just cheat the list_option, don't worry about the compilation time
        // if the user want to list options.
        if(List_Enabled){
    ...
    if (!Run_lno || LNO_Opt == 0 || Get_Trace(TP_LNOPT, TT_LNO_SKIP_LNO)) {
        LNO_enabled = FALSE;
    }
    ...
    snl_debug = 0;
    if (Get_Trace(TP_LNOPT, TT_LNO_SNL_DEBUG2)) snl_debug += 2;
    if (Get_Trace(TP_LNOPT, TT_LNO_SNL_DEBUG1)) snl_debug += 1;

    LNO_Allow_Nonlinear = !Get_Trace(TP_LNOPT,TT_LNO_SKIP_NONLIN);
    LNO_Debug_Delinearization = Get_Trace(TP_LNOPT,TT_LNO_DEBUG_DELIN);

    LNO_Allow_Delinearize = TRUE; // this is true for lno but not for IPL
...
/** DAVID CODE BEGIN **/
    if (snl_debug) {
        fprintf(TFile, "BEFORE Mark_Code:\n");
