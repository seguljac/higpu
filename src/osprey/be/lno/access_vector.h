...
//
//  ACCESS_ARRAYs are used for two purposes. For array accesses and loop step
...
//  ACCESS_ARRAY
//
//		ACCESS_ARRAY represents an array of access_vectors. Each access_vector
...
//  ACCESS_VECTOR
//
//	  	One dimension of the ACCESS_ARRAY. This is used for one dimension of
...
//
//		If this access vector was created by delinearization, what SYMBOL was
...
//	    ACCESS_VECTOR() 
//		
//		Create a new ACCESS_VECTOR, initialized to TOO_MESSY.  No memory is
//		allocated.
//
...
// 
//      ACCESS_VECTOR(const SYSTEM_OF_EQUATIONS *soe, const INT i,
...
/*****************************************************************************
 *
 * INDX_RANGE is used to find limits on loop variable ranges based on array
 * accesses.
 * 
...
public:
    ...
    SYMBOL(const SYMBOL& symbol) { Init(&symbol); }
...
public:
    ...
    SYMBOL Symbol;    // NOTE: here it is not a pointer!
    INT32 Coeff;
...
class INTSYMB_CONST_ITER: public SLIST_ITER
{
    DECLARE_SLIST_CONST_ITER_CLASS(
            INTSYMB_CONST_ITER, INTSYMB_NODE ,INTSYMB_LIST);
...
public:
    ...
    SYMBOL Symbol;      // This is not a pointer!
    mBOOL Is_Loop_Var;
...
public:
    ...
    SYMBOL_LIST(SYMBOL_LIST *sl, MEM_POOL *mem_pool) { Init(sl,mem_pool); }
    ~SYMBOL_LIST();
...
{
    DECLARE_SLIST_CONST_ITER_CLASS(
            SYMBOL_CONST_ITER, SYMBOL_NODE, SYMBOL_LIST);
...
{
    DECLARE_SLIST_CONST_ITER_CLASS(
            SUMPROD_CONST_ITER, SUMPROD_NODE, SUMPROD_LIST);
...
public:
    ...
    ACCESS_VECTOR(UINT16 lnest, MEM_POOL *mem_pool = NULL) {
        Init(lnest, mem_pool);
    }
    ACCESS_VECTOR() { 
...
/** DAVID CODE BEGIN **/
     ...
     */
    ACCESS_VECTOR* divide_by_const(UINT divisor, WN *wn);

    /**
     * Divide this access vector by a loop index variable. If successful, this
     * access vector will hold the remainder, and the quotient is returned.
     *
     * <wn> is an address expression whose base address is a pointer to an
     * array, one of whose dimensions is the given IV.
     *
     * Memory is allocated using the mempool for this access vector.
     */
    ACCESS_VECTOR* divide_by_iv(UINT iv_idx, WN *wn);

    /**
     * Divide this access vector by a SYMBOL. If successful, this access
     * vector will hold the remainder, and the quotient is returned.
     *
     * <wn> is an address expression whose base address is a pointer to an
     * array, one of whose dimensions is the given SYMBOL.
     *
     * Memory is allocated using the mempool for this access vector.
     */
    ACCESS_VECTOR* divide_by_sym(SYMBOL& sym, WN *wn);
#endif  // LNO
     ...
     */
    WN* to_wn(DOLOOP_STACK *stack) const;
/*** DAVID CODE END ***/
};
