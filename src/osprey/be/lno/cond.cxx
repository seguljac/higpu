...
/*****************************************************************************
 *
 * The data structure used here, private to this file (for now) is a
 * COND_BOUNDS_INFO. It is nothing more than a system of equations
 * representing the loop bounds and further outer if conditions, and a stack
 * of symbols indicating which column in the system of equations corresponds
 * to which variable. Associate with each symbol is the outermost region in
 * which each symbol is known to be invariant. For example, if we have
 *
 *	if (n > 0) ...
 *
 * then when we see the if, we associate the if node with the symbol n. If
 * further in we have a use of n in some conditional expression, and the
 * writes that could alter that n (from the use-def chains) could have
 * happened inside the above if, we know that the n's are different.
 * Collect_Outer_Info works, and must work, from the outside in. It calls
 * Kill_Written_Symbols. That code makes sure that all symbols used are from
 * definitions outside of all other conditions that use the same symbol, so
 * that the same values are being reused. If that condition might not be
 * true, we discard all equations involving that symbol.
 *
 * DAVID COMMENT: I do not quite understand Kill_Written_Symbols. Consider the
 * following example:
 *
 *     for (int i = 0; i < 5; ++i) {
 *         if (n <= 0) {
 ...
 *     }
 *
 * The statement "n = 4" is a DEF for "if (n < 6)" and it is under the same
 * conditional as "if (n < 6)", but it does not invalidate the constraints
 * involving n: 0 < n < 6.
 *
 ****************************************************************************/
...
INT COND_BOUNDS_INFO::Lookup_Entry(SYMBOL s, WN* control)
{
    ...
    for (entry = 0; entry < _symbol_info.Elements(); entry++) {
        if (_symbol_info.Bottom_nth(entry).Symbol == s) return entry;
    }
...
void COND_BOUNDS_INFO::Kill_Written_Symbols(ACCESS_VECTOR* av,
        WN* code, WN* control)
{
    if (av->Contains_Lin_Symb() == FALSE) return;

    for (LWN_ITER* i = LWN_WALK_TreeIter(code); i; i = LWN_WALK_TreeNext(i))
    {
        WN* wn = i->wn;
        if (WN_operator(wn) != OPR_LDID) continue;
        ...
        // Do nothing for new symbols.
        if (wnouter == control) continue;
        ...
        if (defs == NULL) {
            ...
            if (TY_kind(ST_type(WN_st(wn))) == KIND_SCALAR) { 
                WN *tmp = wn;
                while (tmp && WN_opcode(tmp) != OPC_IO) {
        ...
        {
            if (LNO_Verbose) {
                fprintf(stderr, "Bad write for %s\n", symbol.Name());
            }

            Symbol_Info().Bottom_nth(entry).Outer_Nondef = control;

            /* Go through every equation and remove those containing this
             * variable. An equation is removed by zeroing all the
             * coefficients. We do not actually remove the equation since
             * parent code relies on the fact that the number of equations
             * only grows.
             */
            for (INT i = Bounds().Num_Le_Constraints() - 1; i >= 0; i--) {
                if (Bounds().Ale()(i, entry) == 0) continue;
...
/*****************************************************************************
 *
 * Add this equation to the soe, possibly adding additional symbols to the
 * Symbol_Info(). An access vector expresses ai+bj <= const_offset, which is
 * exactly what an soe wants. But the soe expects the variables to come in a
 * certain order, so look up variables in the list. Note that, for now, loop
 * indices are represented by the symbol (0,0,i).
 *
 ****************************************************************************/

INT COND_BOUNDS_INFO::Add_Access(ACCESS_VECTOR* av, WN* code, WN* control)
{
    if (av->Too_Messy || av->Contains_Non_Lin_Symb()) return 0;

    Kill_Written_Symbols(av, code, control);

    INT vsz = (av->Lin_Symb != NULL ? av->Lin_Symb->Len() : 0) +
        Symbol_Info().Elements() + av->Nest_Depth() + 1;
    mINT32 *v = CXX_NEW_ARRAY(mINT32, vsz, &LNO_local_pool);
    for (INT vszx = 0; vszx < vsz; vszx++) v[vszx] = 0;
...
void COND_BOUNDS_INFO::Collect_If_Info(WN* wn, BOOL in_then_part)
{
    ...
    if (Pool() != &LNO_local_pool) MEM_POOL_Push(&LNO_local_pool);
    ...
    if (Pool() != &LNO_local_pool) MEM_POOL_Pop(&LNO_local_pool);
}
...
/*****************************************************************************
 *
 * Examine all parents, looking for conditions that we can add to the system
 * of equations.
 *
 ****************************************************************************/
...
void COND_BOUNDS_INFO::Collect_Outer_Info(WN* wn, WN* child)
{
    ...
    /* The order must be from outside to in. */
    Collect_Outer_Info(LWN_Get_Parent(wn), wn);

    // Innermost loop is skipped.
...
/*****************************************************************************
 *
 * Is "av" possibly consistent inside "expr"? Return TRUE if hard to say.
 *
 * "expr" cannot be NULL.
 *
 ****************************************************************************/
...
BOOL Is_Consistent_Condition(ACCESS_VECTOR *av, WN *expr)
{
    ...
    BOOL answer = info.Bounds().Is_Consistent();
