...
//		data structure and the LNO_Info_Map map. If "promote_pointers" is
//		TRUE, try to promote pointers into arrays. Note that
//		"promote_pointers" doesn't set up access_vectors for the generated
//		arrays. If "strict_limit" is TRUE, use a strict limit on the number of
//		loops allowed in a nest (i.e. LNO_MAX_DO_LOOP_DEPTH_STRICT).
//		Otherwise, use a more liberal limit (i.e. LNO_MAX_DO_LOOP_DEPTH-1 =
//		32) return TRUE if there exists a DO_LOOP in the code.
//
...
/* mapping for the parent pointers for all nodes */
...
{
    Is_True(wn != NULL && WN_opcode(wn) == OPC_DO_LOOP,
            ("Get_Do_Loop_Info bug"));

    DO_LOOP_INFO* dli = (DO_LOOP_INFO*)WN_MAP_Get(LNO_Info_Map, wn);
    if (!ok_if_none) {
        FmtAssert(dli != NULL,
                ("Get_Do_Loop_Info(): Unmarked do loop 0x%lx", wn));
    }
...
{
    Is_True(wn != NULL && WN_opcode(wn) == OPC_IF, ("Get_If_Info bug"));

    IF_INFO* ii = (IF_INFO*)WN_MAP_Get(LNO_Info_Map, wn);
    if (!ok_if_none) FmtAssert(ii != NULL, ("Get_If_Info(): Unmarked if"));
