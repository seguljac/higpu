...
static BOOL Handle_Phase_Specific_Options(char *flag)
{
    ...
    if (phase == PHASE_COMMON || phase >= PHASE_COUNT) return FALSE;
...
void Process_Command_Line(INT argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            // Advance to the char after '-'.
            cp = argv[i]+1;

            // Match phase-specific options first.
            if (Handle_Phase_Specific_Options(argv[i])) continue;
...
void Prepare_Source(void)
{
    ...
    if (Err_File_Name != NULL && Err_File_Name[0] == '\0') {
...
/** DAVID CODE BEGIN **/
    Set_Trace_File(Trc_File_Name, TRUE);    // append
    if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
    {
        fprintf(TFile, "\n%s%s\tHICUDA TRACES IN BE\n%s%s\n",
