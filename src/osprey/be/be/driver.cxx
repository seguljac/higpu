...
/** DAVID CODE BEGIN **/
#ifdef HICUDA
#include "opt_du.h"             /* for Print_Du_Info */
#endif  // HICUDA
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
extern WN* (*Pre_Optimizer_p)(INT32, WN*,
        DU_MANAGER*, ALIAS_MANAGER*, IPA_NODE*);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
extern WN* Pre_Optimizer (INT32, WN*, DU_MANAGER*, ALIAS_MANAGER*, IPA_NODE*);
/*** DAVID CODE END ***/
...
extern void (*Perform_Procedure_Summary_Phase_p) (WN*, DU_MANAGER*,
        ALIAS_MANAGER*, void*, IPA_NODE*, WN_MAP);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
#ifdef HICUDA
        // We need lno.so when running PREOPT, due to the pointer
        // promotion phase.
...
/*** DAVID CODE END ***/
    ...
    if (Run_lno || Run_autopar)
    {
        ...
        // Load in ipl.so if we need to perform automatic parallelization and
        // interprocedural analysis has been performed.
...
Phase_Init(void)
{
    ...
    if (LNO_Run_Lego_Set && (LNO_Run_Lego == FALSE)) Run_Distr_Array = FALSE;

    if (Run_cg) CG_Init();
    if (Run_wopt || Run_preopt) Wopt_Init();
    if (Run_ipl) Ipl_Init();
    if (Run_lno || Run_Distr_Array || Run_autopar) Lno_Init();
    if (Run_purple) Prp_Init();
    ...
    if (output_file_name == NULL) {
        output_file_name = (Src_File_Name != NULL) ?
            Last_Pathname_Component(Src_File_Name) : Irb_File_Name;
    }
    ...
    if (need_ipl_output) {
        ...
        ir_output = Open_Output_Info(Obj_File_Name ?
                Obj_File_Name : New_Extension(output_file_name, ".o"));
    }
    if (Emit_Global_Data) {
        Write_BE_Maps = FALSE;
...
/** DAVID CODE BEGIN **/
        ...
        {
            fprintf(TFile,
                    "\n%s\t[Ipl_Processing] BEFORE Pre_Optimizer #1\n%s\n",
                    DBar, DBar);
            fdump_tree(TFile, pu);
        }

        // We can not enable IPL in this pass.
        Run_ipl = FALSE;
        pu = Pre_Optimizer(PREOPT_PRE_PP_PHASE, pu, du_mgr, al_mgr, NULL);
        Run_ipl = TRUE;

        if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
        {
            fprintf(TFile,
                    "\n%s\t[Ipl_Processing] AFTER Pre_Optimizer #1\n%s\n",
                    DBar, DBar);
            fdump_tree(TFile, pu);
        }

        transform_ptr_access_to_array(pu, du_mgr, al_mgr);
        ...
        pu = Pre_Optimizer(PREOPT_IPA0_PHASE, pu, du_mgr, al_mgr, NULL);

#ifdef HICUDA
        if (Get_Trace(TKIND_DEBUG, TDEBUG_HICUDA))
        {
            fprintf(TFile,
                    "\n%s\t[Ipl_Processing] AFTER Pre_Optimizer #2\n%s\n",
                    DBar, DBar);
            fdump_tree(TFile, pu);
        }
...
/** DAVID CODE BEGIN **/
        Perform_Procedure_Summary_Phase(pu, du_mgr, al_mgr, NULL,
                NULL, WN_MAP_UNDEFINED);
/*** DAVID CODE END ***/
    }
...
static WN* LNO_Processing(PU_Info *current_pu, WN *pu)
{
            ...
            REGION_CS_NoEarlierSub_Next(&rgn_iter))
    {
        ...
        Is_True(rwn != NULL, ("BE driver, IR inconsistency, LNO loop"));

        if (Get_Trace(TP_REGION,TT_REGION_ALL)) {
            fprintf(TFile,
                    "===== BE driver, LNO loop: %s %d, stacked=%d\n",
        ...
        /* Don't (re-)run LNO or preopt on nested mp PU's unless early mp
         * processing was requested. In which case LNO and preopt haven't been
         * run on the nexted mp PU's yet.
         */
        ...
        if (!is_mp || Early_MP_Processing)
        {
            if (Run_lno || Run_autopar || (Run_Distr_Array && needs_lno))
            {
                ...
                Set_PU_Info_state(Current_PU_Info, WT_DEPGRAPH,
                        (Current_Dep_Graph != NULL) ?
                        Subsect_InMem : Subsect_Missing);
        ...
        /* This is tricky. */
        if (REGION_CS_ITER_is_not_stacked(&rgn_iter)) pu = rwn;
...
Backend_Processing (PU_Info *current_pu, WN *pu)
{
    ...
    if (Run_lno || Run_Distr_Array || Run_preopt || Run_autopar) {
...
#endif
            /* make -O0, -O1, -O2 a bit faster */
            pu = LNO_Processing(current_pu, pu);
#ifndef KEY 
        ...
        if (Run_autopar) Rewrite_Pragmas_On_Structs(NULL, WN_func_body(pu));
#endif
    }
...
static void Preorder_Process_PUs(PU_Info *current_pu)
{
    ...
    if (!Force_Frame_Pointer_Set)
    {
        ...
        Force_Frame_Pointer = 
            ((PU_cxx_lang(func) && PU_has_region(func)) || Debug_Level > 0) ?
            TRUE : FALSE;
    }
#endif
    ...
    /* If SYMTAB_IPA_on is set then we have run ipl, and therefore already
     * done OMP_prelowering.  So don't do it again.
     */
...
main(INT argc, char **argv)
{
    ...
    if (need_wopt_output || need_lno_output || need_ipl_output) {
        ...
        if (need_ipl_output) Ipl_Extra_Output(ir_output);
        Close_Output_Info();
    } else if (Emit_Global_Data) {
        /* even if dummy pu, don't write any pu's */
