...
WN_Tree_Type(const WN *wn)
{
    ...
    TY_IDX ty = Stab_Mtype_To_Ty(MTYPE_V);
    ...
    if (OPCODE_is_expression(WN_opcode(wn)))
    {
        switch (WN_opc_operator(wn))
        {
            ...
            case OPR_ARRAY:
                ...
                if (! TY_Is_Pointer(ty))
                {
                    /* Create a pointer to be treated as an array when also
                     * used as the base-type of this OPC_ARRAY.  This must be
                     * handled very carefully in WN2C_array().
                     */
                    ty = Stab_Pointer_To(Stab_Array_Of(
                            Stab_Mtype_To_Ty(MTYPE_U1), WN_element_size(wn)));
                }
                else if (! TY_ptr_as_array(Ty_Table[ty]))
                {
                    INT ndims = WN_num_dim(wn);
                    ty = TY_pointed(ty);
                    while (ndims > 0 && TY_Is_Array(ty)) {
                        ndims -= ARB_dimension(TY_arb(ty));
                        ty = TY_AR_etype(ty);
                    }

                    // We should assert that 'ndims <= 0', but sometimes
                    // 'ty' may have already been a ptr-to-elem, so we will
                    // be silent here.
                    /*
                    Is_True(ndims <= 0,
                            ("WN_Tree_Type(): number of dimensions in "
                             "an array access exceeds that of "
                             "the array type."));
                    */
