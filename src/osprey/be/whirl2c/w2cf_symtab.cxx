...
static BOOL
W2CF_Avoid_Changing_Suffix(W2CF_SYMBOL *symbol)
{
    BOOL avoid = FALSE;

    // We don't want to rename any serious symbols.
    switch (W2CF_SYMBOL_kind(symbol)) {
        case SYMKIND_FLD:
  ...
  /* may have been created by different PUs                         */
    ...
    if (W2CF_SYMBOL_kind(symbol) == SYMKIND_ST) {
        ...
        if (Stab_Is_Common_Block(st)) avoid = TRUE;
    }
#endif
...
/**
 * Insert the symbol into the symbol-list associated with the given symbol
 * header. The symbols are in order of increasing unique symids.
 *
 * If the symbol has an INVALID or already used symid, then we create
 * a new one for it and update the symbol accordingly.
 */
static void
W2CF_Insert_Symbol(W2CF_SYMHDR *symhdr, W2CF_SYMBOL *symbol) {
    W2CF_SYMBOL *curr_sym, *prev_sym;
    INT32 symid, next_symid;

    // The "next_symid" is a lower bound on new symids.
    symid = W2CF_SYMBOL_symid(symbol);
    next_symid = W2CF_SYMHDR_next_symid(symhdr);
    if (symid < next_symid) symid = next_symid;

    // Find the insertion point for this new symbol. 
    curr_sym = W2CF_SYMHDR_symbol(symhdr); prev_sym = NULL;
    while (curr_sym != NULL && W2CF_SYMBOL_symid(curr_sym) < symid) {
        prev_sym = curr_sym;
        curr_sym = W2CF_SYMBOL_next(curr_sym);
    }

    if (curr_sym != NULL && W2CF_SYMBOL_symid(curr_sym) == symid) {
    ...
    W2CF_SYMBOL_next(symbol) = curr_sym;
    if (prev_sym == NULL) {
        W2CF_SYMHDR_symbol(symhdr) = symbol;
    } else {
        W2CF_SYMBOL_next(prev_sym) = symbol;
    }
   
    /* Update next_symid if necessary. Note that the new next_symid is always
     * larger than or equal to its former value.
     */
    if (symid == next_symid) {
        // Start from the symbol next to the one just inserted.
        ++next_symid;

        curr_sym = W2CF_SYMBOL_next(symbol);
        while (curr_sym != NULL
            && W2CF_SYMBOL_symid(curr_sym) == next_symid) {
            curr_sym = W2CF_SYMBOL_next(curr_sym);
            ++next_symid;
        }
...
/**
 * Create a symbol based at the given symhdr and with the symid, symkind
 * and associated attributes as specified by 'match_symbol'.
 *
 * If the symid is W2CF_INVALID_SYMID, then invent a new symid.
 */
static W2CF_SYMBOL *
W2CF_Create_Symbol(W2CF_SYMTAB *symtab, W2CF_SYMHDR *symhdr, 
    ...
    // Insert the symbol under the given symbol header, possibly changing
    // the symid if it is already in use or is invalid.
    W2CF_Insert_Symbol(symhdr, symbol);

    /* Construc the symbol name from the base name and the symbol id. */

    INT32 symid = W2CF_SYMBOL_symid(symbol);

    if (W2CF_Avoid_Changing_Suffix(symbol)) {
        // The symbol is not allowed to be renamed, so use the original
        // symbol id.
        symid = W2CF_SYMBOL_symid(match_symbol);
        if (symid == W2CF_INVALID_SYMID) symid = W2CF_NOSUFFIX_SYMID;
    }

    if (symid == W2CF_NOSUFFIX_SYMID) {
        W2CF_SYMBOL_name(symbol) = W2CF_SYMHDR_basename(symhdr);
    } else {
        char *basename = W2CF_SYMHDR_basename_string(symtab, symhdr);

        char *symname = Get_Name_Buf_Slot(strlen(basename) + 32);
        sprintf(symname, "%s%d", basename, symid);

        W2CF_SYMBOL_name(symbol) = W2CF_Symtab_Alloc_Chars(symtab,
            strlen(symname)+1);
