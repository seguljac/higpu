...
/** DAVID CODE BEGIN **/
    // processed by Preopt before pointer promotion in the driver
    RL_PRE_PP_PREOPT,
    RL_IPA0_PREOPT,
    RL_IPA_PRE_PP_PREOPT,
    RL_IPA_POST_PP_PREOPT,
    RL_IPA1_PREOPT, /* processed by IPA controlled Preopt		*/
    RL_IPA_KERNEL_DAS0_PREOPT,
    RL_IPA_KERNEL_DAS1_PREOPT,
    RL_IPA2_PREOPT,
/*** DAVID CODE END ***/
...
/* Region types: one-to-one correspondence with REGION_KIND in wn_core.h
 *
 ...
 * 
 * Make sure you update RID_type_str after modifying this type.
 */
typedef enum {
    RID_TYPE_undefined  = 0x0000,   // initialized value
    RID_TYPE_func_entry = 0x0001,   // root RID for PU associated w/func_entry
    RID_TYPE_loop       = 0x0002,   // it's a loop (RAIL)
    RID_TYPE_pragma     = 0x0004,   // it's an arbitrary region
    RID_TYPE_olimit     = 0x0008,   // it's an arbitrary region
    RID_TYPE_mp	        = 0x0010,   // it's a MP region (transparent)
    RID_TYPE_hicuda     = 0x0020,   // it's a hiCUDA region (transparent)
    RID_TYPE_rpi	    = 0x0040,   // it's a RPI region
    RID_TYPE_cold       = 0x0080,   // it's a cold region (transparent)
    RID_TYPE_swp	    = 0x0100,   // it's a SWP loop (transparent)

    /* DAVID COMMENT: we do not need to change the EH mask as there is still
     * room for non-EH region bits. */
...
/* Flag to tell us whether the boundary info is just NULL or really doesn't
 * exist. This is used by Preopt to tell if it should be conservative or not
 * in it's analysis.
 */
typedef enum {
...
/* Flag to tell us whether region has any return statements in it. This is
 * useful once the region is black for building CFGs - you need to know if
 * this region can exit the PU.
 */
typedef enum {
...
{
    // unique identifier
    // It is the first field so that refs don't require offsets.
    INT id;

    // type, 1:1 mapping to WHIRL region kind
    RID_TYPE rid_type;

    // region nesting depth, right now it is used for the loop level
    INT depth;

    // linenum for start of the original SCF, may be NULL
    SRCPOS srcpos;

    UINT32 flags;		/* includes WHIRL level				*/

    // points-to info for CG
    struct cgrin *cginfo;

    // number of exits from the region
    INT32 num_exits;
    ...
    // The WHIRL node
    // This field should be cleared if the WN is deleted.
    WN *rwn;

    // The WHIRL parent block that contains this region, used by the driver to
    // replace a region once it has been processed.
    WN *parent_block;
...
/** DAVID CODE BEGIN **/
#define RID_TYPE_hicuda(r) (RID_type(r) & RID_TYPE_hicuda)
#define RID_TYPE_hicuda_Set(r) \
    (RID_type(r) = (RID_TYPE)(RID_type(r) | RID_TYPE_hicuda))
#define RID_TYPE_hicuda_Reset(r) \
    (RID_type(r) = (RID_TYPE)(RID_type(r) & ~RID_TYPE_hicuda))
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
/* transparent regions (no boundary): PU, EH, MP, HICUDA, SWP, cold */
