...
GOTO* RINIT::Cancel_internal_gotos(BOOL prop_up)
{
    GOTO *gtmp;
    RGN_LABEL *htmp;
    BOOL found;
    ...
    /* Cancel out GOTOs that go to LABELs within the region. */
    for (gtmp = _goto_list; gtmp != NULL; gtmp = gtmp->Next())
    {
        // Only look at those not yet cancelled.
        if (!gtmp->Outside()) continue;

        found = FALSE;

        //==================================================================
        // cancel gotos that go to labels inside the region
        for (htmp = Label_list(); htmp != NULL; htmp=htmp->Next())
        {
    ...
    for (gtmp = Goto_list(); gtmp != NULL; gtmp = gtmp_next)
    {
        gtmp_next = gtmp->Next();
        if (!gtmp->Outside()) continue;

/** DAVID CODE BEGIN **/
        // An MP or HICUDA region cannot have a GOTO outside.
        if (RID_TYPE_mp(Rid()) || RID_TYPE_hicuda(Rid())) {
            char label_str[20];
            sprintf(label_str, "L%d", gtmp->Label_number());
            ErrMsg(EC_Rgn_Ill_Exit, label_str,
                    Srcpos_To_Line(gtmp->Linenum()));
        }
/*** DAVID CODE END ***/
        ...
        // DAVID COMMENT: how to keep _goto_list consistent?
        // link into list
        gtmp->Set_next(gnew);
        gnew = gtmp;
...
void RINIT::Process_region(WN *wtmp, WN *block, INT32 level,
        RID *root, char *parent_options)
{
    WN *wtmp2;
    char *region_options = NULL;

    Set_PU_has_region(Get_Current_PU());
    Is_True(wtmp != NULL && WN_opcode(wtmp) == OPC_REGION,
    ...
    // only count non-MP regions for LNO
    if (!RID_TYPE_mp(rid)) _nregions++;
    ...
    if (!RID_TYPE_transparent(rid)) {
        ...
        if (parent_options != NULL) {
            ...
            strcat(tmp, " ");
            if (region_options) strcat(tmp, region_options);
            region_options = tmp;
        }
...
/*** DAVID CODE END ***/
        ...
        WN *wtmp2 = WN_last(WN_region_body(wtmp));
        if (wtmp2 != NULL) {
            OPERATOR opr = WN_operator(wtmp2);
            if (opr != OPR_GOTO && opr != OPR_RETURN
                    && opr != OPR_REGION_EXIT) {
                ...
                Is_Trace(Trace(),
                        (TFile,
                         "RINIT::Process_region, added fall-thru "
...
/*** DAVID CODE END ***/
    ...
    if (wtmp2 != NULL && WN_opcode(wtmp2) == OPC_LABEL) {
...
void RINIT::Region_init(WN *block, INT32 level, RID *root, char *options)
{
#ifdef KEY
    ...
    for (WN *wtmp = WN_first(block); wtmp != NULL ; wtmp = WN_next(wtmp))
    {
        ...
        case OPR_BLOCK:
            // DAVID COMMENT: is this possible?
...
extern "C" INT REGION_Initialize(WN *wn, BOOL has_rgns)
{
    RID *rid;
    INT nregions = 0;
    ...
    // Save RID_map in an array because it may change in nested PUs.
    region_map_index++;
    FmtAssert(0 <= region_map_index
            && region_map_index < REGION_MAP_ARRAY_SIZE,
    ...
    if (Region_Skip_List != NULL) Region_skip(NULL, wn);

    // Now look for user inserted regions within the PUs (REGION_init). This
    // should really be moved to when the .B file is read in because it looks
    // at every node.
    if (has_rgns) nregions += REGION_init(wn, rid);
