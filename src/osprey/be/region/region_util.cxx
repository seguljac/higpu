...
/*****************************************************************************
 *
 * Given a RID, determine if this region could be black later on based on the
 * RID type. This is non-recursive.
 *
 ****************************************************************************/
...
BOOL REGION_has_black_regions(RID *rid)
{
    ...
    Is_True(RID_TYPE_func_entry(rid)
            || RID_TYPE_olimit(rid) || RID_TYPE_pragma(rid),
            ("REGION_has_no_black_regions, unsupported region type"));

    /* previously calculated, no traversal req'd */
    if (RID_contains_black(rid)) return TRUE;
...
{
    RID_level(rid) = level;
    for (RID *rtmp = RID_first_kid(rid); rtmp != NULL; rtmp = RID_next(rtmp)) {
...
REGION_LEVEL RID_preopt_level(INT phase)
{
    ...
    {
/** DAVID CODE BEGIN **/
        case PREOPT_PRE_PP_PHASE: return RL_PRE_PP_PREOPT;
        case PREOPT_IPA0_PHASE: return RL_IPA0_PREOPT;
        case PREOPT_IPA_PRE_PP_PHASE: return RL_IPA_PRE_PP_PREOPT;
        case PREOPT_IPA_POST_PP_PHASE: return RL_IPA_POST_PP_PREOPT;
        case PREOPT_IPA1_PHASE: return RL_IPA1_PREOPT;
        case PREOPT_IPA_KERNEL_DAS0_PHASE: return RL_IPA_KERNEL_DAS0_PREOPT;
        case PREOPT_IPA_KERNEL_DAS1_PHASE: return RL_IPA_KERNEL_DAS1_PREOPT;
        case PREOPT_IPA2_PHASE: return RL_IPA2_PREOPT;
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    case RL_PRE_PP_PREOPT: strcpy(buff, "RL_PRE_PP_PREOPT"); break;
    case RL_IPA0_PREOPT: strcpy(buff, "RL_IPA0_PREOPT"); break;
    case RL_IPA_PRE_PP_PREOPT: strcpy(buff, "RL_IPA_PRE_PP_PREOPT"); break;
    case RL_IPA_POST_PP_PREOPT: strcpy(buff, "RL_IPA_POST_PP_PREOPT"); break;
    case RL_IPA1_PREOPT: strcpy(buff, "RL_IPA1_PREOPT"); break;
    case RL_IPA_KERNEL_DAS0_PREOPT:
                         strcpy(buff, "RL_IPA_KERNEL_DAS0_PREOPT"); break;
    case RL_IPA_KERNEL_DAS1_PREOPT:
                         strcpy(buff, "RL_IPA_KERNEL_DAS1_PREOPT"); break;
    case RL_IPA2_PREOPT: strcpy(buff, "RL_IPA2_PREOPT"); break;
/*** DAVID CODE END ***/
...
char* RID_type_str(RID_TYPE type)
{
    ...
    if (type & RID_TYPE_func_entry) strcat(buff," FUNC_ENTRY");
    if (type & RID_TYPE_loop) strcat(buff," LOOP");
    if (type & RID_TYPE_pragma) strcat(buff," PRAGMA");
    if (type & RID_TYPE_olimit) strcat(buff," OLIMIT");
    if (type & RID_TYPE_mp) strcat(buff," MP");
/** DAVID CODE BEGIN **/
    if (type & RID_TYPE_hicuda) strcat(buff, " HICUDA");
/*** DAVID CODE END ***/
    if (type & RID_TYPE_rpi) strcat(buff," RPI");
    if (type & RID_TYPE_cold) strcat(buff," COLD");
    if (type & RID_TYPE_swp) strcat(buff," SWP");
    if (type & RID_TYPE_try) strcat(buff," TRY");
    if (type & RID_TYPE_cleanup) strcat(buff," CLEANUP");
    if (type & RID_TYPE_exc_spec) strcat(buff," EXC_SPEC");
    if (type & RID_TYPE_mask) strcat(buff," MASK");
    if (type & RID_TYPE_guard) strcat(buff," GUARD");
    if (type & RID_TYPE_null_cleanup) strcat(buff," NULL_CLEANUP");

    /* mask */
    if (type & RID_TYPE_eh) strcat(buff," EH");
    if (type == RID_TYPE_undefined) strcpy(buff," UNDEFINED");
...
REGION_KIND REGION_type_to_kind(RID *rid)
{
    ...
    if (RID_type(rid) & RID_TYPE_func_entry) return REGION_KIND_FUNC_ENTRY;
    if (RID_type(rid) & RID_TYPE_loop) return REGION_KIND_LOOP;
    if (RID_type(rid) & RID_TYPE_pragma) return REGION_KIND_PRAGMA;
    if (RID_type(rid) & RID_TYPE_olimit) return REGION_KIND_OLIMIT;
    if (RID_type(rid) & RID_TYPE_mp) return REGION_KIND_MP;
/** DAVID CODE BEGIN **/
    if (RID_type(rid) & RID_TYPE_hicuda) return REGION_KIND_HICUDA;
/*** DAVID CODE END ***/
    if (RID_type(rid) & RID_TYPE_rpi) return REGION_KIND_RPI;
    if (RID_type(rid) & RID_TYPE_cold) return REGION_KIND_COLD;
    if (RID_type(rid) & RID_TYPE_swp) return REGION_KIND_SWP;
    if (RID_type(rid) & RID_TYPE_try) return REGION_KIND_TRY;
    if (RID_type(rid) & RID_TYPE_cleanup) return REGION_KIND_CLEANUP;
    if (RID_type(rid) & RID_TYPE_exc_spec) return REGION_KIND_EXC_SPEC;
    if (RID_type(rid) & RID_TYPE_mask) return REGION_KIND_MASK;
    if (RID_type(rid) & RID_TYPE_guard) return REGION_KIND_GUARD;
    if (RID_type(rid) & RID_TYPE_null_cleanup) return REGION_KIND_NULL_CLEANUP;
    ...
    else
    {
        ...
        switch (WN_region_kind(wn)) {
            ...
            case REGION_KIND_OLIMIT:
                ...
                break;
            case REGION_KIND_MP:
                Is_True(REGION_is_mp(wn),
                        ("REGION_kind_to_type, internal error"));
                RID_TYPE_mp_Set(rid);
                break;
/** DAVID CODE BEGIN **/
            case REGION_KIND_HICUDA:
                RID_TYPE_hicuda_Set(rid);
                break;
/*** DAVID CODE END ***/
