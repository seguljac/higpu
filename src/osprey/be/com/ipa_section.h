...
enum LTKIND
{
    ...
    LTKIND_IV           // One of the independent variables (ivar)
};

// when we set up the system of equations we need to specify the action type
enum ACTION_TYPE
{
    ACTION_EQ,          // equal
    ACTION_LO,          // lower bound?
    ACTION_UP           // upper bound?
};
...
//============================================================================
//
// In IPL and IPA, an IVAR represents either a formal parameter (specified by
// its position) or a global variable (ST_IDX).
//
// In LNO, an IVAR may also represent a local ST_IDX, when mapping a formal
// parameter into the actual argument.
//
//============================================================================
class IVAR
{
...
public:
    ...
    { 
        FmtAssert(_is_formal,
                ("IVAR::Formal_Position(): IVAR is not a formal"));
    ...
    { 
        return (_is_formal == other._is_formal
                ...
                && _mtype == other._mtype
                && ((_is_formal && u._formal_position == other.u._formal_position) || (!_is_formal && u._st_idx == other.u._st_idx)));
    }
...
private:
    ...
    COEFF _coeff;
    DESCR _desc;    // DAVID COMMENT: what does this mean in each case?
    LTKIND _kind : 8;
...
public:
    ...
    /* The following functions are defined in ipa_section_main.cxx. */

    BOOL Equivalent(TERM& t);

    BOOL Is_equal(TERM* t, INT count);

    /* The following functions are defined in ipa_section_print.cxx. */
...
public:
    ...
    /* The following functions are defined in ipa_section.cxx. */
    ...
    // map a linear expression to a vector in the systems of equations
    ...
    // check if the linex is a constant term only (used for step calculation)
    BOOL Is_const();
...
private:
    ...
    // attributes of _flags
#define MESSY_LOOP_UPPER_BOUND  0x01
#define MESSY_LOOP_LOWER_BOUND  0x02
#define MESSY_LOOP_STRIDE       0x04
...
public:
    ...
    BOOL Is_messy_ub() const { return _flags & MESSY_LOOP_UPPER_BOUND; }
    BOOL Is_messy_lb() const { return _flags & MESSY_LOOP_LOWER_BOUND; }
    BOOL Is_messy_step() const { return _flags & MESSY_LOOP_STRIDE; }
    BOOL Is_messy_bounds() const { return _flags & MESSY_BOUNDS; }
    BOOL Is_messy_any_bounds() const { return _flags & MESSY_ANY_BOUNDS; }

    void Set_messy_ub ()    { _flags |= MESSY_LOOP_UPPER_BOUND; }
    void Set_messy_lb()     { _flags |= MESSY_LOOP_LOWER_BOUND; }
    void Set_messy_step()   { _flags |= MESSY_LOOP_STRIDE; }
    ...
    void Add_bound(LINEX* l, SYSTEM_OF_EQUATIONS* soe, 
            mUINT8 depth, INT num_dim, INT num_syms, LOOP_SYMBOL_ARRAY* sym);
...
// linex constraints.
// This is equivalent to the AXLE_NODE structure (LNO).
//------------------------------------------------------------------------
class PROJECTED_NODE
{
private:

    // attributes for _flags
#define MESSY_UPPER_BOUND   0x00000001
#define MESSY_LOWER_BOUND   0x00000002
#define MESSY_STRIDE        0x00000004
#define UNPROJECTED         0x00000008
#define ASSUMED_SHAPE       0x00000010    // DAVID COMMENT: ???
...
public:
    ...
    INT Get_constant_term() { return Get_lower_linex()->Get_constant_term(); }

    void Create_linex(TERM* t);

    void Set_linexs(LINEX* low_new, LINEX* up_new, LINEX* step_new,
            LINEX* segment_length_new, LINEX* segment_stride_new);
    ...
    void Set_constant_linexs(INT32 upper, INT32 lower, INT32 step, 
            INT32 segment_length, INT32 segment_stride);

    // Set linexes for a constant two-strided array section
    void Set_constant_two_strided_section(INT32 lower, INT32 upper, 
            INT32 step, INT32 seg_len, INT32 seg_stride);
...
/** DAVID CODE BEGIN **/
    // copy to 
    void Copy(PROJECTED_NODE *to, MEM_POOL *pool);
/*** DAVID CODE END ***/
...
private:
    ...
    // attributes for _type
#define MESSY_REGION        0x0001
#define NON_MESSY_REGION    0x0002
#define UNPROJECTED_REGION  0x0004
#define IS_MAY_KILL         0x0008
#define IS_MAY_USE          0x0010
#define IS_PASSED           0x0020      // passed as an actual parameter
#define IS_FORMAL           0x0040
    ...
    // An index into the linex kernel (if existed), used for optimizing
    // away the projection operation
    union {
...
public:
    ...
    PROJECTED_REGION(ACCESS_ARRAY *ar, MEM_POOL *mem_pool, LOOPINFO *loop,
            BOOL in_ipl = TRUE, IPA_LNO_READ_FILE *IPA_LNO_File = NULL);

/** DAVID CODE BEGIN **/
    // PROJECTED_REGION(PROJECTED_REGION *p);
/*** DAVID CODE END ***/

    PROJECTED_REGION(mINT16 type, mUINT8 depth, mUINT8 num_dim,
            MEM_POOL *mem_pool);

/** DAVID CODE BEGIN **/
    PROJECTED_REGION(ACCESS_ARRAY *lbnd, ACCESS_ARRAY *ubnd,
            mUINT8 depth, MEM_POOL *pool);
/*** DAVID CODE END ***/
    ...
    {
/** DAVID CODE BEGIN **/
        // There are two flags MESSY_REGION and NON_MESSY_REGION.
        // We need to keep internal consistency.
        _type |= MESSY_REGION;
        _type &= (~NON_MESSY_REGION);
/*** DAVID CODE END ***/
    }
...
/** DAVID CODE BEGIN **/
    PROJECTED_REGION* create_deep_copy(MEM_POOL *pool);
    // void Copy_projected_node(PROJECTED_NODE* node, MEM_POOL *pool);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    /**
     * Return 1 if this instance contains <b>, 0 if not, and -1 if not sure.
     * We do not care about the IS_MAY_USE or IS_MAY_KILL flags.
     */
    INT Contains(PROJECTED_REGION *b);
/*** DAVID CODE END ***/
...
public:
    ...
    void Set_projected_level(mINT16 level) { _projected_level = level; }
...
public:
    ...
    PROJECTED_REGION* Get_projected_region() { return _p; }
...
/* type attributes for REGION_ARRAYS */
#define IPL_HAS_BAD_ALIAS       0x01
#define IPL_IS_LOOP_INVARIANT   0x02
#define IPL_IS_DEF              0x04    // is it a definition?
#define IPL_IS_USE              0x08    // is it a use?
#define IPL_IS_PASSED           0x10    // is is passed?
#define IPL_IS_MAY_USE          0x20    // may be used
#define IPL_IS_MAY_DEF          0x40    // may be defined
#define IPL_IS_FORMAL           0x80    // is a formal parameter?
...
private:
    ...
    union {
        ...
        struct {
            ...
            mINT32 _count;  // # of the elements in the PROJECTED_REGION array
        } u2;
    } u1;
...
public:
    ...
    REGION_ARRAYS(MEM_POOL *m, mINT32 index) {
        u1._regions = (PROJECTED_REGION_INFO_ARRAY*)CXX_NEW(
                PROJECTED_REGION_INFO_ARRAY(m), m);
        _type = 0;
        _sym_index = index;
    }
    ...
    PROJECTED_REGION_INFO_ARRAY* Get_projected_region_array() const {
        return u1._regions;
    }
    ...
    void Set_is_loop_invariant() { _type = _type | IPL_IS_LOOP_INVARIANT; }
...
private:
    ...
    // attributes for _type
#define IPA_SCALAR_MAY_KILL     0x0001
#define IPA_SCALAR_MAY_USE      0x0002
#define IPA_SCALAR_MAY_REDUC    0x0004
#define IPA_SCALAR_KILL         0x0008
#define IPA_SCALAR_USE          0x0010
#define IPA_SCALAR_REDUC        0x0020
#define IPA_ARRAY_REDUC         0x0040
#define IPA_ARRAY_MAY_REDUC     0x0080
#define IPA_SCALAR_PASSED       0x0100
#define IPA_SCALAR_EUSE         0x0200
#define IPA_SCALAR_CALL_EUSE    0x0400
#define IPA_SCALAR_MAY_PASS     0x0800

    mINT32 _index;          // index into the SUMMARY_SYMBOL array
    mINT16 _type;           // type information

    mINT16 _call_index;     // index into the SUMMARY_CALLSITE array when
...
//
// The actual arrays are used in filling Array_Summary, while _*_index
// and _*_count are used in filling Array_Summary_Out.
//-------------------------------------------------------------------------
...
private:
    ...
    union {
        LOOPINFO *_loop;                // loop node (not an array!)
        mINT32 _index;                  // index 
    } u5;
    union {
...
public:
    ...
    { 
        u1._def = (ARRAY_OF_REGION_ARRAYS*)CXX_NEW(
                ARRAY_OF_REGION_ARRAYS(m), m);
        u2._use = (ARRAY_OF_REGION_ARRAYS*)CXX_NEW(
                ARRAY_OF_REGION_ARRAYS(m), m);
        u3._param = (ARRAY_OF_REGION_ARRAYS*)CXX_NEW(
                ARRAY_OF_REGION_ARRAYS(m), m);
        u4._scalar_info = (INT_ARRAY*)CXX_NEW(INT_ARRAY(m), m);
        u5._loop = (LOOPINFO*)CXX_NEW(LOOPINFO(m, index), m);
        u6._formal = (ARRAY_OF_REGION_ARRAYS*)CXX_NEW(
                ARRAY_OF_REGION_ARRAYS(m), m);
    ...
    void Add_array_param(PROJECTED_REGION *p, mINT32 sym_index,
            mINT32 element_size, INT16 callsite_id, INT16 actual_id);
    ...
    void Set_else_index(INT index) { 
        if (! Is_if()) {
            Fail_FmtAssertion(
                    "Invalid type when setting else index for CFG_NODE\n");
        }
    ...
    INT Get_else_index() { 
        if (! Is_if()) {
            Fail_FmtAssertion(
                    "Invalid type when using Get_else_index for CFG_NODE");
            return -1;
        }
...
//--------------------------------------------------------------------------
// Stores the corresponding index into CFG_NODE_INFO::_scalar_info for
// each actual node, if it is a scalar passed in as a reference.
//--------------------------------------------------------------------------
class INT_IDS
{
private:

    INT32 _id;          // index into CFG_NODE_INFO::_scalar_info
    INT32 _cd_idx;      // CFG_NODE_INFO index
...
public:
            ...
            INT cd_size)
    {
        ...
        _project_nodes = CXX_NEW(PROJECTED_ARRAY(&_write_pool), &_write_pool);
        _projected_regions = CXX_NEW(
                PROJECTED_REGION_ARRAY(&_write_pool), &_write_pool);
        _region_arrays = CXX_NEW(
                ARRAY_OF_REGION_ARRAYS(&_write_pool), &_write_pool);
        _cfg_nodes = CXX_NEW(CFG_NODE_INFO_ARRAY(&_write_pool), &_write_pool);
        _loop_nodes = CXX_NEW(LOOPINFO_ARRAY(&_write_pool), &_write_pool);
        _actual_scalar_info_map = (INT_IDS*)MEM_POOL_Alloc(&_write_pool,
                sizeof(INT_IDS)*(actual_count+1));
        ...
        if (cd_size) {
            _cd_map = (INT*)MEM_POOL_Alloc(&_write_pool, sizeof(INT)*cd_size);
        }
...
/*** DAVID CODE END ***/
    ...
    PROJECTED_ARRAY* Get_projected_array() { return _project_nodes; }
    PROJECTED_REGION_ARRAY* Get_projected_region_array() {
        return _projected_regions;
    }
    ARRAY_OF_REGION_ARRAYS* Get_region_array() { return _region_arrays; }
    CFG_NODE_INFO_ARRAY *Get_cfg_node_array() { return _cfg_nodes; }
    LOOPINFO_ARRAY *Get_loopinfo_array() { return _loop_nodes; }
    ...
    INT Get_loopinfo_array_count() const { return _loop_nodes->Lastidx() + 1; }
    INT Get_formal_start_idx() const { return _formal_start_idx; }
    INT Get_formal_count() const { return _formal_count; }
    INT Get_actual_start_idx() const { return _actual_start_idx; }
    INT Get_actual_count() const { return _actual_count; }
    INT Get_callsite_start_idx() const { return _callsite_start_idx; }
