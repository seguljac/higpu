...
public:
    union {
        ST* sym;            // has symbol 
        mUINT64 line_num;   // used to distinguish malloc/alloca etc.
        INT idx;            // index, say, the index of formal argument
    };

    SYMBOL_NAME(ST* symbol) : sym(symbol) {}

    // <not_used> is actually used by the compiler to choose desiriable 
    // overloading constructor.
    SYMBOL_NAME(mUINT64 ln, BOOL not_used) : line_num(ln) {
        not_used = not_used;
    }
...
/*****************************************************************************
 *
 * Uniform names for the following variables/entities in a program:
 * 
 * - named variable visible to the current function
 * - invisible variables
 * - memory block allocated via malloc/alloca
 * - imaginary pointer returned from callee
 *
 ****************************************************************************/
...
private:
    ...
    // interpret the previous union
    UNAME_TYPE _type;
    // ID of the name in the given name space
    // zero if it does not belong to any name space
    INT32 _unique_id;
