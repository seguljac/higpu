...
/** DAVID CODE BEGIN **/
// from be/lno/ptr_promotion.h
class DU_MANAGER;
WN* (*HC_extract_arr_base_p)(WN *addr_wn, ST_IDX& arr_st_idx);
void (*transform_ptr_access_to_array_p)(WN*, DU_MANAGER*);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
WN* (*Pre_Optimizer_p) (INT32, WN*, DU_MANAGER*, ALIAS_MANAGER*, IPA_NODE*);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
void (*Perform_Procedure_Summary_Phase_p) (WN*,
        DU_MANAGER*, ALIAS_MANAGER*, void*, IPA_NODE*, WN_MAP);
/*** DAVID CODE END ***/
#ifdef KEY
...
/** DAVID CODE BEGIN **/
// ----------------------------------------------------------------------
// symbols defined in ipa.so:
//
// I cannot find where <IPA_get_symbol_file_array_p> is defined, so I will
// define my IPA external interface here.

class IPA_EDGE;
class IPA_NODE;
class STATE_ARRAY;
class GLOBAL_ARRAY_TABLE;
class WN_TO_EDGE_MAP;   // fake?

// from ipa/main/analyze/ipa_cg.h
IPA_EDGE* (*IPA_map_callsites_p)(IPA_NODE*, WN*, WN_TO_EDGE_MAP*);
void (*IPA_print_kernel_das_p)(IPA_NODE*);

// from ipa/main/analyze/ipa_section_annot.h
BOOL (*IPA_get_call_data_summary_p)(IPA_EDGE*,
        STATE_ARRAY**, GLOBAL_ARRAY_TABLE**, MEM_POOL*);

// from ipa/main/analyze/ipa_hc_gpu_data_prop.h
void (*IPA_HC_match_gpu_data_with_kernel_das_p)(WN*, IPA_NODE*, MEM_POOL*);

/*** DAVID CODE END ***/

// ----------------------------------------------------------------------
// symbols defined in whirl2c.so 
