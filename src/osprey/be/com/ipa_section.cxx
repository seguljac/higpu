...
{
    if (av == NULL
            || av->Too_Messy || av->Delinearized_Symbol
            || av->Contains_Non_Lin_Symb()
            || Access_vector_is_too_messy(av)) return NULL;
...
/** DAVID CODE BEGIN **/
  if (lb->Num_Vec() > 1 || ub->Num_Vec() > 1
#ifdef IPL_ACCESSV
      || !((IPL_ACCESS_VECTOR*)step)->Is_Const()
#else
      || !step->Is_Const()
#endif
      ) {
...
void LINEX::Map_access_vector(ACCESS_VECTOR *av,
        BOOL Is_LNO, IPA_LNO_READ_FILE *IPA_LNO_File)
{
    ...
    if (av->Contains_Lin_Symb())
    {
        ...
        for (INTSYMB_NODE* cur = iter.First(); !iter.Is_Empty(); 
                cur = iter.Next())
        {
            ...
            if (ST_class(st) == CLASS_CONST)
            {
                ...
                Set_term(LTKIND_CONST, (COEFF)const_value * cur->Coeff,
                        CONST_DESC, 0);
                processed = TRUE;
            }
            else
            {
                Is_True(ST_class(st) == CLASS_VAR,
                        ("LINEX::Map_access_vector(): Unexpected ST_class"));

                IVAR ivar;

                if (ST_IDX_level(ST_st_idx(st)) == GLOBAL_SYMTAB) {
                    new (&ivar) IVAR(st,
                            cur->Symbol.WN_Offset(), cur->Symbol.Type);
                } else {
                ...
                {
                    INT i, n_ivars = IPA_LNO_File->Ivar_Count();
                    for (i = 0; i < n_ivars; i++) {
                        if (*(IPA_LNO_File->Ivar(i)) == ivar) break;
                    }
                    ivar_index = (i < n_ivars) ?
                        i : IPA_LNO_File->Add_Translated_Ivar(ivar);
                }
...
{
    INT num_loop_coeff = 0, last_idx = Num_terms();

    for (INT i = 0; i <= last_idx; ++i) {
        if (Get_term(i)->Get_type() == LTKIND_LINDEX) ++num_loop_coeff;
    }
...
//=================================================================
BOOL LINEX::Loop_coeff_terms_equal(LINEX *kernel_linex)
{
    INT num_coeffs = Num_loop_coeff_terms();
    INT num_coeffs_kernel = kernel_linex->Num_loop_coeff_terms();

    if (num_coeffs != num_coeffs_kernel) return FALSE;

    // DAVID: are loop coeff terms always contiguous?
...
//====================================================================
BOOL LINEX::Has_loop_coeff(INT depth)
{
    INT i, last_idx = _larray.Lastidx();

    for (i = 0; i <= last_idx; ++i)
    {
        TERM *t = &(_larray)[i];
        if ((t->Get_type() == LTKIND_LINDEX) && (t->Get_desc() == depth))
            return TRUE;
    }
...
{
    INT from_last_idx = from->Num_terms();

    for (INT i = 0; i <= from_last_idx; ++i)
    {
...
void PROJECTED_REGION::Project(INT depth, LOOPINFO* loop_info)
{
    if (Is_messy_region()) return;

    // if it is independent of the loop, there is nothing to project away
    PROJECTED_KERNEL *p = Get_projected_kernel();
    FmtAssert(p != NULL, ("Null projected kernel encounterd\n"));
    if (p->Is_independent(depth)) return;

    // check if the kernel has already been projected on the current loop
    if (p->Get_projected_level() > depth) p->Project(depth, loop_info);

    if (p->Get_region() != NULL && p->Get_region()->Is_messy_region()) {
        Set_messy_region();
        return;
    }
    ...
    LINEX_ARRAY *diff = p->Get_Difference();
    if (diff != NULL) diff->Resetidx();
}
...
//-----------------------------------------------------------------------
void LINEX::Substitute_Lindex(INT lindex, LINEX *lx_substitute)
{
    INT lindex_coeff = 0;
...
/*****************************************************************************
 *
 * Construct a PROJECTED_REGION (that is already projected) from the lower and
 * upper bound and a unit stride in each dimension. <depth> is the number of
 * enclosing loops.
 *
 ****************************************************************************/

PROJECTED_REGION::PROJECTED_REGION(ACCESS_ARRAY *lbnd, ACCESS_ARRAY *ubnd,
        mUINT8 depth, MEM_POOL *pool)
{
    mUINT8 ndims = lbnd->Num_Vec();
    ...
    if (lbnd == NULL || lbnd->Too_Messy
            || ubnd == NULL || ubnd->Too_Messy) return;
    for (INT i = 0; i < ndims; ++i) {
        if (Access_vector_is_too_messy(lbnd->Dim(i))) return;
        if (Access_vector_is_too_messy(ubnd->Dim(i))) return;
    }

    // NOW we have a non-messy region
    Set_type(NON_MESSY_REGION);
    Set_projected_array(CXX_NEW(PROJECTED_ARRAY(pool), pool));
    Get_projected_array()->Force_Alloc_array(ndims);
    Get_projected_array()->Setidx(ndims-1);

    for (INT i = 0; i < ndims; ++i)
    {
        PROJECTED_NODE *pn = Get_projected_node(i);
        pn->Init(pool);
        pn->Get_lower_linex()->Map_access_vector(lbnd->Dim(i),
                FALSE, NULL);
        pn->Get_upper_linex()->Map_access_vector(ubnd->Dim(i),
                FALSE, NULL);
        pn->Get_step_linex()->Set_term(LTKIND_CONST,
                (COEFF)1, CONST_DESC, 0);
...
PROJECTED_REGION::PROJECTED_REGION(ACCESS_ARRAY* array, MEM_POOL* m,
        LOOPINFO* loop, BOOL in_ipl, IPA_LNO_READ_FILE* IPA_LNO_File) 
{
    mUINT8 num_dim = array->Num_Vec();
    mUINT8 depth = (loop != NULL) ? loop->Get_nest_level() + 1 : 0;
    INT i;
    ...
    if (in_ipl) IPL_Access_Array_Map->Enter(this, array);

    // check for the cases that result in a messy region
    if (array == NULL || array->Too_Messy) return;
    for (i = 0; i < num_dim; ++i) {
        if (Access_vector_is_too_messy(array->Dim(i))) return;
    }
    ...
    for (i = 0; i < num_dim; ++i) {
        PROJECTED_NODE *pn = Get_projected_node(i);
        pn->Init(m);
        pn->Get_lower_linex()->Map_access_vector(array->Dim(i),
                !in_ipl, IPA_LNO_File);
        pn->Set_unprojected();
    }
    ...
    for (i = 0; i <= kernels->Lastidx(); ++i)
    {
        ...
        if (kernel->Get_depth() != depth ||
                kernel->Get_num_dims() != num_dim) continue;
        ...
        for (j = 0; j < num_dim; ++j) {
            ...
            if (!lower->Loop_coeff_terms_equal(kernel_linex)) break;
        }

        // DAVID COMMENT: should we not init <match> every iteration?
        if (j != num_dim) match = FALSE;
...
INT PROJECTED_REGION::Contains(PROJECTED_REGION *b)
{
    // Back up the flags of both instances.
    mINT16 saved_type = _type, saved_b_type = b->_type;

    // Turn off IS_MAY_USE and IS_MAY_KILL flags in both instances.
    _type &= (~IS_MAY_USE);
    _type &= (~IS_MAY_KILL);
    b->_type &= (~IS_MAY_USE);
    b->_type &= (~IS_MAY_KILL);

    INT result = Compare(b);

    // Restore the flags.
    _type = saved_type;
    b->_type = saved_b_type;
    ...
    Is_True(FALSE, (""));       // should never reach here
    return -1;
}
...
/*****************************************************************************
 *
 * Compare this region <a> and the given region <b>. Return
 *
 * 0 ---- cannot compare
 * 1 ---- a < b 
 * 2 ---- b < a
...
INT PROJECTED_REGION::Compare(PROJECTED_REGION *b)
{
    ...
    if (Get_type() != b->Get_type()) return 0;
    if (Get_num_dims() != b->Get_num_dims()) return 0;

/** DAVID CODE BEGIN **/
    // The two regions are not comparable if either one of them is messy.
    if (Is_messy_region() || b->Is_messy_region()) return 0;
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    // I have no idea why this test is here.
#if 0
    for (i=0; i< Get_num_dims(); ++i) 
    {
        ...
        node_a = &(*array_a)[i];
        node_b = &(*array_b)[i];
        if (!node_a->Equivalent(*node_b)) return 0;
    }
#endif
/*** DAVID CODE END ***/
...
/*****************************************************************************
 *
 * Find the region of access of this kernel, AFTER all the enclosing loops
 * from <level> up are projected away.
 *
 ****************************************************************************/

void PROJECTED_KERNEL::Project(mUINT8 level, LOOPINFO *loop)
{
    ...
    MEM_POOL_Push(local_array_pool);
    {
        ...
        SYSTEM_OF_EQUATIONS* soe = CXX_NEW(SYSTEM_OF_EQUATIONS(
                    0, 0, num_vectors+_depth, local_array_pool),
                local_array_pool);

        LOOP_SYMBOL_ARRAY *syms = CXX_NEW(
                LOOP_SYMBOL_ARRAY(local_array_pool), local_array_pool);
        ...
        PROJECTED_REGION* region = Get_region();
...
void CFG_NODE_INFO::Add_def_array(PROJECTED_REGION* p,
        mINT32 element_size, mINT32 sym_index)
{
    ARRAY_OF_REGION_ARRAYS *def = Get_def_array();

    // Search for array symbol in the list of arrays.
    PROJECTED_REGION_INFO_ARRAY *proj_array = NULL;
 ...
 *
 * sym_index:       index into the SUMMARY_SYMBOL array
 * element_size:    array element size
 * callsite_id:     index into the SUMMARY_CALLSITE array
 * actual_id:       zero-based position of the actual parameter
 *
 ****************************************************************************/

void CFG_NODE_INFO::Add_array_param(PROJECTED_REGION *p, mINT32 sym_index,
        mINT32 element_size, INT16 callsite_id, INT16 actual_id)
{
    ...
    /* DAVID COMMENT: In Add_def/use_array, we search for the region array
     * based on the symbol. Here, each region array has ONLY ONE region.
     */
    ...
    (*proj_array)[proj_array->Newidx()].Set_projected_region(p);
...
//===================================================================
// Store the array section (as a formal parameter) passed.
//
// element_size:    the element size of the array type
// idx_symbol:      the formal's index into the SUMMARY_SYMBOL summary
// idx_formal:      the formal's index into the SUMMARY_FORMAL summary
//===================================================================
void CFG_NODE_INFO::Add_formal_array(PROJECTED_REGION *p,
        mINT32 element_size, mINT32 idx_symbol, mINT32 idx_formal)
{
    p->Set_is_formal();

    // Add a new entry in the list of region arrays.
    ...
    // Save the new entry's index in the formal summary.
    Summary->Get_formal(idx_formal)->Set_region_index(formal_idx);

    // Add the given region to the new region array.
    PROJECTED_REGION_INFO_ARRAY* proj_array = r->Get_projected_region_array();
...
/*****************************************************************************
 *
 * Set the <_difference> field to <pr> - <this>.
 *
 ****************************************************************************/
...
{ 
    for (INT i = 0; i < Get_num_dims(); i++)
    {
        ...
        LINEX* lx_new = &(*_difference)[_difference->Newidx()];
