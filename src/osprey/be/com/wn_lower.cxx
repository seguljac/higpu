...
static WN *lower_stmt(WN *block, WN *tree, LOWER_ACTIONS actions)
{
    ...
    Is_True(OPCODE_is_stmt(opc),
            ("expected statement node, not %s", OPCODE_name(opc)));
    ...
    switch(WN_operator(tree))
    {
        ...
        case OPR_EXC_SCOPE_BEGIN:
            ...
            /*
             *  We don't want to lower the kids, which are not used to
             *  generate code, but only to provide information to the
             *  optimizer. 
             */
            break;
        ...
        case OPR_ASM_STMT:
        {
            ...
            for (i = 2; i < WN_kid_count(tree); i++) {
                WN_kid0(WN_kid(tree,i)) =
                    lower_expr(block, WN_kid0(WN_kid(tree,i)), actions);
            }
            break;
        }
...
/** DAVID CODE BEGIN **/
        ...
        {
            // Skip hiCUDA pragmas.
            WN_PRAGMA_ID pid = (WN_PRAGMA_ID)WN_pragma(tree);
            if (WN_pragmas[pid].users == PUSER_HICUDA) break;
        }
/*** DAVID CODE END ***/
