...
extern WN* True_Bound(WN* wn_func, ST_IDX st_idx_exp)
{
    ...
    for ( ; wni != NULL; wni = WN_WALK_TreeNext(wni)) {
        wn = wni->wn;
        if (WN_operator(wn) == OPR_STID && WN_st_idx(wn) == st_idx_exp) break;
    }

    if (wni == NULL) return NULL; 

    return LWN_Copy_Tree(WN_kid0(wn));
}
...
//-----------------------------------------------------------------------
// NAME: Exp_To_Linex
// FUNCTION: Convert 'wn_exp' to LINEX form, adding terms to the LINEX
...
extern BOOL Exp_To_Linex(WN *wn_exp, LINEX *lx_exp, MEM_POOL *mem_pool,
        BOOL negate, BOOL Is_LNO, IPA_LNO_READ_FILE *IPA_LNO_File)
{
    if (!Is_Exp_Linexable(wn_exp, Is_LNO)) return FALSE;
    ...
    for (INT i = 0; i <= terms.Lastidx(); i++) {
        TERM* tm = &terms[i];
        if (negate) tm->Set_coeff(-tm->Get_coeff());
        lx_exp->Set_term(tm);
    }
...
//   based on its declaration. Return NULL if the variable is not an array.
// NOTE: This has been changed so that the PROJECTED_REGION created is 
//   "zero-based" (i.e. the lower bound is always 0).
//-----------------------------------------------------------------------
extern PROJECTED_REGION* Projected_Region_From_St(WN *wn_func, ST *st,
        MEM_POOL *mem_pool, BOOL Is_LNO, IPA_LNO_READ_FILE *IPA_LNO_File)
{
    // The formal must be an array or a pointer to an array.
    TY_IDX ty_idx = ST_type(st);
    if (TY_kind(ty_idx) == KIND_POINTER) ty_idx = TY_pointed(ty_idx);
    if (TY_kind(ty_idx) != KIND_ARRAY) return NULL;

    INT dim_count = TY_AR_ndims(ty_idx);
    PROJECTED_REGION *pr = CXX_NEW(
            PROJECTED_REGION(NON_MESSY_REGION, 0, dim_count, mem_pool),
            mem_pool);
    pr->Set_is_formal();
    ...
    for (INT j = 0; j < dim_count; j++)
    {
        ...
        /* The lower bound = 0. */
        pn->Get_lower_linex()->Set_term(LTKIND_CONST, (COEFF)0, CONST_DESC, 0);

        LINEX *lx_ub = pn->Get_upper_linex();

        /* The upper bound = arb_ubnd - arb_lbnd. */
...
/** DAVID CODE BEGIN **/
#if 1
            WN *wn_lb = NULL;
            // This variable must be a formal parameter.
            if (st_idx_lb != ST_IDX_ZERO
                    && ST_sclass(St_Table[st_idx_lb]) == SCLASS_FORMAL)
            {
                // TODO: it must not be modified in the procedure.
                wn_lb = WN_LdidScalar(st_idx_lb);
            }

            if (wn_lb == NULL)
            {
                // Unspecified lower bound: OK for the outer-most dimension.
                FmtAssert(j == 0,
                        ("Process_Array_Formals: "
                         "Expecting assumed shape array"));
                pn->Set_assumed_shape();
            }
            else
            {
                BOOL ok = Exp_To_Linex(wn_lb, lx_ub,
                        mem_pool, TRUE, Is_LNO, IPA_LNO_File);
                if (!ok) pn->Set_messy_ub();
            }
#else
            if (st_idx_lb == ST_IDX_ZERO)
            {
            ...
            else
            {
                ...
                { 
                    BOOL ok = Exp_To_Linex(wn_lb_true, lx_ub,
                            mem_pool, TRUE, Is_LNO, IPA_LNO_File);
                    if (!ok) pn->Set_messy_ub();
                } 
            }
...
/** DAVID CODE BEGIN **/
                ...
                WN *wn_ub = NULL;
                // This variable must be a formal parameter.
                if (st_idx_ub != ST_IDX_ZERO
                        && ST_sclass(St_Table[st_idx_ub]) == SCLASS_FORMAL)
                {
                    // TODO: it must not be modified in the procedure.
                    wn_ub = WN_LdidScalar(st_idx_ub);
                }

                if (wn_ub == NULL)
                {
                    // Unspecified lower bound: OK for the outer-most
                    // dimension.
                    FmtAssert(j == 0,
                            ("Process_Array_Formals: "
                             "Expecting assumed shape array"));
                ...
                {
                    BOOL ok = Exp_To_Linex(wn_ub, lx_ub,
                            mem_pool, TRUE, Is_LNO, IPA_LNO_File);
                    if (!ok) pn->Set_messy_ub();
                }
#else
                ...
                else
                {
                    ...
                    { 
                        BOOL ok = Exp_To_Linex(wn_ub_true, lx_ub,
                                mem_pool, FALSE, Is_LNO, IPA_LNO_File);
                        if (!ok) pn->Set_messy_ub();
                    }
                }
#endif
        ...
        /* The stride is 1. */
        pn->Get_step_linex()->Set_term(LTKIND_CONST, (COEFF)1, CONST_DESC, 0);
    }
