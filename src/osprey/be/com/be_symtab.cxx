...
void BE_symtab_alloc_scope_level(const SYMTAB_IDX level)
{
    ...
    {
        // Allocate the bigger scope array.
        // DAVID COMMENT: why do we increase the size in multiple hops?
        BE_SCOPE *temp = CXX_NEW_ARRAY(BE_SCOPE, 1 + next_level * 2,
                &Be_symtab_pool);

        SYMTAB_IDX i;
        for (i = 0; i < next_level; i++) temp[i] = Be_scope_tab[i];
        next_level = 1 + next_level * 2;
        for (; i < next_level; i++) temp[i].be_st_tab = NULL;
