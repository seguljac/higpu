...
{ 
    ...
    if (lx_upper != NULL && lx_upper->Num_terms() >= 0
            && lx_step != NULL && lx_step->Num_terms() >= 0) return;

    Reset_is_unprojected();

    if (lx_upper != NULL) lx_upper->Free_terms();
    if (lx_step != NULL) lx_step->Free_terms();
...
/*****************************************************************************
 *
 * Copy the terms in <this> to the given LINEX.
 *
 * Discard empty terms that may show up during Set_to_kernel_image merge or
 * subtract operation.
 *
 ****************************************************************************/
...
void LINEX::Copy(LINEX *to)
{
    ...
    INT n_terms = Num_terms();
    for (INT i = 0; i <= n_terms; ++i) {
        TERM *t = Get_term(i);
        if (t->Get_coeff() != 0 || t->Get_type() == LTKIND_CONST) {
            to->Set_term(t);
        }
    }
}
...
/*****************************************************************************
 *
 * Deep-copy this instance to the given PROJECTED_NODE using <pool>.
 * If <pool> is NULL, the mem pool in this instance is used for allocation.
 *
 * The mem pool field in <to> is not changed.
 *
 ****************************************************************************/

/** DAVID CODE BEGIN **/
void PROJECTED_NODE::Copy(PROJECTED_NODE *to, MEM_POOL *pool)
{
    if (this == to) return;

    // If the given mem pool is NULL, we use the one in this instance.
    // Apparently, we do not want to use the one in <to>.
    MEM_POOL *m = (pool == NULL) ? pool : Mem_Pool();
/*** DAVID CODE END ***/
...
/*****************************************************************************
 *
 * Use the given mem pool to create a deep copy of this region.
 *
 ****************************************************************************/

PROJECTED_REGION* PROJECTED_REGION::create_deep_copy(MEM_POOL *pool)
{
    PROJECTED_REGION *result = CXX_NEW(
            PROJECTED_REGION(Get_type(), Get_depth(), Get_num_dims(), pool),
            pool);

    result->Set_projected_kernel(Get_projected_kernel());
    
    for (INT i = 0; i < Get_num_dims(); ++i) {
        Get_projected_node(i)->Copy(result->Get_projected_node(i), pool);
    }
...
BOOL LINEX::Equivalent(LINEX &b)
{
    ...
    if (num_terms != b.Num_terms()) return FALSE;

    for (INT i = 0; i <= num_terms; ++i) {
        if (!Get_term(i)->Equivalent(*b.Get_term(i))) return FALSE;
    }
...
//====================================================================
void PROJECTED_NODE::Create_linex(TERM* term)
{
    MEM_POOL* pool = Mem_Pool();
    ...
    Set_lower_linex((LINEX*)CXX_NEW(LINEX(pool), pool));
    Set_step_linex((LINEX*)CXX_NEW(LINEX(pool), pool));
    Set_segment_length_linex((segment_length_count > 0) ?
            (LINEX*)CXX_NEW(LINEX(pool), pool) : NULL);
    Set_segment_stride_linex((segment_stride_count > 0) ?
            (LINEX*)CXX_NEW(LINEX(pool), pool) : NULL);

    Get_upper_linex()->Set_linex_terms(
            uterm_index, uterm_index+uterm_count, term);
    Get_lower_linex()->Set_linex_terms(
            lterm_index, lterm_index+lterm_count, term);
    Get_step_linex()->Set_linex_terms(
            sterm_index, sterm_index+sterm_count, term);
...
/*****************************************************************************
 *
 * Add a linex to the system of equations. Each equation is set up as:
 *
 *      A*X + B*Y + C*Z <= b
 *
 ...
 * b is a vector of constant offsets (of the size that is the sum of the sizes
 * of X, Y and Z).
 *
 ****************************************************************************/

void LINEX::Add_access(SYSTEM_OF_EQUATIONS *soe,
        mUINT8 depth, INT num_dim, INT axle,
        INT num_syms, ACTION_TYPE act, LOOP_SYMBOL_ARRAY *sym, 
        BOOL trace)
{
    INT pos, c = 0;
     ...
     *      # of dimensions (for coupled subscript values) +
     *      # of loop indices +
     *      # of symbolic constants +
     *      1 (for the constant offset)
     *
     * DAVID COMMENT: if the constant is stored in 'c', why do we need this 1?
     */
    ...
    for (INT i = 0; i <= Num_terms(); ++i)
    {
        ...
        switch (term->Get_type())
        {
            ...
            case LTKIND_LINDEX:
                // set the value of matrix A note.  when we first start out
                // this is all zeros we are setting this term to one in the
                // case of the equality constraint implying that this
                // particular dimension or axle is equal to the subscript
                // expression.
    ...
    // linex <= subscript_i
    if (!ltkind_subscr) v[axle] = (act == ACTION_LO) ? -1 : 1;

    if (act == ACTION_LO) {
        // All we need to do is to move the constant to the RHS.
        c = -c;
    } else {
        // Reverse the coefficients.
        // DAVID COMMENT: why not those for coupled subscripts?
        for (INT i = num_dim; i < vector_size; ++i) v[i] = -v[i];
    }

    if (act != ACTION_EQ) {
        // Add it to the inequality constraint.
        soe->Add_Le(v, c);
    } else {
        // Add it to the equality constraint.
        soe->Add_Eq(v, c);
    }
...
/*****************************************************************************
 *
 * Add the axle 'pos' of REGION 'a' to the SOE 'soe'. Convert one equation
 * into two inequalities if convert_equation is TRUE.
 *
 * DAVID COMMENT: does the total number of vars still meet what is being
 * created outside?
 *
 ****************************************************************************/

extern void Add_to_SOE(PROJECTED_REGION *a, const INT pos,
        SYSTEM_OF_EQUATIONS *soe, const BOOL convert_equation,
        LOOP_SYMBOL_ARRAY *sym, INT depth, BOOL trace)
{
#ifdef IPA_SUMMARY
            ...
            && node->Get_upper_linex()->Num_terms() != -1)
    {
        ...
        node->Get_lower_linex()->Add_access(soe, depth, a->Get_num_dims(),
                pos, num_syms, ACTION_LO, sym, trace);
        node->Get_upper_linex()->Add_access(soe, depth, a->Get_num_dims(),
                pos, num_syms, ACTION_UP, sym, trace);
    }
    else
    {
        if (convert_equation) {
            ...
            node->Get_lower_linex()->Add_access(soe, depth, a->Get_num_dims(),
                    pos, num_syms, ACTION_EQ, sym, trace);
        }
    }
}
...
//===================================================================
void PROJECTED_NODE::Init(MEM_POOL *m)
{
    Set_Mem_Pool(m);
...
/*****************************************************************************
 *
 * Merge the given region into this region. Use a system of equations (SOE) to
 * figure out the union's lower and upper bounds.
 *
 * DAVID COMMENT: the following algorithm is not what this function
 * implements.
 *
...
BOOL PROJECTED_REGION::May_Union(PROJECTED_REGION& b, BOOL trace)
{
    if (trace) {
    ...
    if (Is_messy_region()) return FALSE;
    if (b.Is_messy_region()) {
        Set_messy_region();
        return TRUE;
    }
    ...
    for (INT i = 0; i < Get_num_dims(); ++i)
    {
        ...
        if (ax->Has_all_messy_bounds()) continue;
        ...
        {
            // For non-const steps, be conservative, set unit step.
            if (!lx_a_step->Is_const()
                    || lx_a_step->Get_constant_term() != 1) {
        ...
        else
        {
            ...
            INT step_sz = Gcd(a_coeff, b_coeff); 

            // The two ranges can only be merged if the difference between the
            // lower bounds is a multiple of the new (smaller) step size.
            ...
            if (!lx_diff->Is_const()) {
                ...
                if (diff < 0) diff = -diff;
                if (diff % step_sz != 0) step_sz = 1;
            }

            if (lx_a_step->Get_term(0)->Get_coeff() != step_sz) { 
                lx_a_step->Get_term(0)->Set_coeff(step_sz);
                change = TRUE;
            }
        }

        /* Now, try to merge the two ranges. */

        // Create a system of equations.
        // DAVID COMMENT: not sure exactly how many variables are there.
        SYSTEM_OF_EQUATIONS *soe = CXX_NEW(
                SYSTEM_OF_EQUATIONS(0, 0, b.Get_num_dims() + b.Get_depth(),
                    local_pool), local_pool);
        LOOP_SYMBOL_ARRAY *syms = CXX_NEW(
                LOOP_SYMBOL_ARRAY(local_pool), local_pool);
        ...
        // Get rid of the redundant constraints to find the intersection.
        INT num_con = soe->Num_Le_Constraints();
        BOOL *is_redundant = CXX_NEW_ARRAY(BOOL, num_con, local_pool);

        // The following is the first part of SOE::Elim_Simple_Redundant().
        num_con -= soe->Mark_Simple_Redundant(is_redundant);
        if (num_con > 2) {
            // SOE::Mark_New_Redundant() skips the inequalities that are
            // already marked as redundant in the parameter.
        ...
        LINEX *ax_seg_len = ax->Get_segment_length_linex();
        LINEX *bx_seg_len = bx->Get_segment_length_linex();

        if (!ax->Matching_Segment_Stride(bx)
                || (ax_seg_len != NULL && !ax_seg_len->Is_const())
                || (bx_seg_len != NULL && !bx_seg_len->Is_const()))
        {
            ax_seg_len->Free_terms();
            ax->Get_segment_stride_linex()->Free_terms();
        } 
        else if (ax->Get_segment_stride_linex() != NULL)
        { 
            INT difference = ax_seg_len->Get_constant_term() -
                bx_seg_len->Get_constant_term();
            if (difference < 0) difference = -difference;

            ax_seg_len->Set_term(LTKIND_CONST, difference, CONST_DESC, 0);
            ax_seg_len->Simplify();
            ...
            if (!lx_diff->Is_const()) { 
                ...
                difference = lx_diff->Get_constant_term();
                if (difference < 0) difference = -difference; 
                ax_seg_len->Set_term(LTKIND_CONST, difference, CONST_DESC, 0);
                ax_seg_len->Simplify();
            } 
        ...
        // Replace ax's lower bound with bx's lower bound, if necessary.
        LINEX *lb = ax->Get_lower_linex();
        if (lb != NULL && !lb->Equivalent(*(bx->Get_lower_linex()))) { 
            if (!is_redundant[0]) {
                lb->Free_terms();
        ...
        // Replace ax's upper bound with bx's upper bound, if necessary.
        LINEX* ub = ax->Get_upper_linex();
        if (ub != NULL && !ub->Equivalent(*(bx->Get_upper_linex()))) { 
            if (!is_redundant[1]) {
                ub->Free_terms();
...
//=====================================================================
BOOL TERM::Is_equal(TERM* t, INT count)
{
    for (INT i = 0; i <= count; ++i) {
        if (! t[i].Equivalent(this[i])) return FALSE;
    }
    return TRUE;
}
...
{
    // DAVID: what if there are multiple constant terms?
    for (INT i = 0; i <= Num_terms(); ++i) {
...
PROJECTED_REGION* REGION_ARRAYS::Get_Projected_Region(INT i)
{
    ...
    Is_True(info != NULL && info->Lastidx() != -1,
            ("Expecting at least 1 projected region in Reshape\n"));
    Is_True(info->Lastidx() >= i,
            ("Projected_Region %d exceeded size of array %d\n",
             i, info->Lastidx()));

    PROJECTED_REGION_INFO *region_info =  &(*info)[i];
    PROJECTED_REGION *proj_shape = region_info->Get_projected_region();
    Is_True(proj_shape,
            ("NULL Projected Region in REGION_ARRAYS::Get_Projected_Region\n"));
...
void LINEX::Remove_Zero_Terms()
{
    ...
    // Rescale the array.
    INT difference = i - j;
    for (i = 0; i < difference; i++) _larray.Decidx();

    // If all terms have been eliminated, put one constant term back.
...
{
/** DAVID CODE BEGIN **/
    INT last_idx = Num_terms();
    TERM *tm_i = NULL, *tm_j = NULL;
    COEFF coeff_i, coeff_j;

    for (INT i = 0; i <= last_idx; i++)
    {
        tm_i = Get_term(i);
        // We could skip this term if its coefficient is zero.
        if ((coeff_i = tm_i->Get_coeff()) == 0) continue;

        for (INT j = i + 1; j <= last_idx; j++)
        {
            tm_j = Get_term(j);
            // We could skip this term if its coefficient is zero.
            if ((coeff_j = tm_j->Get_coeff()) == 0) continue;

            if (tm_i->Get_type() == tm_j->Get_type()
                    && tm_i->Get_desc() == tm_j->Get_desc()
                    && tm_i->Get_projected_level() == tm_j->Get_projected_level()) {
                tm_i->Set_coeff(coeff_i + coeff_j);
                tm_j->Set_coeff(0);
            }
        }
    }
