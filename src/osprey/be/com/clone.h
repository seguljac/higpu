...
// Generic hash table for mapping a pair of addresses. Used for mapping the
// address of a TY/ST to that of its copy. 
//
class IPO_ADDR_HASH
{
private:
    ...
    INT hash (INT key) { return (key & 0xff) ^ ((key >> 8) & 0xff); }
...
private:
    ...
    SYMTAB_IDX _orig_level;     // level of current PU in orig scope_table
    SYMTAB_IDX _cloned_level;   // level of cloned PU in cloned scope_table
                                // same as _orig_level for PU cloning

    MEM_POOL *_mem;             // for temp. data structures

    IPO_ADDR_HASH *_hash_maps;  // maps for lookup (used for initos and STs)
...
public:
    ...
    IPO_SYMTAB (SCOPE *orig_scope_tab, SCOPE *cloned_scope_tab, 
            SYMTAB_IDX orig_symtab_idx, SYMTAB_IDX cloned_symtab_idx,
            MEM_POOL *m, BOOL Same_file = TRUE) :
        _orig_scope_tab(orig_scope_tab), _cloned_scope_tab(cloned_scope_tab),
        _mem(m), _same_file(Same_file),
        _orig_level(orig_symtab_idx), _cloned_level(cloned_symtab_idx)
    {
        _cloned_st_last_idx =
            cloned_scope_tab[cloned_symtab_idx].st_tab->Size()-1;
        _cloned_inito_last_idx =
            cloned_scope_tab[cloned_symtab_idx].inito_tab->Size()-1;
        _cloned_label_last_idx =
            cloned_scope_tab[cloned_symtab_idx].label_tab->Size()-1;
        _cloned_preg_last_idx =
            cloned_scope_tab[cloned_symtab_idx].preg_tab->Size()-1;
        _cloned_st_attr_last_idx =
            cloned_scope_tab[cloned_symtab_idx].st_attr_tab->Size()-1;
    ...
    void New_Symtab ();     // Creates "new" cloned_scope_tab (for cloning)
    void Update_Symtab (BOOL);  // Appends to cloned_scope_tab (for inlining)

    void Promote_Statics ();    // move local statics to IPA global symtab

    // Initialized data objects: special case for C++ EH
...
private:
    ...
    // file id of a cloned node in the
...
public:
    ...
    // 1. Used only in function cloning (_same_file is true: also for
    // IPO_SYMTAB) Caller needs to specify scope_tab and symtab_idx of this PU

    IPO_CLONE(WN *pu, SCOPE *scope_tab, SYMTAB_IDX symtab_idx, 
            WN_MAP_TAB* map_tab, MEM_POOL *map_pool, MEM_POOL *m) :
