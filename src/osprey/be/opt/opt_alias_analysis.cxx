...
void OPT_STAB::Generate_mu_and_chi_list(WN *wn, BB_NODE *bb)
{
    if (wn == NULL) return;
    ...
    case OPR_MLOAD:
        ...
        // track list of values referenced here (for RVI)
        if (aux_stab[vp_idx].Aux_id_list() == NULL) Update_aux_id_list(vp_idx);
        break;
    ...
    case OPR_REGION_EXIT:
        // check if it's a region exit, need a mu
 ...
 *
 * This function examines the PU twice. The first time it creates the MU and
 * CHI lists for scalar variables, and also identifies the mapping from
 * ILOAD/ISTORE to virtual variables and summarize the alias behavior of the
 * set of ILOAD/ISTORE into the virtual variable. The second pass inserts the
 * virtual variables into the MU and CHI lists.
 *
 ****************************************************************************/
...
void OPT_STAB::Compute_FFA(RID *const rid)
{
    ...
    FOR_ALL_ELEM (bb, cfg_iter, Init(_cfg)) {
        FOR_ALL_ELEM (wn, stmt_iter, Init(bb->Firststmt(), bb->Laststmt())) {
            ...
            found_ip_alias_class_info |=
                Transfer_alias_class_to_occ_and_aux(rid, wn);
        }
    }
