...
// AUX_ID_LIST_ITER is a forward iterator through AUX_ID_LIST
//
// AUX_STAB_ENTRY is an entry in the optimizer's auxillary symbol table
//
// VER_STAB_ENTRY is an entry in the SSA version table
// VER_STAB_LIST_NODE is a singly linked list of pointers to VER_STAB_ENTRY
//
...
private:
    ...
    /* chain of aux_sym pointing to the same ST. used at OPT_STAB build time
     * to search for the right existing aux ID for a given ST, offset, and
     * size. The head of this chain is found via st_chain_map.
     */
    AUX_ID    st_chain;
     ...
     * statically aliased (by equivalence, union, etc.) st_group is maintained
     * as a circularly linked list with the properties that:
     *
     * 1. The entries in any list cover a contiguous region of address space
     * (no gaps), and
     *
...
public:
  ...
  BOOL     Is_real_var(void) const {
      return (stype == VT_NO_LDA_SCALAR) || (stype == VT_LDA_SCALAR);
  }
