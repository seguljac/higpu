...
void DU_NODE::Print(FILE *fp) const
{
    // print the Def node mapcat and map_id
    fprintf(fp, "<%d:%d> ", OPCODE_mapcat(WN_opcode(_wn)), WN_map_id(_wn));
}

void DEF_LIST::Print(FILE *fp)
{
    if (Incomplete()) fprintf(fp, "(Incomplete) ");
...
void DU_MANAGER::Print_Du(WN *def, FILE *fp)
{
    fprintf(fp, "D-U <%d:%d> -> ",
            OPCODE_mapcat(WN_opcode(def)), WN_map_id(def));

    USE_LIST *use_list = Du_Get_Use(def);
    if (use_list != NULL) use_list->Print(fp);

    fprintf(fp, "def:");
    fdump_wn(fp, def);
}

void DU_MANAGER::Print_Ud(WN *use, FILE *fp)
{
    fprintf(fp, "U-D <%d:%d> -> ",
...
BOOL USE_LIST::Contains(const WN *wn)
{
    USE_LIST_ITER use_lst_iter;
    const DU_NODE *tmp;
    FOR_ALL_NODE( tmp, use_lst_iter, Init(this) ) {
        if (tmp->Wn() == wn) return TRUE;
    }
...
void EMITTER::Connect_cr_wn(CODEREP *cr, WN *wn)
{
    // DAVID COMMENT: <wn> is the defining node for <cr>.
    // set the WN <-> cr relationship (1 to 1 relationship)
    WN_MAP_Set(_wn_to_cr_map, wn, cr);
}

void EMITTER::Connect_sr_wn(STMTREP *sr, WN *wn)
{
...
void EMITTER::Duplicate_sr_cr_connections(WN *old_wn, WN *new_wn)
{
     ...
     *
     * DAVID COMMENT: do we need to check if the two nodes have the same
     * number of children?
     */
    if (opc == OPC_BLOCK)
    {
    ...
    else
    {
        for (INT ikid = 0; ikid < WN_kid_count(old_wn); ikid++) {
            Duplicate_sr_cr_connections(
                    WN_kid(old_wn,ikid), WN_kid(new_wn,ikid));
        }
    }
}
...
//
void DU_MANAGER::Set_alias_mgr(ALIAS_MANAGER *am)
{ 
    _alias_mgr = am; 
}


void DU_MANAGER::Ud_Add_Def(WN *use, WN *def)
{
    // Add a def to the U-D chain
    DEF_LIST *deflst = Ud_Get_Def(use);

    // check for uniqueness
    if (deflst != NULL && deflst->Contains(def)) return;
    ...
    {
        // The accuracy of alias analysis is reduced by the use of virtual
        // variable.
        if (! OPERATOR_is_scalar_iload(WN_operator(use)) &&
...
void DU_MANAGER::Du_Add_Use(WN *def, WN *use)
{
    // Add an use to the D-U chain
    USE_LIST *uselst = Du_Get_Use(def);

    // check for uniqueness
    if (uselst != NULL && uselst->Contains(use)) return;
    ...
    if (Tracing()) Print_Du(def, TFile);
}

void DU_MANAGER::Add_Def_Use(WN *def, WN *use)
{
...
void DU_MANAGER::Create_Def_List(WN *use)
{
...
void DU_MANAGER::Create_Use_List(WN *def)
{
...
void EMITTER::Add_defs_use(DU_MANAGER *du_mgr, STMTREP *defstmt, WN *use)
{
...
/*****************************************************************************
 *
 * Identify the DEFs for one USE, by tracing through the PHI and CHI functions
 * to locate the defining WN statement(s).
 *
 * <wn> is the USE; it corresponds to <cr>.
 *
 ****************************************************************************/

void EMITTER::Compute_use_def_var(DU_MANAGER *du_mgr,
        CODEREP *cr, WN *wn, BB_NODE *wn_bb)
{
    if (cr->Kind() != CK_VAR) return;

    // We don't need special handling here for dedicated PREGs because alias
    // analysis maintains the invariant that they are in the Chi list of every
    // call that may affect them.
    ...
    if (!is_real_var && !Opt_stab()->Unique_vsym(cr->Aux_id()))
    {
        ...
        if (OPERATOR_is_scalar_load(WN_operator(wn))) {
            DEF_LIST *deflist = du_mgr->Ud_Get_Def(wn);
            // Do we need to handle the case where there is no deflist yet?
...
void EMITTER::Compute_use_def_expr(DU_MANAGER *du_mgr, WN *wn, BB_NODE *wn_bb)
{
    ...
    if (cr != NULL && cr->Kind() == CK_IVAR)
    {
/** DAVID CODE BEGIN **/
#if 0
        printf("!!!!!!!!!!!!!!!!! FOUND an IVAR:\n");
        dump_tree(wn);
        cr->Print(TRUE, stdout);
#endif  // HICUDA
/*** DAVID CODE END ***/

        MU_NODE *mnode = cr->Ivar_mu_node();
        if (mnode != NULL && mnode->Is_Valid()) {
            CODEREP *mu_cr = mnode->OPND();

/** DAVID CODE BEGIN **/
#if 0
            printf("!!!!!!!!!!!!!!!! PROCESS MU node for this IVAR:\n");
            mu_cr->Print(TRUE, stdout);
#endif  // HICUDA
/*** DAVID CODE END ***/
...
void EMITTER::Compute_use_def_stmt(DU_MANAGER *du_mgr, WN *wn, BB_NODE *wn_bb)
{
    const OPERATOR opr = WN_operator(wn);

    Is_True(OPERATOR_is_stmt(opr) || OPERATOR_is_scf(opr),
            ("EMITTER::Compute_use_def_stmt: non-stmt: %s",
             OPERATOR_name(opr)));

    // For each associated MU stmt, compute the U-D chain for its operand.
        ...
        FOR_ALL_NODE(mnode, mu_iter, Init(stmt->Mu_list()))
        {
            if (mnode->Is_Valid()) {
                ...
                if (! mu_cr->Is_flag_set(CF_IS_ZERO_VERSION)) {
                    ...
                    // Update the loop information for this variable, if any.
                    // NOTE: we change it from a BB_LOOP structure to a WN*.
    ...
    else if (opr == OPR_REGION)
    {
        ...
        // see PV 457243
/** DAVID CODE BEGIN **/
        if (RID_TYPE_mp(rid) || RID_TYPE_hicuda(rid) || RID_TYPE_eh(rid)
/*** DAVID CODE END ***/
                || RID_TYPE_olimit(rid) || RID_TYPE_pragma(rid)
            ...
            // DAVID COMMENT: we do not need to do this for now.
            if (RID_TYPE_mp(rid)) {
    ...
    {
        // There may or may not be a stmtrep for this wn, but we do it here
        // because it's loop-invariant.
        STMTREP *stmt = (STMTREP*)WN_MAP_Get(_wn_to_cr_map, wn);

        // Handle stores with zero-version chi nodes.
        if (OPERATOR_is_scalar_store(opr)) {
            if (stmt->Has_zver()) du_mgr->Du_Set_Incomplete(wn);
        }
...
void EMITTER::Compute_use_def_zero_ver(DU_MANAGER *du_mgr)
{
...
static void Clear_phi_counts(CFG *cfg)
{
    CFG_ITER cfg_iter(cfg);
    BB_NODE *bb;
    FOR_ALL_NODE(bb, cfg_iter, Init()) {
...
void EMITTER::Compute_use_def(DU_MANAGER *du_mgr)
{
    ...
    if (du_mgr->Opt_phase() == PREOPT_LNO_PHASE
            && !Has_do_loop() && !PU_mp_needs_lno(Get_Current_PU())) {
        gen_du = FALSE;
        collect_ipa = FALSE;
    }
    ...
    if (collect_ipa) Collect_IPA_summary(du_mgr, du_mgr->Entry_Wn());
...
extern BOOL Du_Built(DU_MANAGER *du)
{
    return du->Du_built();
}
...
BOOL DU_MANAGER::Verify_add_wn_to_map(WN *wn, WN_MAP *wn_in_tree_map) const
{
    BOOL verified = TRUE;

#ifdef Is_True_On
    if (WOPT_Enable_Verify <= 0) return TRUE;
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
        for (WN *stmt = WN_first(wn); stmt != NULL; stmt = WN_next(stmt)) {
            if (! Verify_add_wn_to_map(stmt, wn_in_tree_map)) verified = FALSE;
        }
    } else {
...
BOOL DU_MANAGER::Verify_wn_in_tree(void) const
{
    BOOL verified = TRUE;
    ...
    if (Entry_Wn() == NULL) return verified;
    ...
    if (! Verify_add_wn_to_map(Entry_Wn(), &wn_in_tree_map)) verified = FALSE;

    // Check all of the DU-chains to see if the nodes they say are in the tree
    // are in the tree.
...
BOOL DU_MANAGER::Verify_scalar_usage(WN *wn) const
{
    BOOL verified = TRUE;
        ...
        if (ud_list == NULL) {
            ...
            if (Tracing()) {
                fprintf(TFile, "Verify_scalar_usage: no def for WN:<%d:%d>:",
                        OPCODE_mapcat(opc), WN_map_id(wn));
                fdump_wn(TFile, wn);
            }
        } else {
            WN *loop = ud_list->Loop_stmt();
            if (loop != NULL && WN_opcode(loop) != OPC_DO_LOOP) {
                verified = FALSE;
                if (Tracing()) {
                    fprintf(TFile, "Verify_scalar_usage: "
                            "non-loop node for WN:<%d:%d>\n",
                            OPCODE_mapcat(opc), WN_map_id(wn));
...
stid_visit_kids:
    ...
    {
        for (WN *stmt = WN_first(wn); stmt != NULL; stmt = WN_next(stmt)) {
            if (! Verify_scalar_usage(stmt)) verified = FALSE;
        }
    }
    break;
    ...
    {
        for (INT kidnum = 0; kidnum < WN_kid_count(wn); kidnum++) {
            if (! Verify_scalar_usage(WN_kid(wn,kidnum))) verified = FALSE;
        }
    }
    break;
    }
...
BOOL DU_MANAGER::Verify(void)
{
    ...
    if (Tracing()) fprintf(TFile, "DU_MANAGER::Verify\n");

    // Verify that all WHIRLs seen in DU_CHAINs are in the tree.
    if (! Verify_wn_in_tree()) verified = FALSE;

/** DAVID CODE BEGIN **/
    if (Tracing()) {
        fprintf(TFile, "DU_MANAGER::Verify_wn_in_tree: %s\n",
                (verified ? "TRUE" : "FALSE"));
    }
/*** DAVID CODE END ***/

    // Verify that all LDIDs have defs.
    if (! Verify_scalar_usage(Entry_Wn())) verified = FALSE;
