...
BOOL CODEREP::Match(CODEREP* cr, INT32 mu_vsym_depth, OPT_STAB *sym)
{
    ...
    if (Is_flag_set(CF_OWNED_BY_TEMP)) return FALSE;
    ...
    switch (Kind())
    {
        ...
        case CK_IVAR:
        {
            ...
            if (Get_mtype_class(OPCODE_rtype(ivar_opc)) != 
                    Get_mtype_class(OPCODE_rtype(cr_opc))) return FALSE;

            // no CSE for PREFETCH op
            if (ivar_opr == OPR_PREFETCH) return FALSE;

            if (ivar_opr == OPR_PARM && cr_opr == OPR_PARM)
            {
                // types must match
                if (ivar_opc != cr_opc) return FALSE;
                // flag must match
                if (Offset() != cr->Offset()) return FALSE;

                // the actual parameter is stored in the base
                if (Ilod_base() != cr->Ilod_base()) return FALSE;

                // the TY_IDX has to be different
                if (Ilod_ty() != cr->Ilod_ty()) return FALSE;
                ...
                return (cr->Ivar_mu_node() || Ivar_mu_node()) ? FALSE : TRUE;
            }

            if (cr->Is_ivar_volatile()) return FALSE;

            if (ivar_opr == OPR_MLOAD) {
                if (cr->Mload_size() != Mload_size()) return FALSE;
            }
...
void CODEREP::Print(INT indent, FILE *fp) const
{
    ...
    if (Kind() == CK_OP)
    {
        for (i = 0; i < Kid_count(); i++) Get_opnd(i)->Print(indent+1, fp);
    }
    else if (Kind() == CK_IVAR)
    {
        if (Ilod_base() != NULL)
...
void CODEREP::Print_node(INT32 indent, FILE *fp) const
{
    INT i;
    char buf[64];
    ...
    case CK_VAR:
        fprintf(fp, ">");	// mark line visually as htable dump
        for (i = 0; i < indent; i++) fprintf(fp, " ");
        fprintf(fp, Bit_field_valid() ? "LDBITS" : "LDID");
        fprintf(fp, " %s %s sym%dv%d %d ty=%x ",
                MTYPE_name(Dsctyp()), MTYPE_name(Dtyp()),
                Aux_id(), Version(), Offset(), Lod_ty());
        break;
    ...
    {
    case CK_VAR:
        if (Is_var_nodef()) fprintf(fp, " (no-def)");
        if (Is_var_volatile()) fprintf(fp, " (vol)");
        break;
    case CK_IVAR:
...
void CODEREP_CONTAINER::Add_it(CODEREP *cr, const mUINT16 idx,
        CODEMAP *htable)
{
    // Replicate cr and prepend it at the beginning.
    SLIST::Prepend(cr);
    cr->Set_coderep_id(htable->Next_coderep_id());

    // Update the bucket pointer in the hash table.
    htable->Set_hash_bucket(idx, Head());
}

void CODEREP_CONTAINER::Delete_it(CODEREP *cr, const mUINT16 idx,
        CODEMAP *htable)
{
    ...
    if (Head() == cr) {
        ...
        FOR_ALL_NODE(prev, cr_iter, Init(this->Head())) {
            if (prev->Next() == cr) cr->Remove(prev);
        }
    }
}
...
/*****************************************************************************
 *
 * Test if this container contains the given <cr>.
 *
 ****************************************************************************/

BOOL CODEREP_CONTAINER::Contains(CODEREP *cr)
{
    CODEREP *tmp;
    CODEREP_ITER cr_iter;
    FOR_ALL_NODE(tmp, cr_iter, Init(this->Head())) {
        if (tmp->Match(cr)) return TRUE;
    }
    return FALSE;
}
...
/*****************************************************************************
 *
 * Return the CODEREP if it exists in the container, and NULL otherwise.
 *
 * While walking through the nodes, this function also establishes the <len>
 * and the <_tail> fields.
 *
 ****************************************************************************/

CODEREP* CODEREP_CONTAINER::Find_cr(CODEREP *cr,
        INT32 mu_vsym_depth, OPT_STAB *sym)
{
    ...
    FOR_ALL_NODE(tmp, cr_iter, Init(this)) {
        Set_Tail(tmp);
        if (tmp->Match(cr, mu_vsym_depth, sym)) return tmp;
        counter++;
    }
...
CODEMAP::CODEMAP(mUINT32 hash_size, CFG *cfg, OPT_STAB *asym, SSA *ssa, 
        IDX_32 phi_hash_size, OPT_PHASE phase, MEM_POOL *pool) :
...
CODEMAP::~CODEMAP(void)
{
    ...
    // We do not need to free the hash table here as we want to pop the
    // Opt_default_pool at the end of optimization phase.
...
CODEREP* CODEMAP::Find_or_append_CR(IDX_32 hash_idx, CODEREP *cr,
        INT32 mu_vsym_depth)
{
...
CODEREP* CODEMAP::Append_CR(IDX_32 hash_idx, CODEREP *cr)
{
    CODEREP_CONTAINER cr_list;
    cr_list.Init_Head(Hash_bucket(hash_idx));

    CODEREP *retv = CXX_NEW_VARIANT(CODEREP(*cr),
            cr->Extra_space_used(), mem_pool);
    retv->Set_usecnt(0);
...
CODEREP* CODEMAP::Find_CR(IDX_32 hash_idx, CODEREP *cr,
        INT32 mu_vsym_depth, BOOL ok_to_fail)
{
    CODEREP_CONTAINER cr_list;
    cr_list.Init_Head(Hash_bucket(hash_idx));
    ...
    if (!ok_to_fail) FmtAssert(FALSE, ("CODEREP::Find_CR, CR was not found"));
...
CODEREP* CODEMAP::Hash_Ivar(CODEREP *cr, INT32 mu_vsym_depth)
{ 
    Is_True(cr->Kind() == CK_IVAR, ("CODEMAP::Hash_Ivar, wrong kind"));
    Is_True(cr->Ilod_base() || cr->Istr_base(),
            ("CODEMAP::Hash_Ivar, no base"));
...
// Print STMTREP tree
//
void STMTREP::Print(FILE *fp) const
{
    if (_opr == OPR_PRAGMA) {
        fprintf(fp, ">");	// mark line visually as htable dump
        ...
        if (Mu_list()) {
            FOR_ALL_NODE(mnode, mu_iter, Init(Mu_list())) 
                if (mnode->OPND())  // Check if the MU has been deleted.
                    fprintf(fp, "%d/cr%d ", mnode->Aux_id(),
                            mnode->OPND()->Coderep_id());
        }
        fprintf(fp, ">\n");
    }

    if (Rhs()) Rhs()->Print(1, fp);
    if (OPERATOR_is_scalar_istore(_opr)
            || _opr == OPR_ISTOREX || _opr == OPR_MSTORE) {
        Lhs()->Istr_base()->Print(1, fp);
    }
    if (_opr == OPR_MSTORE) Lhs()->Mstore_size()->Print(1, fp);
    if (_opr == OPR_ISTOREX) Lhs()->Index()->Print(1, fp);

    Print_node(fp);

    if (OPERATOR_is_call(_opr)) fprintf(fp, " call-flag:0x%x", Call_flags());
    fprintf(fp, " b=%s",Print_bit());
        ...
        FOR_ALL_NODE(cnode, chi_iter, Init(Chi_list())) {
            ...
            if (cnode->Live())  // Check if the MU has been deleted.
                fprintf(fp, "/cr%d/cr%d",
                        cnode->RESULT()->Coderep_id(),
                        cnode->OPND()->Coderep_id()); 
            fprintf(fp, " ");
        }
        fprintf(fp, "> %p\n", this);
    }
...
//
void STMTREP::Print_node(FILE *fp) const
{
    fprintf(fp, ">");	// mark line visually as htable dump
    ...
    if (Black_box()) fprintf(fp, " (black-box)");
}
...
// Print out the CR
void CODEMAP::Print_CR(CODEREP *cr, FILE *fp) const
{
    INT32 i;

    fprintf(fp, "%ccr%d b=%s st_name=%s ",
            ((cr->Usecnt() <= 0) ? '*' : ' '),
            cr->Coderep_id(), cr->Print_bit(),
            Sym()->Aux_stab_entry(cr->Aux_id())->St_name());
    ...
    else if (cr->Kind() == CK_IVAR)
    {
        ...
        // Check if the MU has been deleted.
        if (mnode != NULL && mnode->OPND() != NULL) {
            fprintf(fp, " mu<");
            fprintf(fp, "%d/cr%d", mnode->Aux_id(),
                    mnode->OPND()->Coderep_id());
            fprintf(fp, ">");
...
void CODEMAP::Print_SR(STMTREP *stmt, FILE *fp) const
{
    ...
    if (stmt->Has_mu())
    {
        ...
        if (stmt->Mu_list()) {
            FOR_ALL_NODE(mnode, mu_iter, Init(stmt->Mu_list())) {
                ...
                if (mnode->OPND()) {
                    fprintf(fp, "%d/cr%d ",
                            mnode->Aux_id(), mnode->OPND()->Coderep_id());
                }
            }
        }
        fprintf(fp, ">\n");
...
void CODEMAP::Print(FILE *fp) const
{
    CODEREP_ITER cr_iter;
    CODEREP *cr, *bucket;
    ...
    FOR_ALL_NODE(i, aux_stab_iter, Init())
    {
        ...
        FOR_ALL_NODE(cr, cr_iter, Init(aux->Cr_list()))
        {
            ...
            Is_True(cr->Kind() == CK_VAR,
                    ("CODEMAP::Print: coderep in aux_stab not CK_VAR"));
