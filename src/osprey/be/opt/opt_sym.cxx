...
char* AUX_STAB_ENTRY::St_name(void)
{
    if (st == NULL) return "null";

    if (ST_class(st) == CLASS_CONST) return Targ_Print(NULL, STC_val(st));
...
MU_NODE* MU_LIST::Search_mu_node(AUX_ID var)
{
    MU_LIST_ITER mu_iter;
    MU_NODE *munode;
    FOR_ALL_NODE(munode, mu_iter, Init(this)) {
        if (munode->Aux_id() == var) return munode;
    }
...
void MU_NODE::Set_OPND(CODEREP *cr, BOOL set_dont_prop)
{
    _u._cr = cr; 
    if (cr != NULL && set_dont_prop) cr->Set_flag(CF_DONT_PROP);
}


//  Set the operand of the CHI_NODE
void CHI_NODE::Set_OPND(CODEREP *cr, BOOL set_dont_prop)
{
    _opnd._cr = cr;
    if (cr != NULL && set_dont_prop) cr->Set_flag(CF_DONT_PROP);
}
