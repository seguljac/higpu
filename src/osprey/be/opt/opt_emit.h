...
private:
    ...
    void Compute_use_def_zero_version_var(DU_MANAGER *,
            CODEREP *, WN *, BB_NODE *, BB_NODE *);
...
public:
    ...
    EMITTER(MEM_POOL *lpool, MEM_POOL *gpool, OPT_PHASE opt_phase);
    ~EMITTER(void);
    ...
    BOOL      For_preopt(void) const { return _opt_phase != MAINOPT_PHASE; }
    BOOL      For_mainopt(void) const { return _opt_phase == MAINOPT_PHASE; }
    ...
    BOOL Verify(WN *wn);
    WN* Emit(COMP_UNIT *cu, DU_MANAGER *du_mgr, ALIAS_MANAGER *alias_mgr);
