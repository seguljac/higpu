...
/** DAVID CODE BEGIN **/
extern "C" void Perform_Procedure_Summary_Phase (WN* w,
        DU_MANAGER *du_mgr, ALIAS_MANAGER *alias_mgr,
        EMITTER *emitter, IPA_NODE *proc_node, WN_MAP parent_kernel_map);
#ifdef __linux__
...
private:
    ...
    BOOL  _compare_simp;
/** DAVID CODE BEGIN **/
    BOOL  _copy_propagate;
    BOOL  _const_prop_only;
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
            case PREOPT_PRE_PP_PHASE:
#ifdef HICUDA
            case PREOPT_IPA0_PHASE:
#endif
            case PREOPT_IPA_PRE_PP_PHASE:
            case PREOPT_IPA_POST_PP_PHASE:
            case PREOPT_IPA_KERNEL_DAS0_PHASE:
            case PREOPT_IPA_KERNEL_DAS1_PHASE:
            case PREOPT_IPA2_PHASE:
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
                if (_phase != PREOPT_DUONLY_PHASE)
                {
                    // For all hiCUDA-related phases, we do not need any
                    // optimizations, except the following.
                    WOPT_Enable_Add_Do_Loop_Info = TRUE;
                    // They must be present for DO_LOOPs to be emitted.
                    WOPT_Enable_DCE = TRUE;
                    WOPT_Enable_Aggressive_dce = TRUE;

                    // This must be disabled; otherwise the visibility of GPU
                    // variables and their shapes may be messed up.
                    // For example, foo_call(a) where <a> is assigned an
                    // expression, the shape directive for <a> and the call
                    // may both be propagated the expression of <a>, causing
                    // many troubles in the hiCUDA handler.
                    WOPT_Enable_Copy_Propagate = FALSE;
                }
                if (_phase == PREOPT_PRE_PP_PHASE)
                {
                    WOPT_Enable_Copy_Propagate = TRUE;
                    // Enable this in Do_copy_propagate instead.
                    // WOPT_Enable_Const_Propagate_Only = TRUE;
                }
                if (_phase == PREOPT_IPA2_PHASE)
                {
                    // WOPT_Enable_Aggressive_Code_Motion =
                    // WOPT_Enable_Fold2const =
                    // WOPT_Enable_Copy_Propagate = TRUE;
...
/** DAVID CODE BEGIN **/
                WOPT_Enable_Const_Propagate_Only = _const_prop_only;
/*** DAVID CODE END ***/
...
public:
    ...
    WOPT_SWITCHES(OPT_PHASE phase, INT32 pragma_flags,
            BOOL disable_parm_alias)
    {
        ...
        if (disable_parm_alias) Alias_Pointer_Parms = FALSE;
...
/** DAVID CODE BEGIN **/
        _const_prop_only = WOPT_Enable_Const_Propagate_Only;
/*** DAVID CODE END ***/
        ...
        // For running epre early
        _epre_before_ivr = WOPT_Enable_Epre_Before_Ivr;
        // For running lpre early
        _lpre_before_ivr = WOPT_Enable_Lpre_Before_Ivr;
        // For running spre early
        _spre_before_ivr = WOPT_Enable_Spre_Before_Ivr;
        // For running bdce early
        _bdce_before_ivr = WOPT_Enable_Bdce_Before_Ivr;
...
WN* Pre_Optimizer(INT32 phase, WN *wn_tree,
        DU_MANAGER *du_mgr, ALIAS_MANAGER *alias_mgr, IPA_NODE *proc_node)
{
    ...
    if (Get_Trace(TP_GLOBOPT, -1)) {
        fprintf(TFile,
                "%s\tPre_Optimizer phase=%d\n%s\n", DBar, phase, DBar);
    }

/** DAVID CODE BEGIN **/
    Is_True(phase == PREOPT_IPA0_PHASE || phase == PREOPT_IPA1_PHASE
            || phase == PREOPT_LNO_PHASE
            || phase == PREOPT_DUONLY_PHASE
            || phase == PREOPT_PRE_PP_PHASE
            || phase == PREOPT_IPA_PRE_PP_PHASE
            || phase == PREOPT_IPA_POST_PP_PHASE
            || phase == PREOPT_IPA_KERNEL_DAS0_PHASE
            || phase == PREOPT_IPA_KERNEL_DAS1_PHASE
            || phase == PREOPT_IPA2_PHASE
            || phase == PREOPT_PHASE || phase == MAINOPT_PHASE,
            ("Unknown optimizer phase."));
/*** DAVID CODE END ***/

    Is_True(WN_opcode(wn_orig) == OPC_FUNC_ENTRY
            || WN_opcode(wn_orig) == OPC_REGION,
    ...
    // Enable frequency display for ASCII WHIRL dumps.
    ...
    else
    {
        ...
        if (lower_flags != LOWER_NULL) {
            ...
            if (Cur_PU_Feedback) Cur_PU_Feedback->Reset_Root_WN(wn_tree);
        }
    ...
    // DAVID COMMENT: what are htable and itable?
    ...
    /* Verify that the input tree is a DAG. */
    ...
    REGION_LEVEL rgn_level = (lower_fully) ?
        RL_MAINOPT : RID_preopt_level(phase);
...
/*** DAVID CODE END ***/
    ...
    ALIAS_CLASSIFICATION ac(comp_unit->Opt_stab(),
            AC_DESTINATION_OPT_STAB, &alias_class_pool);
    ...
    comp_unit->Cfg()->Compute_dom_tree(TRUE);
    // create post-dominator tree
    comp_unit->Cfg()->Compute_dom_tree(FALSE);
    comp_unit->Cfg()->Remove_fake_entryexit_arcs();
    // create dominance frontier
    comp_unit->Cfg()->Compute_dom_frontier();
    // create control-dependence set
    comp_unit->Cfg()->Compute_control_dependence();
    ...
    comp_unit->Ssa()->Construct(comp_unit->Htable(),
            comp_unit->Cfg(), comp_unit->Opt_stab());
...
/** DAVID CODE BEGIN **/
    // comp_unit->Cfg()->Print(stderr);
    // comp_unit->Opt_stab()->Print(stdout);
/*** DAVID CODE END ***/
    ...
    if (WOPT_Enable_DCE) {
        ...
        comp_unit->Do_dead_code_elim(TRUE, TRUE, TRUE, TRUE,
                WOPT_Enable_Copy_Propagate, dce_renumber_pregs, NULL);
...
/** DAVID CODE BEGIN **/
    // Remove assuming GOTOs in the BB_DOSTEP nodes so that DO_LOOPs can be
    // emitted properly.
    if (phase == PREOPT_IPA_KERNEL_DAS0_PHASE
            || phase == PREOPT_IPA_KERNEL_DAS1_PHASE) {
        comp_unit->Cfg()->fix_bb_step();
    }
/*** DAVID CODE END ***/
    ...
    {
#ifdef TARG_IA64
        if (WHIRL_Mtype_B_On) comp_unit->Introduce_mtype_bool();
#endif
        ...
        if (WOPT_Enable_SSA_PRE)
        {
...
#ifdef Is_True_On  
                SET_OPT_PHASE("Verify CODEMAP");
                Is_True(comp_unit->Verify_IR(
                            comp_unit->Cfg(),comp_unit->Htable(),5),
                        ("Verify CFG wrong after dce"));
                if (Get_Trace(TKIND_INFO, TINFO_TIME)) 
                    SET_OPT_PHASE("Skip verify CODEMAP because timing trace is on");
                else
            ...
            if (WOPT_Enable_DCE)
            {
                ...
                comp_unit->Do_dead_code_elim(
                        FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, NULL);

                if (comp_unit->Cfg()->Feedback()) {
                    comp_unit->Cfg()->Feedback()->Verify(comp_unit->Cfg(),
                            "after Dead Code Elimination 2");
                }
            }
    ...
    { 
        ...
        if (Cur_PU_Feedback) Cur_PU_Feedback->Reset_Root_WN(opt_wn);

/** DAVID CODE BEGIN **/
        WN_MAP parent_kernel_map = WN_MAP_UNDEFINED;
        if (phase == PREOPT_IPA_KERNEL_DAS0_PHASE
                || phase == PREOPT_IPA_KERNEL_DAS1_PHASE)
        {
            // Pre-process the generated WHIRL tree.
            parent_kernel_map = WN_MAP32_Create(&Opt_local_pool);
            HC_kernel_preprocess(opt_wn, parent_kernel_map);
        }
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
            ...
            {
                // Print out the DU-chains.
                fprintf(TFile,
                        "\n%s\tPre_Optimizer (Run_ipl)\n%s\n", DBar, DBar);
                du_mgr->Print_Du_Info(TFile);
            }
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
            Perform_Procedure_Summary_Phase(opt_wn, du_mgr, alias_mgr,
                    comp_unit->Emitter(), proc_node, parent_kernel_map);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
        if (phase == PREOPT_IPA_KERNEL_DAS0_PHASE
                || phase == PREOPT_IPA_KERNEL_DAS1_PHASE)
        {
            // Remove the parent kernel map.
            WN_MAP_Delete(parent_kernel_map);
        }
/*** DAVID CODE END ***/

        // If we are running preopt for LNO (usually this will be -O3) and we
        // aren't going to run IPA, we re-run alias classification now that
        // we've had the opportunity to renumber PREGs. We don't re-run alias
        // classification here in the IPA case because renumbering will have
        // taken place in IPL's preopt so we (probably?) don't benefit (much?)
        // from redoing it. This second pass of alias classification will
        // (hopefully) let LNO see better (and sometimes far better) alias
        // information, especially for malloc()'ed arrays and other constructs
        // that whose alias behavior was hazy because of the front end's reuse
        // of PREGs.
