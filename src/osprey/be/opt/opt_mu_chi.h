...
public:
    ...
    MU_NODE(void) {}
    MU_NODE(const MU_NODE& mn) {
        Set_aux_id(mn._aux_id);
        _u = mn._u;
    }
    ...
    /* DAVID COMMENT: how is it different from the copy constructor? */
    void Clone(MU_NODE *mu) {
        Set_aux_id(mu->Aux_id());
        Set_OPND(mu->OPND());
    }
...
public:
    ...
    MU_NODE *New_mu_node_no_dups(AUX_ID var, MEM_POOL *pool) {
        if (MU_NODE *munode = Search_mu_node(var)) return munode;
...
inline void MU_LIST::Clone_mu_list(MU_LIST *mu_list, MEM_POOL *pool)
{
...
private:
    ...
    /* Overload _aux_id for other flags, watch out for little endianess
     * porting.
     *
     * DAVID COMMENT: the endianness should not matter.
     */
    struct {
        UINT32 _live:1;
...
inline void CHI_LIST::Clone_chi_list(CHI_LIST *chi_list, MEM_POOL *pool) 
{
