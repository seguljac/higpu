...
void CFG::Add_one_region(WN *wn, END_BLOCK *ends_bb)
{
    ...
    // Is this a region we should be handling?  Or is it a black box? If it's
    // a black-box, we just leave it as a "region" op and don't lower it
    // further.
    if (RID_level(rid) >= Rgn_level())
    {
        ...
        // NOTE: we don't add buffer blocks before or after black box regions.
        // For exits from a black box, we can't change the label inside to
        // break a critical edge so to prevent SSAPRE from inserting into the
        // region we link it up with the fake exit. (PV475285)
        ...
        if (ends_bb) *ends_bb = END_FALLTHRU;
...
{
    if (!WOPT_Enable_Verify) return TRUE;
...
//
BOOL CFG::Verify_wn(WN *wn, WN *parent, WN_MAP wn_map)
{
    if (!WOPT_Enable_Verify) return TRUE;
    ...
    if (WN_opcode(wn) == OPC_BLOCK)
    {
        for (WN *stmt = WN_first(wn); stmt != NULL; stmt = WN_next(stmt)) {
            if (! Verify_wn(stmt, wn, wn_map)) return FALSE;
        }
    }
    else
    {
        for (INT32 i = 0; i < WN_kid_count(wn); i++) {
            if (! Verify_wn(WN_kid(wn,i), wn, wn_map)) return FALSE;
        }
    }
...
/*****************************************************************************
 *
 * For each BB_DOSTEP node, removes its assuming-goto. This is a hack because
 * we do not want to run DCE to achieve this (as DCE might alter other
 * things).
 *
 ****************************************************************************/
    
void CFG::fix_bb_step()
{
    CFG_ITER cfg_iter(this);
    BB_NODE *bb;
    ...
    FOR_ALL_NODE(bb, cfg_iter, Init())
    {
        if (bb->Kind() != BB_DOSTEP) continue;

        // Look for such a block that contains exactly two statements:
        // one is the increment, the other is the GOTO assumed in the emitter
        STMT_LIST* step_stmts = bb->Stmtlist();
        if (step_stmts->Len() != 2) continue;

        STMTREP *first_stmt = step_stmts->Head();
        STMTREP *last_stmt = step_stmts->Tail();
        if (first_stmt != NULL && first_stmt->Opr() == OPR_STID
                && first_stmt->Rhs()->Kind() == CK_OP
                && last_stmt != NULL && last_stmt->Opr() == OPR_GOTO) {
            bb->Remove_stmtrep(last_stmt);
        }
    }
}
