...
private:
    ...
    UINT32    usecnt:13;    // number of times this node's expression appears
...
#define ILLEGAL_BB 0xfffff	     // initialization value for emit_bb
    ...
    union {
        ...
        } isivar;   // for code kind IVAR (ILOD)
    }u2;
...
public:
    ...
    // memory deallocation by pop mempool
    ~CODEREP_CONTAINER(void) {}

    void Init(CODEREP *h) { SLIST::Set_Head(h); len = 0; }
    void Clear(void) { SLIST::Clear(); len = 0; }

    // add cr to the container
    void Add_it(CODEREP *cr, const mUINT16 idx, CODEMAP *htable);
    // delete coderep from bucket
    void Delete_it(CODEREP *cr, const mUINT16 idx, CODEMAP *htable);

    // test if coderep_container contains cr
    BOOL Contains(CODEREP *cr);

    // return the coderep if it exists in MAP, return NULL otherwise.
    CODEREP *Find_cr(CODEREP *cr, INT32 mu_vsym_depth, OPT_STAB *sym);
};
...
private:
    ...
    OPT_STAB   *sym;
    CFG        *_cfg;
    mUINT32     size;               // hash table size (size of hash_vec)
    ...
    // hash functions
    IDX_32 Hash_lda(ST *st, IDTYPE ofst) {
        return (ST_st_idx(st) + ofst) % size;
    }
    ...
    IDX_32 Hash_rconst(ST *st, IDTYPE ofst) {
        return (ST_st_idx(st) + ofst) % size;
    }
    ...
    IDX_32 Hash(CODEREP *cr);

    /* Search in the hash bucket with the given index for <cr>. Create it if
     * not found.
     */
    CODEREP* Find_or_append_CR(IDX_32 hash_idx, CODEREP *cr,
            INT32 mu_vsym_depth);
    ...
    CODEREP* Find_CR(IDX_32 hash_idx, CODEREP *cr,
            INT32 mu_vsym_depth = 0, BOOL ok_to_fail = FALSE);

    /* Get the hash bucket for the given hash index, assuming it is within the
     * bound.
     */
    CODEREP* Hash_bucket(IDX_32 idx) const { return hash_vec[idx]; }
...
public:
    ...
    void Set_hash_bucket(IDX_32 idx, CODEREP *l) { hash_vec[idx] = l; }
...
private:
    ...
    // The LHS of an assignment stmt
    CODEREP   *_lhs;
    // TODO: define the alias variable list in case of pointer dereference

    // The RHS of an assignment stmt or the expression in an OPR_TRUEBR
    CODEREP   *_rhs;
    ...
    /* This union is controlled by the SRF_USELIST flag. If it's set, use the
     * _use_list value, else use the _wn value. Should only be used during
     * emit time.
     */
    union {
...
public:
    ...
    // enter rhs of the stmt to HASH
    void      Enter_rhs(CODEMAP*, OPT_STAB*, COPYPROP*, EXC*);
    // enter lhs of the stmt to HASH
