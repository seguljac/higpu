...
public:
    ...
    void Do_bitwise_dce(BOOL copy_propagate); // bitwise dead code elimination
    void Do_code_motion(void);        // Take PRE approach
    void Do_copy_propagate(void);     // copy propagation for whole PU
    void Do_dead_code_elim(BOOL do_unreachable, BOOL do_dce_global,
            BOOL do_dce_alias, BOOL do_agg_dce, BOOL do_identity_removal,
            BOOL do_preg_renumbering,
            BOOL *paths_removed);     // dead-code elim for whole PU
    ...
    void Do_load_pre(BOOL do_consts, BOOL do_loads);  // LOAD PRE based on SSA
    ...
    // find uninitialized local vars for an entry to the PU
    void Find_uninit_locals_for_entry(BB_NODE*);
#endif
