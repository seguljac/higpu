...
//  Exported type:
//
//   DU_NODE:      One node in D-U chain or U-D chain. It points to
...
//   DU_NODE, DEF_LIST, DEF_LIST_ITER, USE_NODE, USE_LIST, and USE_LIST_ITER.
//
//   DU_NODE::DU_NODE
//   DU_NODE::~DU_NODE
...
private:
    ...
    WN *_wn;    // the WN node that defines/uses this memory content
...
public:
    ...
    // Loop_stmt for LNO: According to Dror and Ding-Kai, the loop statement
    // for a use is defined as the outermost DO_LOOP containing the use and
    // whose entry is dominated by a definition reaching this use.
    ...
    /* Does the list contain the given def WN? */
    BOOL Contains(const WN *wn);

    // flag access
    BOOL Incomplete(void) const	{ return (_flags & DL_INCOMPLETE) != 0; }
    void Set_Incomplete(void) { if (this) _flags |= DL_INCOMPLETE; }
    void Reset_Incomplete(void) { _flags &= ~DL_INCOMPLETE; }
};
...
public:
    ...
    USE_LIST(DU_NODE *du, UINT flags) { _flags = flags; SLIST::Init(du); }
    ~USE_LIST(void) { _flags = UL_NONE; }

    void Init(void) { _flags = UL_NONE; }

    /* Does the list contain the given use WN? */
    BOOL Contains(const WN *wn);

    // flag access
    BOOL Incomplete( void ) const { return (_flags&UL_INCOMPLETE) != 0; }
    void Set_Incomplete( void ) { _flags |= UL_INCOMPLETE; }
    void Reset_Incomplete( void ) { _flags &= ~UL_INCOMPLETE; }
...
private:
    ...
    WN_MAP         _du_map;     // <def node, USE_LIST>
    WN_MAP         _ud_map;     // <use node, DEF_LIST>

    // global variable whose value is restored
    WN_MAP         _val_restored_map;
    WN_MAP         _bb_map;
...
public:
    ...
    void      Print_Ud(WN *use, FILE *fp=stderr); // print one U-D list
    void      Print_Du(WN *def, FILE *fp=stderr); // print one D-U list
    ...
    void Ud_Put_Def(WN *use, DEF_LIST *deflst) const {
        WN_MAP_Set(_ud_map, use, deflst);
    }
    void Du_Put_Use(WN *def, USE_LIST *uselst) const {
        WN_MAP_Set(_du_map, def, uselst);
    }
    DEF_LIST *Ud_Get_Def(WN *use) const {
        return (DEF_LIST *)WN_MAP_Get(_ud_map,use);
    }
    USE_LIST *Du_Get_Use(WN *def) const {
        return (USE_LIST *)WN_MAP_Get(_du_map,def);
    }
    ...
    BOOL Is_value_restored(WN *def) {
        return (_val_restored_map != WN_MAP_UNDEFINED)
...
#ifdef Is_True_On
    void Check_bb_id(IDTYPE bb_id) {
        FmtAssert( bb_id < _bb_cnt, ("DU_MANAGER: invalid BB id"));
    }
#else
