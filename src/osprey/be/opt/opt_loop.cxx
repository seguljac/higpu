...
private:
    ...
    WN_MAP _wn_map;     // a map to maintain the parent pointer
    OPT_PHASE _phase;
...
WN* NORMALIZE_LOOP::Normalize_do_loop(WN *wn, OPT_PHASE phase)
{
...
/*****************************************************************************
 *
 * Invokes Normalize_do_loop on each DO_LOOP.
 *
 * If "wn" is a DO_LOOP, return a new loop if necessary. Otherwise, return
 * NULL.
 *
 ****************************************************************************/

WN* NORMALIZE_LOOP::Normalize_loop_stmt(WN *wn, WN *parent)
{
    if (wn == NULL) return NULL;

    // Save the parent pointer in the map.
    ...
    if (opc == OPC_BLOCK)
    {
        WN *stmt, *nextstmt = NULL;
        for (stmt = WN_first(wn); stmt != NULL; stmt = nextstmt)
        {
            // Save the next stmt in case we get rid of stmt.
            nextstmt = WN_next(stmt);

            // Work on the stmt recursively.
            WN *replace_stmt = Normalize_loop_stmt(stmt, wn);
            if (replace_stmt == NULL) continue;

            /* Replace the current stmt with the new one, by inserting the new
             * stmt after the old one and removing the old one.
             *
             * NOTE: for now the old tree is not deleted completely
             * (WN_DELETE_Tree), just the stmt node because we probably didn't
             * copy trees that we use from the original.
             */
            WN_INSERT_BlockAfter(wn, stmt, replace_stmt);
    ...
    else
    {
        ...
        for (INT i = 0; i < nkids; i++) {
            ...
            if (OPCODE_is_scf(WN_opcode(kid))) {
                // The kid cannot be a DO_LOOP.
                WN *insert = Normalize_loop_stmt(kid, wn);
                Is_True(insert == NULL, ("not a BLOCK"));
            }
        }
    }

    if (opc == OPC_DO_LOOP) return Normalize_do_loop(wn, _phase);

    return NULL;
}


WN* COMP_UNIT::Normalize_loop(WN *wn)
{
    if (wn == NULL) return NULL;
    ...
    WN *result = norm_loop.Normalize_loop_stmt(wn, NULL);
