...
private:
    ...
    CODEREP* Du2cr(CODEMAP *htable, OPT_STAB *opt_stab, VER_ID du,
            STMTREP *stmt);
...
public:
    ...
    SSA(MEM_POOL *gpool, MEM_POOL *lpool) {
        mem_pool = gpool; loc_pool = lpool;
    }
    ~SSA(void) {}
    ...
    // Construct SSA form.
    void Construct(CODEMAP *htable, CFG *cfg, OPT_STAB *opt_stab);
