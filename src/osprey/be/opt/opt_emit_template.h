...
WN* Gen_exp_wn(CODEREP *exp, EMITTER *emitter)
{
    WN      *wn;
    OPCODE   op;
    OPERATOR opr;
...
/*** DAVID CODE END ***/
    ...
    case CK_IVAR:
        ...
        {
            // Generate WN for the address.
            WN *kid0 = Gen_exp_wn(exp->Ilod_base(), emitter);

            wn = WN_CreateIload(opr, exp->Dtyp(), exp->Dsctyp(),
                    exp->Offset(), exp->Ilod_ty(), exp->Ilod_base_ty(), kid0,
                    exp->I_field_id());

            if (opr == OPR_ILDBITS) {
                WN_set_bit_offset_size(wn,
                        exp->I_bit_offset(), exp->I_bit_size());
            }

            emitter->Alias_Mgr()->Gen_alias_id(wn,
                    exp->Points_to(emitter->Opt_stab()));

            // for mainopt
            if (emitter->Gen_lno_info()) WN_add_lno_info(wn, exp);
        } 
        break;
    ...
    case CK_LDA:
        if (exp->Is_flag_set(CF_LDA_LABEL)) {
            ...
            wn = WN_CreateLda(OPR_LDA, exp->Dtyp(), MTYPE_V, exp->Offset(),
                    exp->Lda_ty(), exp->Lda_base_st(), exp->Afield_id());
            ...
            IDTYPE ip_alias_class = exp->Points_to(
                    emitter->Opt_stab())->Ip_alias_class();
    ...
    case CK_VAR:
    {
        ...
        opr = (aux_entry->Bit_size() > 0 && aux_entry->Field_id() == 0) ?
            OPR_LDBITS : OPR_LDID;
        wn = WN_Create(opr, exp->Dtyp(), exp->Dsctyp(), 0);

        TY_IDX ty_idx = exp->Lod_ty();
        Is_True(ty_idx != TY_IDX_ZERO,
                ("Gen_exp_wn: NULL Lod_ty() for CK_VAR node"));
        ...
        STMT_CONTAINER *stmt_container, EMITTER *emitter)
{
    ...
    case OPR_STBITS:
    {
        if (Stores_proj_op_to_temp(srep, emitter->Opt_stab())
                && srep->Proj_op_uses() == 1) {
            // The RHS of this stmt will be (re)combined with a projection
            // operation at the unique point where the srep->Lhs() is used. So
            // we do nothing here.
            return NULL;
        }
        ...
        /* CVTL-RELATED start (performance) */
        if (!emitter->For_preopt() && WOPT_Enable_Cvt_Folding
                && rhs_cr->Kind() == CK_OP
                && (rhs_cr->Opr() == OPR_CVT &&
        ...
        if (aux_entry->Is_non_dedicated_preg()
            ...
            if (lhs_offset == 0) {
                if (!aux_entry->Some_version_renumbered()) {
                    ...
                    lhs_offset = emitter->Opt_stab()->Alloc_preg(
                            lhs->Dsctyp(), "renumbered PREG", NULL);
                }
                emitter->Preg_renumbering_map().Insert(
                        lhs->Coderep_id(), (IDTYPE)lhs_offset);
            }
        } else {
            lhs_offset = lhs->Offset();
        }
    ...
    if (emitter->Do_rvi()) {
        if (WN_has_mu(rwn, emitter->Cfg()->Rgn_level())
                && srep->Mu_list() != NULL) {
            emitter->Rvi()->Map_mu_list(rwn, srep->Mu_list());
        }
        if (WN_has_chi(rwn, emitter->Cfg()->Rgn_level())
                && srep->Chi_list() != NULL) {
...
template < class EMITTER >
void Gen_stmt_list_wn(STMT_LIST *stmt_list,
        STMT_CONTAINER *stmt_container, EMITTER *emitter)
{
...
template < class EMITTER >
void Gen_bb_wn(BB_NODE *bb, EMITTER *emitter)
{
    /* For the given basic block, generate the WHIRL stmt list from its
     * stmtlist.
     */
