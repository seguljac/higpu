...
/** DAVID CODE BEGIN **/
  // Do not copy-propagate to base variables that appear in hiCUDA data
  // directives. We do not deal with these directives directly because we also
  // want to prevent copy-propagation to these variables in the kernel region.
  if (st_attr_is_hicuda_data(ST_st_idx(Opt_stab()->St(x->Aux_id()))))
  {
      return NULL;
  }
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
  // Ensure that the definition expression is a constant
  // if we only want constant propagation.
  if (WOPT_Enable_Const_Propagate_Only)
  {
      if (expr->Kind() != CK_CONST && expr->Kind() != CK_RCONST) return NULL;
  }
/*** DAVID CODE END ***/
...
/*****************************************************************************
 *
 * Return the new root of the tree if copy propagation results in a new tree.
 *
 ****************************************************************************/

CODEREP* COPYPROP::Copy_propagate_cr(CODEREP *x, BB_NODE *curbb, 
        BOOL inside_cse, BOOL in_array, BOOL no_complex_preg)
{
    CODEREP *expr;
    CODEREP *cr = Alloc_stack_cr(x->Extra_ptrs_used());
    BOOL need_rehash;
...
void COPYPROP::Copy_propagate_stmt(STMTREP *stmt, BB_NODE *bb)
{
    CODEREP *x;

    if (! Propagate_into_stmt(stmt)) return;

    if (stmt->Op() == OPC_XPRAGMA
            && WN_pragma(stmt->Orig_wn()) == WN_PRAGMA_COPYIN_BOUND) return;

    CODEREP *rhs = stmt->Rhs();
    if (rhs != NULL) {
        x = Copy_propagate_cr(rhs, bb, FALSE, FALSE/*in_array*/);
        if (x) {
...
void COPYPROP::Copy_propagate(BB_NODE *bb)
{
    CODEREP *cr;
    PHI_NODE *phi; 
    PHI_LIST_ITER phi_iter;
    ...
    FOR_ALL_ELEM (phi, phi_iter, Init(bb->Phi_list())) {
        Opt_stab()->Push_coderep(phi->Aux_id(),
                phi->Live() ? phi->RESULT() : NULL);
    }
    ...
    Is_True(! Past_ret_reg_def(), 
            ("COPYPROP::Copy_propagate: "
             "_past_ret_reg_def flag set incorrectly"));
    ...
    if (Htable()->Phase() == MAINOPT_PHASE || bb->Kind() != BB_DOEND) {
        ...
        // Iterate through each phi again to see if their operands have same
        // values; if so, replace by a real assignment.
        FOR_ALL_ELEM (phi, phi_iter, Init(bb->Phi_list())) {
            if (phi->Live()) {
                cr = phi->RESULT();
...
void COMP_UNIT::Do_copy_propagate()
{
    MEM_POOL cp_pool;
...
/** DAVID CODE BEGIN **/
    // In the phase PREOPT_PRE_PP_PHASE, we only do constant propagation.
    BOOL saved_const_prop_only = WOPT_Enable_Const_Propagate_Only;
    if (Phase() == PREOPT_PRE_PP_PHASE)
    {
        WOPT_Enable_Const_Propagate_Only = TRUE;
    }
/*** DAVID CODE END ***/
