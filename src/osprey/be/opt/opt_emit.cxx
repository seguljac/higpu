...
inline WN* STMTREP::Gen_wn(
        STMT_CONTAINER *stmt_container, EMITTER *emitter)
{
    return Gen_stmt_wn(this, stmt_container, emitter);
}

inline void STMT_LIST::Gen_wn(
        STMT_CONTAINER *stmt_container, EMITTER *emitter)
{
    Gen_stmt_list_wn(this, stmt_container, emitter);
}

inline WN* CODEREP::Gen_wn(EMITTER *emitter)
{
    WN *wn = Gen_exp_wn(this, emitter);

    /* We only link it with the WN node if it represents a scalar variable or
     * an indirect variable that is a parameter.
     *
     * DAVID COMMENT: this appears to be done in <Gen_exp_wn> anyway.
     */
    if (Kind() == CK_VAR || (Kind() == CK_IVAR && Opr() == OPR_PARM)) {
...
inline void BB_NODE::Gen_wn(EMITTER *emitter)
{
...
inline BOOL Verify_Block_Stmt_Sequence(WN *block)
{
    WN *tmp = WN_first(block);
    while (tmp && WN_next(tmp) != NULL) tmp = WN_next(tmp);
    return (tmp == WN_last(block)) ? TRUE : FALSE;
}

static WN* Create_block_stmt(BB_NODE *ff, BB_NODE *ll)
{
    ...
    while (ff->First_stmtrep() == NULL &&
            ...
            ff->Firststmt() == NULL &&
#endif
            ff->Kind() == BB_GOTO && ff != ll) ff = ff->Next();
    if (ff == ll && ll->First_stmtrep() == NULL) return wn;
    ...
    for (BB_NODE *tmpf = ff; tmpf != NULL; tmpf = tmpf->Next()) {
        ...
        if (tmpf->Laststmt() != NULL) WN_last(wn) = tmpf->Laststmt();
...
static WN* Create_block(EMITTER *emitter, BB_NODE *ff, BB_NODE *ll)
{
    emitter->Gen_wn(ff, ll);
    return Create_block_stmt(ff, ll);
}

BOOL EMITTER::Raise_altentry(BB_NODE *bb)
{
...
void EMITTER::Raise_func_entry(BB_NODE *bb, BB_NODE *last_bb)
{
    FmtAssert(bb != NULL,
    ...
    else
    {
        ...
        // Go through region pragmas and convert aux_ids back to STs and
        // offsets.
        if (REGION_is_EH(_opt_func)) {
            Opt_stab()->Convert_EH_pragmas(_opt_func);
        }

        // Connect between entry chi and stmtrep was delayed until region
        // created.
...
static WN* Raise_if_stmt(EMITTER *emitter, BB_NODE **bb)
{
...
void EMITTER::Gen_wn(BB_NODE *first_bb, BB_NODE *last_bb)
{
    ...
    while (bb != NULL) {
        switch (bb->Kind()) {
            ...
            case BB_REPEATBODY:
            case BB_WHILEEND:
                if (!Can_raise_to_scf(bb)) goto ordinary;
            ...
            case BB_REGIONSTART:
            {
                ...
                RID *bb_rid = bb_region->Rid();

/** DAVID CODE BEGIN **/
                // We want to emit MP, HICUDA and EH regions, and any
                // transparent regions when Preopt is called from IPA or LNO.
                if (RID_TYPE_mp(bb_rid) || RID_TYPE_hicuda(bb_rid)
                    || RID_TYPE_eh(bb_rid)
                    // kludge for 7.2, see pv 457243
                    || RID_TYPE_olimit(bb_rid) || RID_TYPE_pragma(bb_rid))
/*** DAVID CODE END ***/
                {
...
/*** DAVID CODE END ***/
                    ...
                    if (bb_region->Region_end() == last_bb) return;
                    prev_bb = bb;
...
WN* EMITTER::Emit(COMP_UNIT *cu, DU_MANAGER *du_mgr, ALIAS_MANAGER *alias_mgr)
{
    _htable = cu->Htable();
    _cfg = cu->Cfg();
    ...
    // Tell everyone that the generated WHIRL has feedback info if it does.
    if (Cur_PU_Feedback) {
...
BOOL EMITTER::Can_raise_to_scf(BB_NODE *bb)
{
    ...
    switch (bb->Kind())
    {
        case BB_LOGIF:
            if (bb->Ifinfo() != NULL)
            {
                if (bb->Succ()->Contains(bb->If_then())
                        && bb->Succ()->Contains(bb->If_else())) return TRUE;

                BB_NODE *bb_if = bb->If_merge();
                if (bb_if != NULL) {
                    if (bb_if->Label_stmtrep() == NULL) {
                        if (bb_if->Labnam() == 0)
                            bb_if->Set_labnam(_cfg->Alloc_label());
                        bb_if->Add_label_stmtrep(Mem_pool());
                    }
                    FOR_ALL_ELEM(pred, bb_pred_iter, Init(bb_if->Pred())) {
                        if (pred->Branch_stmtrep() == NULL) {
                            new_goto = CXX_NEW(STMTREP(OPC_GOTO), Mem_pool());
                            new_goto->Init_Goto(NULL, bb_if->Labnam(), 0);
...
/** DAVID CODE BEGIN **/
            STMTREP *last_step_stmt = bb_step->Stmtlist()->Tail();
            if (last_step_stmt == NULL
                    || last_step_stmt->Opr() != OPR_STID
                    || last_step_stmt->Rhs()->Kind() != CK_OP) {
                printf("!!! FAIL THROUGH HERE!\n");
                last_step_stmt->Print();
                printf("!!! END OF FAIL THROUGH HERE!\n");
            } else
/*** DAVID CODE END ***/
#endif
