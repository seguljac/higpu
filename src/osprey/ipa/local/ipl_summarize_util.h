...
// Find the common loop that surrounds wn1 and wn2. If a wn belongs to the
// bounds expression, then it is considered to be outside the loop.
WN* LNO_Common_Loop(WN* wn1, WN* wn2);

// return the stmt or scf surrounding an expression node
extern WN* IPL_get_stmt_scf(WN* av);
...
struct AUX_SYMBOL_ACCESS
{
    ...
    {
        UINT32 index = ST_IDX_index(st_idx);
        SYMTAB_IDX level = ST_IDX_level(st_idx);

        Is_True(index < Aux_Symbol_Info[level].size(),
                ("array out of bound for Aux_Symbol"));
        return Aux_Symbol_Info[level][index];
    }
...
extern void Set_lang(SUMMARY_PROCEDURE *proc);
...
extern INT Get_stmt_id(void);

extern void
Reset_cd_and_stmt (WN_MAP Summary_Map, INT saved_cd_idx, INT saved_stmt_id);

extern SUMMARY_CONTROL_DEPENDENCE* Get_new_cd(void);
...
// DAVID HAN FIX:
//
// Unnamed namespace cannot be used here because Chi_To_Idx_Map is then
// considered static and its definition is removed from ipl_summarize_util.h
// which does not use this variable.
//
// namespace {
template <class X>
struct ptr_hash
...
// -------------------------------------------------------
// This class is used for checkpointing of indices into various summary
// arrays. It is highly recommended to use it with
// SUMMARIZE<>::Restore_from_check_point. With very few exceptions, using
// Decidx on an individual summary array is not safe.
// -------------------------------------------------------
