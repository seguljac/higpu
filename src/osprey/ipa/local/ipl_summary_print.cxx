...
void SUMMARY_PROCEDURE::Print(FILE *fp, INT32 id) const
{
    ...
    if (Is_may_inline()) fputs(", may inline", fp);

    if (Is_must_inline()) fputs(", must inline", fp);

    if (Is_no_inline()) fputs(", no inline", fp);

    if (Is_varargs()) fputs(", varargs", fp);

    if (Is_alt_entry()) fputs(", alternate entry", fp);

    if (Has_alt_entry()) {
        fprintf( fp, ", has %d alternate entries", Get_altentry_count());
    }

    if (Has_pstatic()) fputs(", has local statics", fp);

    if (Is_direct_mod_ref()) fputs(", has direct mods and refs", fp);

    if (Is_exc_inline()) fputs(", has exceptions", fp);

    if (Is_no_delete()) fputs(", no delete", fp);

    if (Is_block_data()) fputs(", block data", fp);

    if (Has_addr_taken_reset()) fputs(", addr_taken reset", fp);

    if (Has_PU_freq()) fputs(", PU freq reset", fp);

    if (Has_formal_pragma()) fputs(", formal pragma", fp);

    if (Has_parallel_pragma()) fputs(", parallel pragma", fp);

    if (Has_parallel_region_pragma()) fputs(", parallel region pragma", fp);

    if (Has_fstatic()) fputs(", fstatic", fp);

    if (Has_unknown_calls()) fputs(", unknown calls", fp);

    if (Has_incomplete_array_info()) fputs(", incomplete array info", fp);

    if (Has_mp_needs_lno()) fputs(", mp needs lno", fp);
    ...
    if (Has_messy_regions()) fputs(", has messy array regions", fp);

    if (Has_unstructured_cflow()) fputs(", has unstructured cflow \n", fp);
