...
// A hash table that stores terms that can be re-used. The key is a 64-bit
// element, and the hash table stores the index into the term array.
...
extern void Init_write_asections(MEM_POOL*);

/**
 * Map the array sections per PU into the output file, so the memory can then
 * all be freed.
 */
extern void Map_asections(ARRAY_SUMMARY *summary, SUMMARY_PROCEDURE *p);

//------------------------------------------------------------------------
// write out the summary information for flow array section analysis
//------------------------------------------------------------------------
...
private:
    ...
    // contain index and count of each set of projected regions
    ...
    /* offset in the output file for each array */
...
public:
    ...
    ARRAY_SUMMARY_OUTPUT(MEM_POOL *m)
    {
        ...
        _projected_regions = CXX_NEW(PROJECTED_REGION_ARRAY(m), m);
    ...
    IVAR_ARRAY *Get_ivar_array() { return _ivars; }
    INT_ARRAY *Get_scalar_array() { return _scalar_items; }

    INT Get_term_offset() const { return offset_term; }

    TERM *Get_term(INT i) { return &(*_terms)[i]; }
    PROJECTED_NODE* Get_projected_node(INT i) { return &(*_project_nodes)[i]; }
    PROJECTED_REGION *Get_projected_region(INT i) {
        return &(*_projected_regions)[i];
    }
    REGION_ARRAYS* Get_region_array (INT i) { return &(*_region_arrays)[i]; }
    CFG_NODE_INFO* Get_cfg_node(INT i) { return &(*_cfg_nodes)[i]; }
    IVAR* Get_ivar(INT i) { return &(*_ivars)[i]; }
    LOOPINFO *Get_loopinfo(INT i) { return &(*_loopinfo_nodes)[i]; }
    ...
    INT Get_projected_node_count() { return _project_nodes->Lastidx(); }
    INT Get_projected_region_count() { return _projected_regions->Lastidx(); }
    ...
    void Write_summary(struct output_file *fl, INT cur_sec_disp);
    ...
    // Returns an index into the term array, or -1 if not found.
