...
static void Process_Command_Line(INT argc, char **argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        if (arg_str == NULL || arg_str[0] != '-') continue;

        if (Process_Command_Line_Group(arg_str+1, IPL_Option_Groups)) continue;

        if (strcmp(arg_str, "-cmds") == 0) {
            driver_argc = argc - i - 1;
            if (driver_argc > 0) driver_argv = argv + i + 1;
            i = argc;	    /* stop processing */
        }
    }

    if (OPT_Reorg_Common_Set) Do_Split_Commons = OPT_Reorg_Common;

    Do_Split_Commons_Set = (OPT_Reorg_Common_Set && OPT_Reorg_Common);

    Do_Par = (IPA_Enable_Array_Summary && Run_preopt);

    if (Do_Par) WOPT_Enable_Generate_DU = TRUE;
...
/** DAVID CODE BEGIN **/
    Summary->Set_ipa_node_context(proc_node, parent_kernel_map);
/*** DAVID CODE END ***/
    Summary->Summarize(w);
    WB_IPL_Set_Array_Summary(NULL);
...
void Ipl_Extra_Output(Output_File *ir_output)
{
#ifndef KEY
    if(IPA_Enable_Reorder)/*&&Feedback_Enabled[PROFILE_PHASE_BEFORE_VHO] )*/
#endif // !KEY
        // reorder, free pointers, set hot_flds
        Summary->Finish_collect_struct_access();
