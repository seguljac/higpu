...
template <PROGRAM program>
INT SUMMARIZE<program>::Process_polynomial_jump_function(WN *w)
{
    OPCODE op = WN_opcode(w);

    if (!MTYPE_is_integral(OPCODE_rtype(op))) return -1;
    if (!OPCODE_is_expression(op)) return -1;
    if (OPCODE_is_load(op)) return -1;
    ...
    else if (OPCODE_nkids(op) == 1)
    {
        /* unary operators
         *
         * Treat it as a binary operator with const value zero. The evaluator
         * will have to discard the const value based on the opcode.
         *
         * When the opcode is a cvtl, use the const value field for the
         * conversion bits.
         */
        expr->Set_mtype(OPCODE_rtype(op));
        expr->Set_opcode(op);
    ...
    if (expr->Is_expr_unknown()) {
        // Back track if this expression is not regular.
        Restore_from_check_point(&chk_pt);
        expr_idx = -1;
    } else {
...
template <PROGRAM program>
void SUMMARIZE<program>:: Record_ref_formal(WN* w)
{
    INT i;
    WN* w2;
    WN *parent_w;
    ...
    // don't bother with constants and register variables
    if (OPCODE_has_sym (WN_opcode (w))
            && (ST_class(WN_st(w)) == CLASS_PREG)) return;
...
template <PROGRAM program>
void SUMMARIZE<program>::Record_ref_all_formal(WN* w, BOOL parm_store)
{
    INT i,j;
    WN* w2;
    WN *parent_w;
...
static void Record_unknown_memory_op(WN* wn)
{
    ...
    Summary->Get_procedure(
            Summary->Get_procedure_idx())->Set_has_incomplete_array_info();
}
...
//-----------------------------------------------------------
template <PROGRAM program>
void SUMMARIZE<program>::Record_ref(WN *w)
{
    ...
    ST *s = OPCODE_has_sym(WN_opcode(w)) ? WN_st(w) : NULL;
    if (s != NULL
            && ST_st_idx(s) != ST_base_idx(s) && !ST_is_weak_symbol(s)) {
        s = ST_base(s);
    }
    ...
    switch (WN_operator(w))
    {
        ...
        case OPR_LDID:
            ...
            if (parent_w != NULL
                    && WN_operator(parent_w) == OPR_ILOAD) return;
        ...
        case OPR_MLOAD:
        {
            ...
            switch (WN_operator(w2))
            {
                case OPR_ARRAY:
                    ...
                    if (!OPCODE_has_sym(WN_opcode(w2))) return;
                    // fall through
                ...
                case OPR_LDID:
                    ...
                    if (ST_st_idx(s) != ST_base_idx(s)
                            && !ST_is_weak_symbol(s)) s = ST_base(s);

                    switch (ST_sclass(s))
                    {
                        case SCLASS_COMMON:
                            if (!Inliner_copy_prop) {
                                Record_global_ref(w2, s, OPR_ILOAD,
                                        refcount_only);
                            }
                            break;
                        ...
                        case SCLASS_FORMAL_REF:
                            if (Inliner_copy_prop) break;
                            // may fall through

                        case SCLASS_FORMAL:
                            if (!refcount_only) Record_ref_formal(w);
                            break;

                        default:
                            if (Trace_Modref
                                    || (Trace_CopyProp && Inliner_copy_prop))
                            {
...
template <PROGRAM program>  
BOOL SUMMARIZE<program>::Process_control_dependence(WN *w, INT node_index)
{
    if (w == Get_entry_point()) return TRUE;

    /* Find the "statement" that contains this WN. */
    WN* stmt_node = w;

    while (stmt_node != NULL && !OPCODE_is_scf(WN_opcode(stmt_node))
            && WN_opcode(LWN_Get_Parent(stmt_node)) != OPC_BLOCK) {
        stmt_node = LWN_Get_Parent(stmt_node);
    }

    /* Find its control-dependent statement through CFG. */
    ...
    else
    {
        ...
        // control-dependence for OpenMP single pragma, which does not have
        // corresponding WN.
        if (cond_stmt == NULL) return FALSE;
#endif
    ...
    /* Make sure that there is a CD summary for this control statement. */
    ...
    /* Add a new STMT summary to the appropriate branch of the CD summary. */
    ...
    SUMMARY_STMT *stmt = Get_new_stmt(cd_idx, branch, stmt_node);

    if (Do_Par) WN_MAP32_Set(Stmt_Map, stmt_node, cd_idx + 1);

    /* Set up the new STMT summary based on the given WN node (not the whole
     * statement).
     */
    ...
    switch (WN_operator(w))
    {
        ...
        default:
        {
            ...
            } else if (OPCODE_is_scf(opc) || OPCODE_is_non_scf(opc)) {
    ...
    /* We need to follow the cd-chain all the way to the function entry point,
     * but don't want to repeat if the chain has already been established
     * (hence the check for "new_cd_created").
     */
    if (new_cd_created) {
        if (cd->Is_entry())
            Set_cd_head_of_chain(cd_idx);
...
template <PROGRAM program>
void SUMMARIZE<program>::Copy_summary_ctrl_dep(SUMMARY_CONTROL_DEPENDENCE *cd)
{
    SUMMARY_CONTROL_DEPENDENCE *ctrl_dep = New_ctrl_dep();
    *ctrl_dep = *cd;
    Set_cd_ctrl_index(cd, Get_ctrl_dep_idx());
    // DAVID COMMENT: why not leaving the WN* as it is?
    ctrl_dep->Set_map_id(WN_map_id(cd->Get_wn()));

    /* Copy STMT summary. */

    // Allocate enough entries in the STMT summary.
    INT stmt_idx = Get_stmt_idx();
    _stmt.Setidx(stmt_idx + cd->Get_true_count() + cd->Get_false_count());

    SUMMARY_STMT *stmts = NULL;
...
template <PROGRAM program>
inline void SUMMARIZE<program>::Generate_summary_control_dependence(void)
{
    SUMMARY_CONTROL_DEPENDENCE *cd;

    // We only start the operation from a head-of-chain CD; the descending CDs
    // are recursively copied in Copy_summary_ctrl_dep.
    while ((cd = Get_next_cd_chain()) != NULL) Copy_summary_ctrl_dep(cd);
