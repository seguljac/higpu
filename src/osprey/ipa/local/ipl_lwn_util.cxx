 ...
 * replaced with IPL_LWN_ functions (except macros like LWN_Get/Set_Parent).
 *
 * This is used to resolve conflicts of calling LWN_ functions when both
 * ipl.so and lno.so are loaded into memory.
 *
 ****************************************************************************/
 ...
 */
extern WN* IPL_LWN_Get_Next_Tree_Node (const WN* wn) {
  WN* ret_wn = NULL;
 ...
 */
extern WN* IPL_LWN_Get_Next_Stmt_Node (const WN* wn) {
  WN* ret_wn = NULL;
 ...
 */
extern WN* IPL_LWN_Get_Next_SCF_Node (const WN* wn) {
  WN* ret_wn = NULL;
...
void IPL_LWN_Insert_Block_Before (WN *block, WN *wn, WN *in)
{
  WN *first;
  WN *last;
  WN *w;

  Is_True(wn || block, ("IPL_LWN_Insert_Block_Before: nowhere to insert"));

  if (wn) {
    WN *parent_wn = LWN_Get_Parent(wn);
    Is_True (parent_wn, ("IPL_LWN_Insert_Block_Before: parent is NULL\n"));
  
    if (block == NULL)
      block = parent_wn;
    else {
      Is_True(block == parent_wn,
	      ("IPL_LWN_Insert_Block_Before: supplied block not parent"));
    }
  }

  Is_True(!block || (WN_opcode(block) == OPC_BLOCK),
	  ("IPL_LWN_Insert_Block_Before: block is not a BLOCK node"));
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("IPL_LWN_Insert_Block_Before: insert node 0x%p not a stmt/scf\n", wn));
  Is_True((in != NULL),
	  ("IPL_LWN_Insert_Block_Before: insert a NULL node?"));
  ...
  if (wn) {
    ...
    Is_True (kids, ("IPL_LWN_Insert_Block_Before: could not find node 'wn'"));
  }
#endif
...
void IPL_LWN_Insert_Block_After (WN *block, WN *wn, WN *in)
{
  WN *first;
  WN *last;
  WN *w;

  Is_True(wn || block, ("IPL_LWN_Insert_Block_After: nowhere to insert"));

  if (wn) {
    WN *parent_wn = LWN_Get_Parent(wn);
    Is_True (parent_wn, ("IPL_LWN_Insert_Block_After: parent is NULL\n"));
  
    if (block == NULL)
      block = parent_wn;
    else {
      Is_True(block == parent_wn,
	      ("IPL_LWN_Insert_Block_After: supplied block not parent"));
    }
  }

  Is_True(!block || (WN_operator(block) == OPR_BLOCK),
	  ("IPL_LWN_Insert_Block_After: block is not a BLOCK node"));
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("IPL_LWN_Insert_Block_After: insert node 0x%p not a stmt/scf\n", wn));
  Is_True((in != NULL),
	  ("IPL_LWN_Insert_Block_After: insert a NULL node?"));
  Is_True(OPCODE_is_stmt(WN_opcode(in)) || OPCODE_is_scf(WN_opcode(in)), 
	  ("IPL_LWN_Insert_Block_After: insert node 0x%p not a stmt/scf\n", in));
  ...
  if (wn) {
    ...
    Is_True (kids, ("IPL_LWN_Insert_Block_After: could not find node 'wn'"));
  }
#endif
...
WN* IPL_LWN_Extract_From_Block(WN* item)
{
...
WN* IPL_LWN_Extract_From_Block(WN* parent, WN* item)
{
  FmtAssert(LWN_Get_Parent(item) == parent,
	    ("Bad parent for IPL_LWN_Extract_From_Block"));
 ...
 */
extern void IPL_LWN_Delete_From_Block(WN *block, WN* wn) {
  WN *node;
  WN* parent_wn;
  ...
  Is_True (wn, ("IPL_LWN_DeleteFromBlock: deleting a NULL node"));
  Is_True (((!block) || (WN_opcode(block) == OPC_BLOCK)),
           ("IPL_LWN_DeleteFromBlock: Expecting a BLOCK node"));
  parent_wn = LWN_Get_Parent(wn);
  Is_True (((!block) || (block == parent_wn)),
           ("IPL_LWN_DeleteFromBlock: block is not the parent"));
  
  /* make sure the operators are legal */
  Is_True (OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
           ("IPL_LWN_DeleteFromBlock: Expecting a SCF or a stmt node"));
  ...
  FmtAssert (node != NULL,
             ("IPL_LWN_DeleteFromBlock: could not find node to delete"));
 ...
 *
 ***********************************************************************/
extern void IPL_LWN_Delete_Tree_From_Block(WN* wn) {
  IPL_LWN_Delete_Tree (IPL_LWN_Extract_From_Block(wn));
}

static WN* IPL_LWN_Copy_Tree_R(WN *,BOOL,WN_MAP,BOOL,WN_MAP,STACK_OF_WN *,
			   BOOL); 

/* copy a tree */
/* if copy_access is TRUE, create copies of all the access vectors */
extern WN* IPL_LWN_Copy_Tree(WN *wn, BOOL copy_access, WN_MAP access_map,
...
#endif
  WN *result = 
    IPL_LWN_Copy_Tree_R(wn,copy_access,access_map,copy_version,version_map,
...
static  WN* IPL_LWN_Copy_Tree_R(WN *wn, BOOL copy_access, WN_MAP access_map,
			    BOOL copy_version, WN_MAP version_map,
			    STACK_OF_WN *region_stack,
			    BOOL copy_all_nodes) {
  WN* ret_wn = NULL;
  ...
  if (wn) {
    ...
    IPL_LWN_Copy_Linenumber(wn, ret_wn);
    ...
    if (copy_access) {  // copy the access vectors
      if (oper == OPR_ARRAY) {
        ...
        if (LNO_enabled) {
          Is_True (array, ("IPL_LWN_Copy_Tree missing array-access-map"));
        }
        if (array) {
        ...
        if (LNO_enabled) {
          Is_True (dli, ("IPL_LWN_Copy_Tree missing do-loop access-map"));
        }
        if (dli) {
        ...
        if (LNO_enabled) {
          Is_True (ii, ("IPL_LWN_Copy_Tree missing if-access-map"));
        }
        if (ii) {
    ...
    Is_True(copy_access == FALSE,
	    ("Compile with -DLNO to copy access vectors in IPL_LWN_Copy_Tree_R"));
#endif
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
      ...
      kid = WN_first (wn);
      if (kid) {
        ret_kid = IPL_LWN_Copy_Tree_R (kid,
      ...
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        if (WN_kid (wn, kidno)) { 
          ret_kid = IPL_LWN_Copy_Tree_R (WN_kid (wn, kidno), 
                                     copy_access, access_map,
                                     copy_version, version_map,
				     region_stack,
				     copy_all_nodes);
...
void IPL_LWN_Copy_Def_Use_Node(WN* from_node, WN* to_node, DU_MANAGER* du)
{ 
...
void IPL_LWN_Copy_Def_Use(WN* from_exp, WN* to_exp, DU_MANAGER* du)
{
  OPCODE fop = WN_opcode(from_exp);
  OPCODE top = WN_opcode(to_exp);

  FmtAssert(fop == top && OPCODE_is_expression(fop),
	    ("Opcodes unequal IPL_LWN_Copy_Def_Use(%d,%d) or not expr", fop, top));

  for (INT k = 0; k < WN_kid_count(from_exp); k++)
    IPL_LWN_Copy_Def_Use(WN_kid(from_exp,k), WN_kid(to_exp,k), du);

  IPL_LWN_Copy_Def_Use_Node(from_exp, to_exp, du);
 ...
 */
void IPL_LWN_Parentize(WN* wn)
{
    WN_Parentize(wn, Parent_Map, Current_Map_Tab);
}

/* Given a tree, check that its parent pointers are consistent.
 * Return TRUE if ok, FALSE otherwise.
 */
BOOL IPL_LWN_Check_Parentize (const WN* wn) {
    BOOL isok = TRUE;
    if (!OPCODE_is_leaf (WN_opcode (wn))) { 
        ...
        if (!isok) break;
        isok &= IPL_LWN_Check_Parentize (kid);
        if (!isok) break;
        kid = WN_next (kid);
      }
    } else if (WN_opcode(wn) == OPC_REGION) {
      isok &= (LWN_Get_Parent(WN_region_body(wn)) == wn);
      isok &= IPL_LWN_Check_Parentize (WN_region_body(wn));
    } else {
      INT kidno;
      WN* kid;
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        ...
        if (kid) { 
          Is_True ((LWN_Get_Parent (kid) == wn),
                   ("IPL_LWN_Check_Parentize(): kid 0x%p (%s), parent 0x%p (%s)\n",
...
// only.
void IPL_LWN_Parentize_One_Level (const WN* wn) {
   INT i;
 ...
 */
void IPL_LWN_Delete_Tree(WN *wn)
{
    if (wn == NULL) return;
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
        ...
        for (INT i = 0; i < WN_kid_count(wn); i++) {
            IPL_LWN_Delete_Tree(WN_kid(wn,i));
        }
    }
    ...
    if (parent && WN_operator(parent) == OPR_BLOCK) {
        IPL_LWN_Delete_From_Block(parent,wn);
    } else {
        WN_Delete(wn);
    }
}
...
void IPL_LWN_Delete_LNO_dep_graph(WN *wn)
{
...
void IPL_LWN_Delete_CG_dep_graph(WN *wn)
{
...
void IPL_LWN_Update_Def_Use_Delete_Tree(WN *wn, DU_MANAGER* du)
{
  if (du == NULL)
    du = Du_Mgr;
  ...
  if (opcode == OPC_BLOCK) {
    WN* kid = WN_first (wn);
    while (kid) {
      IPL_LWN_Update_Def_Use_Delete_Tree(kid, du);
      kid = WN_next(kid);
    }
  } 
  ...
  for (INT i = 0; i < WN_kid_count(wn); i++) {
    IPL_LWN_Update_Def_Use_Delete_Tree(WN_kid(wn,i), du);
  }
}

void IPL_LWN_Update_Dg_Delete_Tree(WN *wn, ARRAY_DIRECTED_GRAPH16* dg)
{
  FmtAssert(wn, ("IPL_LWN_Update_Dg_Delete_Tree null wn!"));
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    ...
    if (kid != NULL) {
      ...
      kid = WN_next(kid);
      while (kid != NULL) {
	IPL_LWN_Update_Dg_Delete_Tree(prev, dg);
	prev = kid;
	kid = WN_next(kid);
      }
      IPL_LWN_Update_Dg_Delete_Tree(prev, dg);
    ...
    for (INT i = 0; i < WN_kid_count(wn); i++) {
      IPL_LWN_Update_Dg_Delete_Tree(WN_kid(wn,i), dg);
    }
  }
...
extern WN *IPL_LWN_Get_Statement(WN *wn)
{
...
WN *IPL_LWN_CreateDO(WN *index, WN *start, WN *end, WN *step,
		       WN *body)
{
  WN *loop_info = IPL_LWN_CreateLoopInfo(IPL_LWN_Copy_Tree(index),NULL,0,0,0);
...
WN *IPL_LWN_CreateLoopInfo(WN *induction, WN *trip, UINT16 trip_est,
...
WN *IPL_LWN_CreateDoWhile(WN *test, WN *body)
{
...
WN *IPL_LWN_CreateWhileDo(WN *test, WN *body)
{
...
WN *IPL_LWN_CreateIf(WN *test, WN *if_then, WN *if_else)
{
...
WN *IPL_LWN_CreateCondbr(INT32 label_number,WN *exp)
{
...
WN *IPL_LWN_CreateReturn()
{
   WN* wn = WN_CreateReturn();
   return wn;
}

WN *IPL_LWN_CreateCompgoto(INT32 num_entries, WN *value,
			WN *block, WN *deflt)
{
...
#ifndef KEY
WN *IPL_LWN_CreateIstore(OPCODE opc, WN_OFFSET offset,
...
WN *IPL_LWN_CreateMstore(WN_OFFSET offset, TY_IDX ty, WN *value,
...
WN *IPL_LWN_CreateStid(OPCODE opc, WN_OFFSET offset, 
...
WN *IPL_LWN_CreateStid(OPCODE opc, WN *orig_op, WN *value)
{
  Is_True((WN_operator(orig_op) == OPR_LDID) ||
          (WN_operator(orig_op) == OPR_STID),
	  ("Illegal orig_op in IPL_LWN_Create_Stid"));
...
WN *IPL_LWN_CreateLdid(OPCODE opc, WN *orig_op)
{
  Is_True(orig_op, ("IPL_LWN_CreateLdid() called will null orig_op"));
  Is_True((WN_operator(orig_op) == OPR_LDID) ||
          (WN_operator(orig_op) == OPR_STID),
	  ("Illegal orig_op in IPL_LWN_Create_Ldid"));
...
WN *IPL_LWN_CreateDivceil(TYPE_ID type, WN *kid0, WN *kid1)
{
  ...
  for (i = 0; i < nkids; i++) {
    if (WN_operator(kids[i]) != OPR_PARM) {
      ...
      kids[i] = IPL_LWN_CreateParm(type, kids[i], Be_Type_Tbl(type), 
			       WN_PARM_BY_VALUE);
    }
  }
  ...
  IPL_LWN_Parentize_One_Level(wn);

  return (wn);
}


WN *IPL_LWN_CreateDivfloor(TYPE_ID type, WN *kid0, WN *kid1)
{
  OPCODE op = OPCODE_make_op(OPR_INTRINSIC_OP, type, MTYPE_V);
  INT i;
  WN *kids[2];
...
WN *IPL_LWN_CreateExp1(OPCODE opc, WN *kid0)
{
...
WN *IPL_LWN_CreateExp2(OPCODE opc, WN *kid0, WN *kid1)
{
...
#ifndef KEY
WN *IPL_LWN_CreateIload(OPCODE opc, WN_OFFSET offset, 
...
WN *IPL_LWN_CreateMload(WN_OFFSET offset, TY_IDX ty,WN *addr,
		    WN *num_bytes)
{
...
WN *IPL_LWN_CreateCvtl(OPCODE opc, INT16 cvtl_bits, WN *kid0)
{
...
WN *IPL_LWN_CreateParm(TYPE_ID rtype, WN *parm_node, TY_IDX ty, UINT32 flag)
{
...
#ifdef LNO
WN *IPL_LWN_CreatePrefetch(WN_OFFSET offset, UINT32 flag, WN* addr) {
#ifdef KEY
  Num_Prefetches ++;
#endif
...
WN* IPL_LWN_Int_Type_Conversion(WN *wn, TYPE_ID to_type)
{
...
#ifdef LNO
extern WN *IPL_LWN_Loop_Trip_Count(const WN *loop)
{
  ...
  WN *new_lb = IPL_LWN_Copy_Tree(lb);
  WN *new_ub = IPL_LWN_Copy_Tree(ub);
  if (Du_Mgr) {
    IPL_LWN_Copy_Def_Use(lb,new_lb,Du_Mgr);
    IPL_LWN_Copy_Def_Use(ub,new_ub,Du_Mgr);
  }
  trip_cnt = IPL_LWN_CreateExp2( OPCODE_make_op(OPR_SUB,trip_mtype,MTYPE_V),
			    new_ub,new_lb);
  if (OPCODE_operator(ub_compare) != OPR_GT &&
      OPCODE_operator(ub_compare) != OPR_LT)
    trip_cnt = IPL_LWN_CreateExp2( OPCODE_make_op(OPR_ADD,trip_mtype,MTYPE_V),
			      trip_cnt, IPL_LWN_Copy_Tree(incr) );
  trip_cnt = IPL_LWN_CreateExp2( OPCODE_make_op(OPR_DIV,trip_mtype,MTYPE_V),
			    trip_cnt, IPL_LWN_Copy_Tree(incr) );
...
extern void IPL_LWN_Set_Frequency_Tree(const WN* wn, const INT32 count)
{
  if (Cur_PU_Feedback) {
    IPL_LWN_ITER* wniter = IPL_LWN_WALK_StmtIter( (WN *) wn );
    while (wniter) {
      WN *cur = wniter->wn;
      wniter = IPL_LWN_WALK_TreeNext(wniter);
...
extern void IPL_LWN_Copy_Frequency_Tree(const WN* wn, const WN* wn_from)
{
  if (Cur_PU_Feedback) {
...
extern void IPL_LWN_Adjust_Frequency_Tree(const WN* wn, const INT32 count)
{
  if (Cur_PU_Feedback) {
    IPL_LWN_ITER *wniter = IPL_LWN_WALK_StmtIter( (WN *) wn );
...
extern void IPL_LWN_Scale_Frequency_Tree(const WN* wn, const float ratio)
{
  if (Cur_PU_Feedback) {
    IPL_LWN_ITER *wniter = IPL_LWN_WALK_StmtIter( (WN *) wn );
...
#else
void IPL_LWN_Set_Frequency_Tree(const WN*, const INT32) {}
void IPL_LWN_Copy_Frequency_Tree(const WN*, const WN *) {}
void IPL_LWN_Adjust_Frequency_Tree(const WN*, const INT32) {}
void IPL_LWN_Scale_Frequency_Tree(const WN*, const float) {}
