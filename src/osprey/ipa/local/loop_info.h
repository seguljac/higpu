...
// mUINT8 Depth
//
//  What is the "depth" of this DO loop. The outer DO loop (good or bad) is
//  numbered 0. The next inner 1, etc.
//
// WN *Guard
//
//  Points to the if statement that "guards" this loop.  By guard we guarantee
//  that the do loop is not zero trip count.  If this is NULL, then the loop
//  is non-zero trip count without any special guard.
//
// IF_INFO Contains information about block structured IFs
//	
//---------------------------------------------------------------------------
...
extern BOOL Record_scalar_flow(WN* stid);
