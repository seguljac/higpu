...
private:
    ...
    /* For formals, callsites, etc. that can have multiple entries for a
     * procedure, _*_index gives the starting index into the corresponding
     * SUMMARY_* array and _*_count gives the number of entries in that array
     * that belong to this procedure.
     */

    // index into the SYMMARY_SYMBOL array for the procedure symbol
    mINT32 _symbol_index;
    mINT32 _callsite_index;         // call sites the procedure contains
    mINT32 _ctrl_dep_index;
    mINT32 _formal_index;           // formal parameters
    mINT32 _global_index;           // non-local variables accessed
    mINT32 _common_index;
    mINT32 _feedback_index;         // one per procedure

    mINT32 _array_section_index;    // index into CFG_NODE_INFO array
...
#ifdef KEY
    void Set_has_pragma_side_effect() {
        _state |= PROC_HAS_PRAGMA_SIDE_EFFECT;
    }
    BOOL Has_pragma_side_effect() const {
        return _state & PROC_HAS_PRAGMA_SIDE_EFFECT;
    }
#endif
...
#ifdef KEY
    // Function runtime addr used by -IPA:icall_opt.
    UINT64  _func_runtime_addr;
#endif
...
#define IPL_FUNC_PTR            0x0001
#define IPL_INTRINSIC_FUNC      0x0002
#define IPL_HAS_CALLSITE_FREQ	0x0004
// for pragma support of inline/noinline
#define IPL_CALL_MUST_INLINE	0x0008
#define IPL_CALL_NO_INLINE	    0x0010

#ifndef PATHSCALE_MERGE_ZHC
#ifdef KEY
#define IPL_ICALL_SLOT          0x0020  
#endif
#endif

#define IPL_IN_CASE_CLAUSE      0x0040

#ifdef KEY
#define IPL_ICALL_TARGET        0x0080
#endif

#define IPL_IS_VIRTUAL_CALL     0x0100
    
private:

    union {
        WN* w;				    // for the standalone inliner, we need the wn
        mINT32 _map_id;			// map id corresponding to the whirl node
        mUINT64 _targ_runtime_addr;	// used by icall-optimization
                                    // IPL_ICALL_TARGET must be set.
    } u1;

    union {
        // index into the SUMMARY_SYMBOL array for callee function symbol
        mINT32 _symbol_index;
        // index into the SUMMARY_VALUE array for callee function address
        // (function pointer)
        mINT32 _value_index;
    } u2;

    // starting index into the SUMMARY_ACTUAL array for arguments
    ...
    mUINT32 _vtable_offset;     // virtual function position
...
public:
    ...
    void Init() { bzero(this, sizeof(SUMMARY_CALLSITE)); }
...
private:
    ...
    // index into the SUMMARY_SYMBOL array
    mINT32 _symbol_index;
...
#define IPL_FORMAL_LOOP_INDEX		0x04 // ref_parm used as a loop index,
    ...
    // the position of the formal parameter (starting from 0)
    mINT16 _position;

    // index into CFG_NODE_INFO::_formal
    // (PROJECT_REGION_ARRAY in ARRAY_SUMMARY)
    // describing dimensions (needed for array section analysis)
    mINT32 _region_index;

    // mtype of element of formal being passed
    // (NOT of the address being passed)
    mINT32 _machine_type;

    // Type info assoc. with this formal
    TY_IDX _ty;
...
/* Describe an actual parameter: its name, value (including jump functions)
 * and how it is passed if it has a symbol table entry.
 */

typedef enum ipa_pass_type
{
    // Actual is not a simple variable, so its pass type is not applicable.
    PASS_UNKNOWN = 0,
...
private:
    ...
    mINT32 _symbol_index;           // -1 for constant argument
    mINT32 _value_index : 24;       // index into the SUMMARY_VALUE array
    IPA_PASS_TYPE _pass_type : 8;

    // index into CFG_NODE_INFO::_param if type == PASS_ARRAY_SECTION, or
    // index into SCALAR_INFO for other types
    mINT32 _index;

    TY_IDX _ty;                     // type info assoc. with this actual

    BOOL _is_value_parm;            // is this actual passed by value?
...
enum IPA_CONST_TYPE
{
    ...
    VALUE_CONST = 3,		// is a symbol of CLASS_CONST
    ...
    VALUE_CHI = 9,		    // is const. if the chi function does not modify
                            // the value
    VALUE_CALLSITE = 10,	// used in expressions for execution cost of calls
    VALUE_NOT_CONST = 11,	// found to be *NOT* a constant
};

/**
 * The first two entries in the SUMMARY_VALUE array are always the constant
 * 0 and 1 respectively with mtype == I4.
 */
class SUMMARY_VALUE
{
private:
    ...
    union {
        ...
        // index into the SUMMARY_FORMAL array for pass-through jump functions
        mINT32 _formal_index;

        struct {
            // index into the SUMMARY_SYMBOL array for a static or global
            // variable (used only if IPA finds out that it is a constant.)
            mINT32 _global_index;
            // ST_IDX of a global symbol during IPA's constant propagation
            // phase
            ST_IDX _global_st_idx;
        } _global;

        // index into the SUMMARY_SYMBOL array for a variable passed as an
        // actual parameter and has undetermined type
        mINT32 _symbol_index;

        mINT32 _expr_index;         // index into the SUMMARY_EXPR array

        mINT32 _phi_index;          // index into the SUMMARY_PHI array

        mINT32 _chi_index;          // index into the SUMMARY_CHI array

        mINT32 _callsite_index;     // index into the SUMMARY_CALLSITE array
    ...
    mTYPE_ID _target_mtype;         // the mtype of what this value points to
                                    // (used only if _addr_of is true)

    // which of the above union is used
 ...
 *
 *          <var> <op> <int const>/<var>
 *
 * where <var> could be a variable or another expression. 
 *
 ****************************************************************************/
...
private:
    ...
    /* Describes which array _node_index points to: SUMMARY_VALUE, SUMMARY_PHI
     * or SUMMARY_EXR.
     */
    enum IDX_TYPE {
        EXPR_UNKNOWN = 0,
    ...
    union {
        ...
        struct {
            ...
            mINT32 _node_index : 24;
            IDX_TYPE _type : 6;

            mINT32 _kid : 2;		//  which kid is non-constant?
        } _s1;
...
public:
    ...
    void Set_kid (INT kid) { if (_has_const_operand) _u._s1._kid = kid; }
    INT Get_kid () const { return _has_const_operand ? _u._s1._kid : -1; }
    ...
    BOOL Is_expr_value (INT kid) const {
        return _has_const_operand ?
            (_u._s1._type == EXPR_VALUE) : (_u._s2[kid]._type == EXPR_VALUE);
    }
    ...
    BOOL Is_expr_phi (INT kid) const {
        return _has_const_operand ?
            (_u._s1._type == EXPR_PHI) : (_u._s2[kid]._type == EXPR_PHI);
    }
    ...
    BOOL Is_expr_expr (INT kid) const {
        return _has_const_operand ?
            (_u._s1._type == EXPR_EXPR) : (_u._s2[kid]._type == EXPR_EXPR);
    }
    ...
    BOOL Is_expr_chi (INT kid) const {
        return _has_const_operand ?
            (_u._s1._type == EXPR_CHI) : (_u._s2[kid]._type == EXPR_CHI);
    }
    ...
    INT32 Get_node_index (INT kid) const {
        return _has_const_operand ?
            _u._s1._node_index : _u._s2[kid]._node_index;
    } 
...
private:
    ...
    // WHIRL stmt reference
    // Which field is used when: SUMMARIZE<program>::Copy_summary_ctrl_dep
    union {
        WN *_w;                 // used in <cdg> mode (i.e. not real)
        mINT32 _map_id;         // used in real mode
    } u1;

    union {
        mINT32 _expr_index;		// index into the SUMMARY_EXPR array
        mINT32 _do_loop_index;  // index into the SUMMARY_DO_LOOP array
    } u2;

    mINT32 _true_stmt_index;    // list of reachable stmts that IPA cares
                                // if the expression is true
                                // the first index to the STMT summary
...
private:
    ...
    union {
        struct {
            mINT32 _findex;		    // index into the SUMMARY_FORMAL array
            mINT32 _addr_count;		// addr passed count for locals/formals
        } s;

        mINT64 _const_value;		// only use when IPL_CMOD bit is set
    ...
    union {  
        ...
        ST_IDX _st_idx_func; 		// parent function symbol: for locals
    } u2; 
...
public:
    ...
    mINT32 Get_common_index() const { return u1.c._common_index; }

    void Set_common_shape_index(INT index) {
        u1.c._common_shape_index = index;
    }
    mINT32 Get_common_shape_index() const { return u1.c._common_shape_index; }

    // addr passed count for locals and formals ONLY if their addr taken and
    // saved bit has not been set
...
/* mod/ref states, used by both SUMMARY_SYMBOL and SUMMARY_GLOBAL */

#define IPL_IMOD            0x0001  // indirect mod, i.e. *p
#define IPL_DMOD	        0x0002  // direct mod, i.e. p
#define IPL_IREF	        0x0004  // indirect ref, i.e. *p
#define IPL_DREF	        0x0008  // direct ref, i.e. p
#define IPL_AREF            0x0010  // address ref, i.e. &p
#define IPL_IKILL           0x0020  // indirect kill, i.e. *p
#define IPL_DKILL           0x0040  // direct kill, i.e. p
#define IPL_CMOD            0x0080  // direct assignment of a constant
#define IPL_MODCOUNT        0x0100  // mod count is non-zero
#define IPL_PARM            0x0200  // passed as a parameter to a routine
// ref types that will be used by copy propagation
#define IPL_COPY_REF        0x0400
// cref or dref only because the LDID is under a STID return-preg used by
// copy prop toreset addr passed bit
#define IPL_CDREF_PREG_ONLY 0x0800
// any ref, i.e. p, *p, or &p including indirect mod, kill
#define IPL_REF             0x003d
#define IPL_MODREF_ANY      0x007f  // any mod or ref
...
private:
    ...
    mINT32 _symbol_index;       // index into the SUMMARY_SYMBOL array
