...
// If we are not computing array section information, at least fill in the
// necessary header fields.
static void init_array_size_fields(SUMMARY_FILE_HEADER *header_addr)
{
...
void IPA_irb_write_summary(Output_File *fl)
{
    ...
    if (Summary->Has_procedure_entry()) {
        size = (Summary->Get_procedure_idx() + 1) * sizeof(SUMMARY_PROCEDURE);
    ...
    if (Summary->Has_common_shape_entry()) {
        ...
        offset_common_shape = (INT)ir_b_save_buf(Summary->Get_common_shape(0),
                size, sizeof(INT64), 0, fl); 
    ...
    if (Summary->Has_struct_access_entry()) {
        size = (Summary->Get_struct_access_idx() + 1) *
            sizeof(SUMMARY_STRUCT_ACCESS);

        offset_struct_access = (INT)ir_b_save_buf(
                Summary->Get_struct_access(0), size, sizeof(INT64), 0, fl);
        offset_struct_access = offset_struct_access - cur_sec_disp;
    }

    // Write the array summary to the file.
    if (Do_Par) Array_Summary_Output->Write_summary(fl, cur_sec_disp);
...
void IPA_Trace_Summary_Section (FILE *f,		// File to trace to
			   const void *sbase,
