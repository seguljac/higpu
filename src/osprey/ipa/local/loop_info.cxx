...
//
//     Mark all the DO_LOOPs and all the IFs in the function using the
//     DO_LOOP_INFO_BASE data structure and the IPL_info_map map. Use
//     IPL_loop_pool to store the annotations.
//
//  void IPL_Build_Access_Vectors(WN* func_nd)
//
//     Build access vectors for all the loops and array expressions that occur
//     in the given WHIRL node.
//
//--------------------------------------------------------------------------
...
/** DAVID CODE BEGIN **/
#include "ipl_access_vector.h"      // for actual building of access arrays
...
/** DAVID CODE BEGIN **/
static void IPL_Build_Access_Vectors(WN* wn, IPA_NODE *node,
        UINT& gdata_dir_id, DOLOOP_STACK *stack, MEM_POOL *pool);

static void IPL_Fill_Propagated_GPU_Data(IPA_NODE *node, DOLOOP_STACK *stack);
/*** DAVID CODE END ***/
...
static void Mark_Code(WN* wn,
        STACK_OF_WN* stack, DLI_BASE_STACK* dlistack, IF_STACK* if_stack,
        mUINT8 depth)
{
...
extern void IPL_Build_Access_Vectors(WN *func_nd)
{
    ...
    DOLOOP_STACK *stack = CXX_NEW(DOLOOP_STACK(&IPL_local_pool),
            &IPL_local_pool);

/** DAVID CODE BEGIN **/
    IPA_NODE *node = Summary->Get_ipa_node();

    // Fill PROJECTED_REGION fields in propagated HC_GPU_DATA.
    IPL_Fill_Propagated_GPU_Data(node, stack);

    UINT gdata_dir_id = 0;
    IPL_Build_Access_Vectors(func_nd,
            node, gdata_dir_id, stack, &IPL_loop_pool);
    Is_True(node == NULL
            || gdata_dir_id == node->get_gpu_data_list()->Elements(), (""));
/*** DAVID CODE END ***/
    ...
    if (Get_Trace(TP_IPL, TT_IPL_VERBOSE)) IPL_Print_Access(stderr, func_nd);
}
...
static void IPL_Build_Do_Access(WN* wn, DOLOOP_STACK *stack)
{
    ...
    FmtAssert(dli != NULL, ("Unmapped DO loop in IPL_Build_Do_Access"));

    MEM_POOL *pool = dli->Pool();

    // first the step
/** DAVID CODE BEGIN **/
    IPL_ACCESS_VECTOR *step = CXX_NEW(
            IPL_ACCESS_VECTOR(stack->Elements(),pool), pool);
/*** DAVID CODE END ***/

    stack->Push(wn);

/** DAVID CODE BEGIN **/
    dli->Set_step((ACCESS_VECTOR*)step);
/*** DAVID CODE END ***/
    dli->Set_lb(NULL);
    dli->Set_ub(NULL);
    WN *s = WN_step(wn);
    ...
    { 
        ...
        if (!step->Is_Const()) {
            // can't get a bound if we don't know sign of step
            goto return_point;
        }
...
/** DAVID CODE BEGIN **/
        IPL_ACCESS_ARRAY *lb = CXX_NEW(
                IPL_ACCESS_ARRAY(num_bounds,stack->Elements(),pool),pool);

        dli->Set_lb((ACCESS_ARRAY*)lb);

        // now actually do the building
        lb->Set_LB(WN_kid(l,0), stack, step->Const_Offset);
/*** DAVID CODE END ***/
        ...
        // now actually do the building
/** DAVID CODE BEGIN **/
        IPL_ACCESS_ARRAY *ub = CXX_NEW(
                IPL_ACCESS_ARRAY(num_bounds,stack->Elements(),pool),pool);
        dli->Set_ub((ACCESS_ARRAY*)ub);
        ub->Set_UB(u, stack);
/*** DAVID CODE END ***/
...
return_point:
    ...
    if (!dli->Get_lb()) {
/** DAVID CODE BEGIN **/
        ACCESS_ARRAY *lb= (ACCESS_ARRAY*)
            CXX_NEW(IPL_ACCESS_ARRAY(1,stack->Elements()+1,pool),pool);
/*** DAVID CODE END ***/
        dli->Set_lb(lb);
        lb->Too_Messy = TRUE;
    }
...
/*****************************************************************************
 *
 * Build the access array for the given array reference (ARRAY).
 *
 ****************************************************************************/

static void IPL_Build_Access_Array(WN *wn,
        DOLOOP_STACK *stack, MEM_POOL *pool)
{
/** DAVID CODE BEGIN **/
    IPL_ACCESS_ARRAY *array = CXX_NEW(
            IPL_ACCESS_ARRAY(WN_num_dim(wn), stack->Elements(), pool), pool);
/*** DAVID CODE END ***/
/** DAVID CODE BEGIN **/
    // printf("In IPL: Before Set_Array: Du_Mgr = %p\n", Summary->Get_du_mgr());
/*** DAVID CODE END ***/
    array->Set_Array(wn, stack);
/** DAVID CODE BEGIN **/
    // printf("In IPL: After Set_Array: Du_Mgr = %p\n", Summary->Get_du_mgr());
/*** DAVID CODE END ***/

    // Store it in a map for later use (process_actual_array_node).
...
/** DAVID CODE BEGIN **/
        IPL_ACCESS_ARRAY *cond = CXX_NEW(
                IPL_ACCESS_ARRAY(num_lors+1,stack->Elements(),pool),pool);
        cond->Set_IF(compare,stack,TRUE,FALSE,0);
        info->Condition = (ACCESS_ARRAY*)cond;
...
/** DAVID CODE BEGIN **/
        IPL_ACCESS_ARRAY *cond = CXX_NEW(
                IPL_ACCESS_ARRAY(num_lands+1,stack->Elements(),pool),pool);
        cond->Set_IF(compare,stack,FALSE,TRUE,0);
        info->Condition = (ACCESS_ARRAY*)cond;
        info->Condition_On_Then = !top_level_not;
/*** DAVID CODE END ***/
    }
...
/*****************************************************************************
 *
 * Fill the PROJECTED_REGION in the ALLOC section of <gdata>.
 *
 ****************************************************************************/

static void IPL_Fill_GPU_Data(HC_GPU_DATA *gdata, DOLOOP_STACK *stack,
        MEM_POOL *pool)
{
    if (gdata == NULL || !gdata->is_arr_section()) return;

    HC_ARRSECTION_INFO *hasi = gdata->get_alloc_section();

    // Convert the lower and upper bounds to ACCESS_ARRAYs.
    WN *lbnd_wn = hasi->get_lower_bound();
    IPL_ACCESS_ARRAY *lbnd_aa = CXX_NEW(IPL_ACCESS_ARRAY(
                WN_num_dim(lbnd_wn), stack->Elements(), pool), pool);
    lbnd_aa->Set_Array(lbnd_wn, stack);

    WN *ubnd_wn = hasi->get_upper_bound();
    IPL_ACCESS_ARRAY *ubnd_aa = CXX_NEW(IPL_ACCESS_ARRAY(
                WN_num_dim(ubnd_wn), stack->Elements(), pool), pool);
    ubnd_aa->Set_Array(ubnd_wn, stack);

    // Use these two ACCESS_ARRAYs to construct the PROJECTED_REGION.
    hasi->set_section((ACCESS_ARRAY*)lbnd_aa, (ACCESS_ARRAY*)ubnd_aa,
            stack->Elements());
}

/*****************************************************************************
 *
 * Fill the PROJECTED_REGION fields in all propagated annotations in <node>.
 *
 ****************************************************************************/
...
static void IPL_Fill_Propagated_GPU_Data(IPA_NODE *node, DOLOOP_STACK *stack)
{
    ...
    // We use a more permanent pool to allocate PROJECTED_REGION.
    MEM_POOL *pool = node->Mem_Pool();
            ...
            annot = annot_iter.Next())
    {
        ...
        for (UINT i = 0; i < n_formals; ++i)
        {
            ...
            if (gdata != NULL) IPL_Fill_GPU_Data(gdata, stack, pool);
        }
    }
}
...
/*****************************************************************************
 *
 * Parse the given GLOBAL/CONSTANT COPYIN pragma, which is in procedure
 * <parent_proc> with ID <pragma_id>. Use <stack> to construct the
 * PROJECTED_REGION for the ALLOC section.
 *
 ****************************************************************************/

static void IPL_Build_GPU_Data(WN *pragma_wn, IPA_NODE *node,
        UINT gdata_dir_id, DOLOOP_STACK *stack, MEM_POOL *pool)
{
    ...
    IPL_Fill_GPU_Data(gdata, stack, pool);

    WN_MAP_Set(IPL_info_map, pragma_wn, (void*)gdata);
}
...
/*****************************************************************************
 *
 * Walk through the given PU, and build the access vectors. Called by a
        ...
        DOLOOP_STACK *stack, MEM_POOL *pool)
{
    ...
    if (opr == OPR_BLOCK) {
        ...
        while (kid != NULL) {
/** DAVID CODE BEGIN **/
            IPL_Build_Access_Vectors(kid, node, gdata_dir_id, stack, pool);
/*** DAVID CODE END ***/
            kid = WN_next(kid);
        }
        return;
...
/** DAVID CODE BEGIN **/
    // We do not want to build them during IPL before IPA because pointer
    // promotion has not been done yet.
    if (node != NULL)
    {
        if (opr == OPR_PRAGMA || opr == OPR_XPRAGMA)
        {
            ...
            {
                IPL_Build_GPU_Data(wn, node, gdata_dir_id++, stack,
                        node->Mem_Pool());
            }
        }
    }
...
static void IPL_Print_One_Access(FILE *fp, WN* wn)
{
    ...
    {
/** DAVID CODE BEGIN **/
        IPL_ACCESS_ARRAY *array = (IPL_ACCESS_ARRAY*)
            WN_MAP_Get(IPL_info_map,wn);
/*** DAVID CODE END ***/
        if (trace_section)
        {
...
void DO_LOOP_INFO_BASE::Print(FILE *fp, INT indentation)
{
    char buf[80];
    INT i;

    for (i = 0; i < indentation && i < 79; i++) buf[i] = ' ';
    buf[i] = '\0';

    if (Has_calls()) fprintf(fp,"%sIt has calls \n", buf);
    if (Has_gotos())
        fprintf(fp,"%sIt has non-DO or non-IF controlflow\n", buf);
    if (Is_inner_loop()) fprintf(fp,"%sIs an inner loop \n", buf);
    fprintf(fp, "%sDepth is %d \n", buf, Get_depth());

    fprintf(fp,"%sThe lb is ", buf);
/** DAVID CODE BEGIN **/
    IPL_ACCESS_ARRAY *lb = (IPL_ACCESS_ARRAY*)Get_lb();
    if (lb != NULL)
        lb->Print(fp, TRUE);
    else 
        fprintf(fp, "<null>\n");
...
/** DAVID CODE BEGIN **/
    IPL_ACCESS_ARRAY *ub = (IPL_ACCESS_ARRAY*)Get_ub();
    if (ub != NULL)
        ub->Print(fp, TRUE);
    else 
        fprintf(fp, "<null>\n");
...
/** DAVID CODE BEGIN **/
    IPL_ACCESS_ARRAY *step = (IPL_ACCESS_ARRAY*)Get_step();
    if (step != NULL) {
        step->Print(fp);
        fprintf(fp, "\n");
    } else
