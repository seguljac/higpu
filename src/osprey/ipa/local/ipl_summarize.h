...
class SUMMARY_DESC
{
    ...
    public:
        ...
        void Set_convertible_to_global() {
            _convertible_to_global = TRUE;
        }
        void Reset_convertible_to_global() {
            _convertible_to_global = FALSE;
        }
        BOOL Is_convertible_to_global() const {
            return _convertible_to_global;
        }
...
private:
    ...
    // original entry point, for alt entry point processing.
    ...
    /* For each procedure, record if a SUMMARY_GLOBAL entry is created,
     * and if so, the index corresponds to that entry into the
     * SUMMARY_GLOBAL array */ 
...
/** DAVID CODE BEGIN **/
#ifdef HICUDA
    // the current IPA node being processed
    // If this field is set, data access summary (DAS) for kernels inside will
    // be constructed.
    IPA_NODE *_curr_proc_node;

    // a map from each WN node to the symbol of its parent kernel region
    WN_MAP _parent_kernel_map;

    // a map that links IPA_EDGEs with WN nodes
    // We do not want to use the one in <_curr_proc_node> because we want this
    // system to be separate from Map_Callsites.
    typedef HASH_TABLE<WN*,IPA_EDGE*> WN_TO_EDGE_MAP;
    WN_TO_EDGE_MAP *_wte_map;
...
/*** DAVID CODE END ***/
    ...
    SUMMARY_STRUCT_ACCESS* New_struct_access(
            mUINT32 ty_index, mUINT32 flatten_flds) {
        INT new_idx = _struct_access.Newidx();
        _struct_access[new_idx].Init(
                ty_index, flatten_flds, &reorder_ipl_pool);
...
/** DAVID CODE BEGIN **/
    void process_symbol_in_kregion(WN *wn);
    void clear_projected_regions();
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    ...
    void Set_ipa_node_context(IPA_NODE *node, WN_MAP parent_kernel_map)
    {
        _curr_proc_node = node;
        _parent_kernel_map = parent_kernel_map;
        _wte_map = NULL;
        _hc_ctrl_flags = 0;
    }

    IPA_NODE* Get_ipa_node() const { return _curr_proc_node; }
    WN_MAP Get_parent_kernel_map() const { return _parent_kernel_map; }
    WN_TO_EDGE_MAP* Get_wn_to_edge_map() const { return _wte_map; }

#endif  // HICUDA
/*** DAVID CODE END ***/

    SUMMARY_PROCEDURE* Get_procedure(INT id) const {
        return &(_procedure[id]);
    }
    SUMMARY_PROC_INFO* Get_proc_info(INT id) const {
        return &(_proc_info[id]);
    }
    SUMMARY_CALLSITE* Get_callsite(INT id) const {
        return &(_callsite[id]);
    }
    ...
    BOOL Has_inline_attr() const	{ return _inline_attr.Lastidx() != -1; }
...
/** DAVID CODE BEGIN **/
#ifdef HICUDA
        Set_ipa_node_context(NULL, WN_MAP_UNDEFINED);
#endif  // HICUDA
/*** DAVID CODE END ***/
    ...
    void Finish_collect_struct_access(void)
    {
        ...
        /* free Ptr_to_ty_vector, local_cands, flds[] of each
