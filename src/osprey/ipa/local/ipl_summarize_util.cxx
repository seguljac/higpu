...
/*****************************************************************************
 *
 * Given a CALL node, return the index into the CALLSITE array. This is stored
 * as a map (Summary_Map).
 *
 ****************************************************************************/
...
INT32 IPL_get_callsite_id(WN *w)
{
    ...
    FmtAssert(opr == OPR_CALL
            || opr == OPR_ICALL || opr == OPR_INTRINSIC_CALL,
            ("IPL_get_callsite_id: Expecting CALL node"));

    return WN_MAP32_Get(Summary_Map, w) - 1;
}
...
WN* IPL_get_stmt_scf(WN* av)
{
    ...
    while (1) {
        ...
        if (parent_w == NULL) break;

        if (OPCODE_is_stmt(WN_opcode(parent_w))
                || OPCODE_is_scf(WN_opcode(parent_w))) return parent_w;
    }
...
void Init_Aux_Symbol_Info(SYMTAB_IDX level)
{
    FmtAssert(Aux_Symbol_Info.size() >= level,
            ("Processing a nested procedure before it's parent"));
    
    // Delete vectors for previous PUs.
    while (Aux_Symbol_Info.size() > level) Aux_Symbol_Info.pop_back();

    typedef AUX_SYMBOL_INFO::value_type AUX_ST_INFO;

    // Create a new vector big enough to hold all symbols at the given level.
    AUX_ST_INFO new_level(ST_Table_Size(level));

    // Is this the normal way of vector insertion?
...
// in a nice order without duplications. So we build the whole thing in memory
// first, and then generate the summary info.
...
private:
    ...
    // number of STMT_CALL under _cd
    INT _true_call_count;
    INT _false_call_count;

    /* head_of_chain: For each chain of control dependency, it should end up
     * control dependent on the entry of the function. In some cases, we give
     * up following the chain when the conditional expression is too
     * complicated. Thus, we might end up with multiple chains, and we set
     * head_of_chain to true for the cd node of the beginning of each chain.
     * In main IPA, we do a forward pass through all the conditional
     * expression and mark dead stmts accordingly. We would like to evaluate
     * each condition AFTER all its control dependent nodes. Thus, we need to
     * write out each node in the right order.
     */
    BOOL _head_of_chain;

    INT ctrl_index;             // index to the cd node in the summary info
 ...
 */
inline SUMMARY_STMT* CTRL_DEP::Get_new_stmt(BOOL branch, WN *w)
{
    INT idx;
...
inline SUMMARY_STMT* CTRL_DEP::Get_last_stmt(BOOL branch)
{
    if (branch == TRUE) {
        return (_cd.Get_true_count() == 0) ? NULL :
            &(_true_stmts[_true_stmts.Lastidx()]);
    } else {
        return (_cd.Get_false_count() == 0) ? NULL :
            &(_false_stmts[_false_stmts.Lastidx()]);
    }
...
/************************************************************************
 *
 * Utility functions for the control dependence graph.
 *
 ************************************************************************/

void Init_cdg(MEM_POOL *m)
{
    CTRL_DEP_mem = m;
...
INT Get_stmt_id(void)
{
    return stmt_id;
}

/**
 * Reset the cd and stmt strucutres to the saved checkpoint. All CD nodes with
 * IDs larger than 'saved_cd_idx' and all statements with IDs larger than
 * 'saved_stmt_id' are removed from the corresponding structures.
 */
void Reset_cd_and_stmt(WN_MAP Summary_Map,
        INT saved_cd_idx, INT saved_stmt_id)
{
    // This check is an optimization.
    if (stmt_id > saved_stmt_id)
    {
        INT cd_idx = cdg->Lastidx();
        if (cd_idx > saved_cd_idx) cd_idx = saved_cd_idx;

        for ( ; cd_idx >= 0; --cd_idx)
        {
            CTRL_DEP &p = (*cdg)[cd_idx];

            // Remove all "true" statements with an ID > saved_stmt_id
            ...
            if (need_to_reset) p.Reset_true_stmts(stmt_idx);

            // Remove all "false" statements with an ID > saved_stmt_id
            ...
            if (need_to_reset) p.Reset_false_stmts(stmt_idx);
        }
    }

    while (saved_cd_idx < cdg->Lastidx()) {
        WN_MAP32_Set(Summary_Map,
                (*cdg)[cdg->Lastidx()].Get_cd()->Get_wn(), 0);
        cdg->Decidx();
    }
} // Reset_cd_and_stmt


SUMMARY_CONTROL_DEPENDENCE* Get_new_cd(void)
{
    INT idx = cdg->Newidx();
    (*cdg)[idx].Init();
...
SUMMARY_CONTROL_DEPENDENCE* Get_cd_by_idx(INT idx)
{
    return (idx <= cdg->Lastidx()) ? (*cdg)[idx].Get_cd() : NULL;
} // Get_cd_by_idx
...
INT Get_cd_idx(SUMMARY_CONTROL_DEPENDENCE *cd)
{
    return cdg->Idx((CTRL_DEP*)cd);
}

void Inc_cd_call_count(INT idx, BOOL branch)
{
    (*cdg)[idx].Inc_call_count(branch);
}

INT Get_cd_call_count(INT idx)
{
    return (*cdg)[idx].Get_call_count();
}

INT Get_cd_call_count(INT idx, BOOL branch)
{
    return (*cdg)[idx].Get_call_count(branch);
}

void Set_cd_head_of_chain(INT idx)
{
    (*cdg)[idx].Set_head_of_chain();
} // Set_cd_head_of_chain

void Set_cd_ctrl_index(SUMMARY_CONTROL_DEPENDENCE *cd, INT index)
{
    INT idx = cdg->Idx((CTRL_DEP*)cd);
    (*cdg)[idx].Set_ctrl_index(index);
} // Set_cd_ctrl_index

INT Get_cd_real_idx(SUMMARY_CONTROL_DEPENDENCE *cd)
{
    INT idx = cdg->Idx((CTRL_DEP*)cd);
    return (*cdg)[idx].Get_ctrl_index();
} // Get_cd_real_idx

SUMMARY_STMT* Get_new_stmt(INT idx, BOOL branch, WN *w)
{
    return (*cdg)[idx].Get_new_stmt(branch, w);
} // Get_new_stmt

SUMMARY_STMT* Get_last_stmt(INT idx, BOOL branch)
{
 ...
 * in a cyclic fashion.
 */
SUMMARY_CONTROL_DEPENDENCE* Get_next_cd_chain(void)
{
    iterator_idx++;
...
SUMMARY_CONTROL_DEPENDENCE* Get_controlling_stmt(WN *stmt_node)
{
    INT idx = WN_MAP32_Get(Stmt_Map, stmt_node) - 1;
    if (idx < 0 || idx > cdg->Lastidx()) return NULL;

    return (*cdg)[idx].Get_cd();
} // Get_controlling_stmt


SUMMARY_STMT* Search_for_summary_stmt(WN *stmt_node,
        BOOL &branch, INT &stmt_idx)
{
    INT idx = WN_MAP32_Get(Stmt_Map, stmt_node) - 1;
    ...
    if ((count = cd->Get_false_count()) > 0) {
        wn_array = (*cdg)[idx].Get_false_wns ();
        for (INT i = 0; i < count; i++) {
            if (wn_array[i] == stmt_node) {
                branch = FALSE;
                stmt_idx = i;
