...
Propagate_Constants (IPA_NODE* node, WN* w, VALUE_DYN_ARRAY* cprop_annot)
{
    ...
    if (WN_first(block)) {
/** DAVID CODE BEGIN **/
	IPO_LWN_Insert_Block_Before(WN_func_body(w), WN_first(WN_func_body(w)),
				block);
/*** DAVID CODE END ***/
    }
...
void IPA_Propagate_Constants(IPA_NODE* n, BOOL delete_const_param)
{
    ...
    if (cprop_annot == NULL || cprop_annot == (VALUE_DYN_ARRAY*)-1) return;
    ...
    for (i = 0; i < WN_num_formals(w); ++i) {
        if ((*cprop_annot)[i].Is_remove_param()) --param_count;
    }

    if (param_count == WN_num_formals(w)) return;

    // create the necessary opcode
    WN* func_node = WN_CreateEntry (param_count, WN_st_idx(w),
            WN_func_body(w), WN_func_pragmas(w), WN_func_varrefs(w));
...
/** DAVID CODE BEGIN **/
    WN* parent_block = LWN_Get_Parent(call);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
	    LWN_Set_Parent(WN_kid(call,i), new_call);
/*** DAVID CODE END ***/
	    ++first;
	} else {
...
/** DAVID CODE BEGIN **/
	IPO_LWN_Insert_Block_Before (parent_block, call, new_call);
	IPO_LWN_Extract_From_Block (parent_block, call);
/*** DAVID CODE END ***/
    } else {
...
extern void IPO_propagate_globals(IPA_NODE *n)
{
#if 0
    ...
    if (gannot == NULL || modref_info == NULL) return;
    ...
    {
        if (gannot->Top(i) || gannot->Bottom(i)) continue;
        ...
        for (UINT32 j = 0; j < gvals.Elements(); ++j)
        {
            ...
            if (st_idx == ST_IDX_ZERO) continue;
            if (TY_mtype(ST_type(st_idx)) != value->Get_mtype()) continue;
            ...
            if (modref_info->Is_def_elmt(modref_key)) {
                Generate_Assignment_Stmt(ST_ptr(st_idx), new_block,
                        Gen_WN_Const(*value));
            } else {
    ...
    if (need_to_update_array_bounds) Update_array_bounds(pu);

    // If there are assignment statements then stick them into the tree.
    if (WN_first(new_block)) {
/** DAVID CODE BEGIN **/
        IPO_LWN_Insert_Block_Before(WN_func_body(pu),
                WN_first(WN_func_body(pu)), new_block);
/*** DAVID CODE END ***/
    }

/** DAVID CODE BEGIN **/
    IPO_LWN_Parentize(pu);
/*** DAVID CODE END ***/
    WN_verifier(pu);
