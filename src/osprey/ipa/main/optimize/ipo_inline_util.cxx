...
/** DAVID CODE BEGIN **/
#include "ipo_lwn_util.h"           // for LWN_Get_Parent(), etc
/*** DAVID CODE END ***/
#ifdef KEY
...
void Fix_Return_Pregs (WN *Call, const RETURN_PREG& rp)
{
    ...
    if (node != NULL) {
        ...
        Is_True(parent != NULL, ("corrupted parent map"));
        WN *grandparent = LWN_Get_Parent(parent);
        if (grandparent != NULL && WN_operator (grandparent) == OPR_COMMA) {
...
Get_enclosing_region (IPA_NODE * n, IPA_EDGE * e)
{
    ...
    for (; parent != NULL;
            parent = WN_Get_Parent(parent, Caller_Parent_Map, Caller_Map_Tab))
    {
