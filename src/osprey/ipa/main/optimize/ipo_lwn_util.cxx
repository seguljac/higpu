 ...
 */
extern WN* IPO_LWN_Get_Next_Tree_Node (const WN* wn) {
  WN* ret_wn = NULL;
 ...
 */
extern WN* IPO_LWN_Get_Next_Stmt_Node (const WN* wn) {
  WN* ret_wn = NULL;
 ...
 */
extern WN* IPO_LWN_Get_Next_SCF_Node (const WN* wn) {
  WN* ret_wn = NULL;
...
void IPO_LWN_Insert_Block_Before (WN *block, WN *wn, WN *in)
{
  WN *first;
  WN *last;
  WN *w;

  Is_True(wn || block, ("IPO_LWN_Insert_Block_Before: nowhere to insert"));

  if (wn) {
    WN *parent_wn = LWN_Get_Parent(wn);
    Is_True (parent_wn, ("IPO_LWN_Insert_Block_Before: parent is NULL\n"));
  
    if (block == NULL)
      block = parent_wn;
    else {
      Is_True(block == parent_wn,
	      ("IPO_LWN_Insert_Block_Before: supplied block not parent"));
    }
  }

  Is_True(!block || (WN_opcode(block) == OPC_BLOCK),
	  ("IPO_LWN_Insert_Block_Before: block is not a BLOCK node"));
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("IPO_LWN_Insert_Block_Before: insert node 0x%p not a stmt/scf\n", wn));
  Is_True((in != NULL),
	  ("IPO_LWN_Insert_Block_Before: insert a NULL node?"));
  ...
  if (wn) {
    ...
    Is_True (kids, ("IPO_LWN_Insert_Block_Before: could not find node 'wn'"));
  }
#endif
...
void IPO_LWN_Insert_Block_After (WN *block, WN *wn, WN *in)
{
  WN *first;
  WN *last;
  WN *w;

  Is_True(wn || block, ("IPO_LWN_Insert_Block_After: nowhere to insert"));

  if (wn) {
    WN *parent_wn = LWN_Get_Parent(wn);
    Is_True (parent_wn, ("IPO_LWN_Insert_Block_After: parent is NULL\n"));
  
    if (block == NULL)
      block = parent_wn;
    else {
      Is_True(block == parent_wn,
	      ("IPO_LWN_Insert_Block_After: supplied block not parent"));
    }
  }

  Is_True(!block || (WN_operator(block) == OPR_BLOCK),
	  ("IPO_LWN_Insert_Block_After: block is not a BLOCK node"));
  Is_True(!wn || OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
	  ("IPO_LWN_Insert_Block_After: insert node 0x%p not a stmt/scf\n", wn));
  Is_True((in != NULL),
	  ("IPO_LWN_Insert_Block_After: insert a NULL node?"));
  Is_True(OPCODE_is_stmt(WN_opcode(in)) || OPCODE_is_scf(WN_opcode(in)), 
	  ("IPO_LWN_Insert_Block_After: insert node 0x%p not a stmt/scf\n", in));
  ...
  if (wn) {
    ...
    Is_True (kids, ("IPO_LWN_Insert_Block_After: could not find node 'wn'"));
  }
#endif
...
WN* IPO_LWN_Extract_From_Block(WN* item)
{
...
WN* IPO_LWN_Extract_From_Block(WN* parent, WN* item)
{
  FmtAssert(LWN_Get_Parent(item) == parent,
	    ("Bad parent for IPO_LWN_Extract_From_Block"));
 ...
 */
extern void IPO_LWN_Delete_From_Block(WN *block, WN* wn) {
  WN *node;
  WN* parent_wn;
  ...
  Is_True (wn, ("IPO_LWN_DeleteFromBlock: deleting a NULL node"));
  Is_True (((!block) || (WN_opcode(block) == OPC_BLOCK)),
           ("IPO_LWN_DeleteFromBlock: Expecting a BLOCK node"));
  parent_wn = LWN_Get_Parent(wn);
  Is_True (((!block) || (block == parent_wn)),
           ("IPO_LWN_DeleteFromBlock: block is not the parent"));
  
  /* make sure the operators are legal */
  Is_True (OPCODE_is_stmt(WN_opcode(wn)) || OPCODE_is_scf(WN_opcode(wn)),
           ("IPO_LWN_DeleteFromBlock: Expecting a SCF or a stmt node"));
  ...
  FmtAssert (node != NULL,
             ("IPO_LWN_DeleteFromBlock: could not find node to delete"));
...
extern void IPO_LWN_Delete_Tree_From_Block(WN* wn) {
  IPO_LWN_Delete_Tree (IPO_LWN_Extract_From_Block(wn));
}

static WN* IPO_LWN_Copy_Tree_R(WN *,BOOL,WN_MAP,BOOL,WN_MAP,STACK_OF_WN *,
			   BOOL); 

/* copy a tree */
/* if copy_access is TRUE, create copies of all the access vectors */
extern WN* IPO_LWN_Copy_Tree(WN *wn, BOOL copy_access, WN_MAP access_map,
...
#endif
  WN *result = 
    IPO_LWN_Copy_Tree_R(wn,copy_access,access_map,copy_version,version_map,
...
static  WN* IPO_LWN_Copy_Tree_R(WN *wn, BOOL copy_access, WN_MAP access_map,
			    BOOL copy_version, WN_MAP version_map,
			    STACK_OF_WN *region_stack,
			    BOOL copy_all_nodes) {
  WN* ret_wn = NULL;
  ...
  if (wn) {
    ...
    IPO_LWN_Copy_Linenumber(wn, ret_wn);
    ...
    if (copy_access) {  // copy the access vectors
      if (oper == OPR_ARRAY) {
        ...
        if (LNO_enabled) {
          Is_True (array, ("IPO_LWN_Copy_Tree missing array-access-map"));
        }
        if (array) {
        ...
        if (LNO_enabled) {
          Is_True (dli, ("IPO_LWN_Copy_Tree missing do-loop access-map"));
        }
        if (dli) {
        ...
        if (LNO_enabled) {
          Is_True (ii, ("IPO_LWN_Copy_Tree missing if-access-map"));
        }
        if (ii) {
    ...
    Is_True(copy_access == FALSE,
	    ("Compile with -DLNO to copy access vectors in IPO_LWN_Copy_Tree_R"));
#endif
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
      ...
      kid = WN_first (wn);
      if (kid) {
        ret_kid = IPO_LWN_Copy_Tree_R (kid,
      ...
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        if (WN_kid (wn, kidno)) { 
          ret_kid = IPO_LWN_Copy_Tree_R (WN_kid (wn, kidno), 
                                     copy_access, access_map,
                                     copy_version, version_map,
				     region_stack,
				     copy_all_nodes);
...
void IPO_LWN_Copy_Def_Use_Node(WN* from_node, WN* to_node, DU_MANAGER* du)
{ 
...
void IPO_LWN_Copy_Def_Use(WN* from_exp, WN* to_exp, DU_MANAGER* du)
{
  OPCODE fop = WN_opcode(from_exp);
  OPCODE top = WN_opcode(to_exp);

  FmtAssert(fop == top && OPCODE_is_expression(fop),
	    ("Opcodes unequal IPO_LWN_Copy_Def_Use(%d,%d) or not expr", fop, top));

  for (INT k = 0; k < WN_kid_count(from_exp); k++)
    IPO_LWN_Copy_Def_Use(WN_kid(from_exp,k), WN_kid(to_exp,k), du);

  IPO_LWN_Copy_Def_Use_Node(from_exp, to_exp, du);
 ...
 */
BOOL IPO_LWN_Check_Parentize (const WN* wn) {
    BOOL isok = TRUE;
    if (!OPCODE_is_leaf (WN_opcode (wn))) { 
        ...
        if (!isok) break;
        isok &= IPO_LWN_Check_Parentize (kid);
        if (!isok) break;
        kid = WN_next (kid);
      }
    } else if (WN_opcode(wn) == OPC_REGION) {
      isok &= (LWN_Get_Parent(WN_region_body(wn)) == wn);
      isok &= IPO_LWN_Check_Parentize (WN_region_body(wn));
    } else {
      INT kidno;
      WN* kid;
      for (kidno=0; kidno<WN_kid_count(wn); kidno++) {
        ...
        if (kid) { 
          Is_True ((LWN_Get_Parent (kid) == wn),
                   ("IPO_LWN_Check_Parentize(): kid 0x%p (%s), parent 0x%p (%s)\n",
...
// only.
void IPO_LWN_Parentize_One_Level (const WN* wn) {
   INT i;
 ...
 */
void IPO_LWN_Delete_Tree(WN *wn)
{
    if (wn == NULL) return;
    ...
    if (WN_opcode(wn) == OPC_BLOCK) {
        ...
        for (INT i = 0; i < WN_kid_count(wn); i++) {
            IPO_LWN_Delete_Tree(WN_kid(wn,i));
        }
    }
    ...
    if (parent && WN_operator(parent) == OPR_BLOCK) {
        IPO_LWN_Delete_From_Block(parent,wn);
    } else {
        WN_Delete(wn);
    }
}
...
void IPO_LWN_Delete_LNO_dep_graph(WN *wn)
{
...
void IPO_LWN_Delete_CG_dep_graph(WN *wn)
{
...
void IPO_LWN_Update_Def_Use_Delete_Tree(WN *wn, DU_MANAGER* du)
{
  if (du == NULL)
    du = Du_Mgr;
  ...
  if (opcode == OPC_BLOCK) {
    WN* kid = WN_first (wn);
    while (kid) {
      IPO_LWN_Update_Def_Use_Delete_Tree(kid, du);
      kid = WN_next(kid);
    }
  } 
  ...
  for (INT i = 0; i < WN_kid_count(wn); i++) {
    IPO_LWN_Update_Def_Use_Delete_Tree(WN_kid(wn,i), du);
  }
}

void IPO_LWN_Update_Dg_Delete_Tree(WN *wn, ARRAY_DIRECTED_GRAPH16* dg)
{
  FmtAssert(wn, ("IPO_LWN_Update_Dg_Delete_Tree null wn!"));
  ...
  if (WN_opcode(wn) == OPC_BLOCK) {
    ...
    if (kid != NULL) {
      ...
      kid = WN_next(kid);
      while (kid != NULL) {
	IPO_LWN_Update_Dg_Delete_Tree(prev, dg);
	prev = kid;
	kid = WN_next(kid);
      }
      IPO_LWN_Update_Dg_Delete_Tree(prev, dg);
    ...
    for (INT i = 0; i < WN_kid_count(wn); i++) {
      IPO_LWN_Update_Dg_Delete_Tree(WN_kid(wn,i), dg);
    }
  }
...
WN *IPO_LWN_CreateDO(WN *index, WN *start, WN *end, WN *step,
		       WN *body)
{
  WN *loop_info = IPO_LWN_CreateLoopInfo(IPO_LWN_Copy_Tree(index),NULL,0,0,0);
...
WN *IPO_LWN_CreateLoopInfo(WN *induction, WN *trip, UINT16 trip_est,
...
WN *IPO_LWN_CreateDoWhile(WN *test, WN *body)
{
...
WN *IPO_LWN_CreateWhileDo(WN *test, WN *body)
{
...
WN *IPO_LWN_CreateIf(WN *test, WN *if_then, WN *if_else)
{
...
WN *IPO_LWN_CreateCondbr(INT32 label_number,WN *exp)
{
...
WN *IPO_LWN_CreateReturn()
{
   WN* wn = WN_CreateReturn();
   return wn;
}

WN *IPO_LWN_CreateCompgoto(INT32 num_entries, WN *value,
			WN *block, WN *deflt)
{
...
#ifndef KEY
WN *IPO_LWN_CreateIstore(OPCODE opc, WN_OFFSET offset,
...
WN *IPO_LWN_CreateMstore(WN_OFFSET offset, TY_IDX ty, WN *value,
...
WN *IPO_LWN_CreateStid(OPCODE opc, WN_OFFSET offset, 
...
WN *IPO_LWN_CreateStid(OPCODE opc, WN *orig_op, WN *value)
{
  Is_True((WN_operator(orig_op) == OPR_LDID) ||
          (WN_operator(orig_op) == OPR_STID),
	  ("Illegal orig_op in IPO_LWN_Create_Stid"));
...
WN *IPO_LWN_CreateLdid(OPCODE opc, WN *orig_op)
{
  Is_True(orig_op, ("IPO_LWN_CreateLdid() called will null orig_op"));
  Is_True((WN_operator(orig_op) == OPR_LDID) ||
          (WN_operator(orig_op) == OPR_STID),
	  ("Illegal orig_op in IPO_LWN_Create_Ldid"));
...
WN *IPO_LWN_CreateDivceil(TYPE_ID type, WN *kid0, WN *kid1)
{
  ...
  for (i = 0; i < nkids; i++) {
    if (WN_operator(kids[i]) != OPR_PARM) {
      ...
      kids[i] = IPO_LWN_CreateParm(type, kids[i], Be_Type_Tbl(type), 
			       WN_PARM_BY_VALUE);
    }
  }
  ...
  IPO_LWN_Parentize_One_Level(wn);

  return (wn);
}


WN *IPO_LWN_CreateDivfloor(TYPE_ID type, WN *kid0, WN *kid1)
{
  OPCODE op = OPCODE_make_op(OPR_INTRINSIC_OP, type, MTYPE_V);
  INT i;
  WN *kids[2];
...
WN *IPO_LWN_CreateExp1(OPCODE opc, WN *kid0)
{
...
WN *IPO_LWN_CreateExp2(OPCODE opc, WN *kid0, WN *kid1)
{
...
#ifndef KEY
WN *IPO_LWN_CreateIload(OPCODE opc, WN_OFFSET offset, 
...
WN *IPO_LWN_CreateMload(WN_OFFSET offset, TY_IDX ty,WN *addr,
		    WN *num_bytes)
{
...
WN *IPO_LWN_CreateCvtl(OPCODE opc, INT16 cvtl_bits, WN *kid0)
{
...
WN *IPO_LWN_CreateParm(TYPE_ID rtype, WN *parm_node, TY_IDX ty, UINT32 flag)
{
...
#ifdef LNO
WN *IPO_LWN_CreatePrefetch(WN_OFFSET offset, UINT32 flag, WN* addr) {
#ifdef KEY
  Num_Prefetches ++;
#endif
...
WN* IPO_LWN_Int_Type_Conversion(WN *wn, TYPE_ID to_type)
{
...
extern WN *IPO_LWN_Loop_Trip_Count(const WN *loop)
{
  ...
  WN *new_lb = IPO_LWN_Copy_Tree(lb);
  WN *new_ub = IPO_LWN_Copy_Tree(ub);
  if (Du_Mgr) {
    IPO_LWN_Copy_Def_Use(lb,new_lb,Du_Mgr);
    IPO_LWN_Copy_Def_Use(ub,new_ub,Du_Mgr);
  }
  trip_cnt = IPO_LWN_CreateExp2( OPCODE_make_op(OPR_SUB,trip_mtype,MTYPE_V),
			    new_ub,new_lb);
  if (OPCODE_operator(ub_compare) != OPR_GT &&
      OPCODE_operator(ub_compare) != OPR_LT)
    trip_cnt = IPO_LWN_CreateExp2( OPCODE_make_op(OPR_ADD,trip_mtype,MTYPE_V),
			      trip_cnt, IPO_LWN_Copy_Tree(incr) );
  trip_cnt = IPO_LWN_CreateExp2( OPCODE_make_op(OPR_DIV,trip_mtype,MTYPE_V),
			    trip_cnt, IPO_LWN_Copy_Tree(incr) );
...
{
  if (Cur_PU_Feedback) {
    IPO_LWN_ITER* wniter = IPO_LWN_WALK_StmtIter( (WN *) wn );
    while (wniter) {
      WN *cur = wniter->wn;
      wniter = IPO_LWN_WALK_TreeNext(wniter);
...
#else
void IPO_LWN_Set_Frequency_Tree(const WN*, const INT32) {}
void IPO_LWN_Copy_Frequency_Tree(const WN*, const WN *) {}
void IPO_LWN_Adjust_Frequency_Tree(const WN*, const INT32) {}
void IPO_LWN_Scale_Frequency_Tree(const WN*, const float) {}
