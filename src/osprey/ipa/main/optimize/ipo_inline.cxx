...
Create_array(WN* base)
{
  ...
  for (i=0; i< ndims; ++i) {  
    ...
    opc1 = OPC_I4SUB;
/** DAVID CODE BEGIN **/
    size_wn = IPO_LWN_CreateExp2( opc1, upper, lower);
/*** DAVID CODE END ***/
    opc1 = OPC_I4INTCONST;
    one = WN_CreateIntconst(opc1, 1);
    opc1 = OPC_I4ADD;
/** DAVID CODE BEGIN **/
    size_wn = IPO_LWN_CreateExp2(opc1, size_wn, one);
/*** DAVID CODE END ***/
    WN_array_dim(array_wn,i) = size_wn;
    opc1 = OPC_I4INTCONST;
...
/** DAVID CODE BEGIN **/
	IPO_LWN_Insert_Block_Before (copy_in_block, NULL, copy_stmt.first);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
  IPO_LWN_Insert_Block_Before(LWN_Get_Parent(Call_Wn ()), Call_Wn (),
			  intrinsic_call);

  WN* rreg_ldid = WN_CreateLdid(ldop, rreg1, ST_st_idx(rst), pty);
  sp_tmp_def = IPO_LWN_CreateStid(stop, saved_sp, MTYPE_To_PREG(Pointer_type),
			      pty, rreg_ldid);

  IPO_LWN_Insert_Block_Before(LWN_Get_Parent(Call_Wn()), Call_Wn(), sp_tmp_def);
/*** DAVID CODE END ***/
  return saved_sp;
}
...
/** DAVID CODE BEGIN **/
  IPO_LWN_Insert_Block_After(
          LWN_Get_Parent(Call_Wn()), Call_Wn(), intrinsic_call);
/*** DAVID CODE END ***/
}
...
/** DAVID CODE BEGIN **/
    WN* parent_wn = LWN_Get_Parent(Call);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
	IPO_LWN_Insert_Block_Before(copy_in_block, NULL, copy_wn);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    IPO_LWN_Insert_Block_Before(aux.copy_in_block, NULL, copy_in);
/*** DAVID CODE END ***/
...
/** DAVID CODE BEGIN **/
    IPO_LWN_Insert_Block_After(aux.copy_out_block, NULL, copy_wn);
/*** DAVID CODE END ***/
...
IPO_INLINE::Walk_and_Update_Callee (IPO_INLINE_AUX& aux)
{
    ...
    WN_Parentize(aux.inlined_body,
            Caller_node()->Parent_Map(), Caller_Map_Table());
