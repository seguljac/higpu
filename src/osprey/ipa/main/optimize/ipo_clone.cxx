...
/** DAVID CODE BEGIN **/
void IPO_Clone(IPA_NODE *orig_node, IPA_NODE *clone_node, STR_IDX clone_name)
/*** DAVID CODE END ***/
{
...
/*** DAVID CODE END ***/
    {
        ...
        char *clone_name_str = (char*)alloca(strlen(orig_node->Name())+15);
        sprintf(clone_name_str, "%s..clone..%u", ST_name(orig_st), clone_num);
        clone_name = Save_Str(clone_name_str);
    }

    ST *clone_st = New_ST(ST_level(orig_st));
    ST_Init(clone_st, clone_name,
            CLASS_FUNC, SCLASS_TEXT, ST_export(orig_st), clone_pu_idx);
