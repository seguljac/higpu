...
void Rename_Call_To_Cloned_PU(IPA_NODE *caller, IPA_NODE *callee,
        IPA_EDGE *e, IPA_CALL_GRAPH *cg)
{
    // switch to the caller context
    IPA_NODE_CONTEXT context(caller);

    // map callsites to WN nodes
...
static void Perform_Transformation(IPA_NODE* caller, IPA_CALL_GRAPH* cg)
{
    ...
    if (IPA_Space_Access_Mode == SAVE_SPACE_MODE &&
            caller->Get_partition_group() != partition_num) return;

    if (IPA_Enable_Recycle) MEM_POOL_Push(&Recycle_mem_pool);
#endif
...
#ifdef KEY
        if (caller->Is_Recursive()) callee->Set_Undeletable();
#endif
...
static void IPO_main(IPA_CALL_GRAPH *cg)
{
    ...
    if (IPA_Enable_Padding) IPO_Pad_Symtab(IPA_Common_Table);

    if (IPA_Enable_Split_Common) IPO_Split_Common();
    ...
    if (IPA_Enable_EH_Region_Removal) {
        // Remove EH regions that are not required
        IPA_Remove_Regions (walk_order, cg);
    }
#endif
...
/*** DAVID CODE END ***/
    ...
    // PU reordering heuristics
    {
        ...
        {	// reorder by node frequency
            ...
            while (!emit_order.empty())
            {
                ...
                if (!PU_Written(cg->Graph(), emit, &emit->File_Header()))
                {
                    // bug 4668
                    check_for_nested_PU(emit);
                    IPA_Rename_Builtins(emit);
    ...
    if (IPA_Enable_Reorder) IPO_Finish_reorder();
    //MEM_POOL_Pop (&reorder_local_pool);pop reorder_candidate
...
//-----------------------------------------------------------------------
static void BE_Symtab_Initialize()
{
    if (BE_symtab_initialized) return;

    BE_symtab_initialize_be_scopes();
    BE_symtab_alloc_scope_level(GLOBAL_SYMTAB);

    for (SYMTAB_IDX level = 0; level <= GLOBAL_SYMTAB; ++level)
    {
        // No need to deal with levels that don't have st_tab's. Currently
        // this should be only zero.
        if (Scope_tab[level].st_tab != NULL) {
            Scope_tab[level].st_tab->Register(*Be_scope_tab[level].be_st_tab);
        } else {
            Is_True(level == 0, ("Nonexistent st_tab for level %d", level));
        }
    }
...
//-----------------------------------------------------------------------
static void BE_Symtab_Finalize()
{
    if (!BE_symtab_initialized) return;
    ...
    for (SYMTAB_IDX idx = GLOBAL_SYMTAB + 1; idx > 0; --idx)
    {
        // No need to deal with levels that don't have st_tab's. Currently
        // this should be only zero.
        SYMTAB_IDX level = idx - 1;
        if (Scope_tab[level].st_tab != NULL) {
            Scope_tab[level].st_tab->Un_register(
                    *Be_scope_tab[level].be_st_tab);
            Be_scope_tab[level].be_st_tab->Clear();
        } else {
            Is_True(level == 0, ("Nonexistent st_tab for level %d", level));
        }
    }
...
void Perform_Interprocedural_Optimization(void)
{
    ...
    if (IPA_Enable_ipacom) ipa_compile_init();

#ifdef TODO
    // should move to analysis phase
    if (IPA_Enable_Feedback) setup_IPA_feedback();
#endif
    ...
    // Must be called before any of the output files are written, since symtab
    // must be compiled first.
#ifdef Is_True_On
    CGB_IPA_Initialize(IPA_Call_Graph);
#endif

    if (IPA_Enable_AutoGnum) Autognum_Driver();

    if (IPA_Enable_ipacom) ipacom_process_symtab(IP_global_symtab_name());
    ...
    if (IPA_Enable_Alias_Class) Ip_alias_class->Init_maps();
