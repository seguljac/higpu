...
/*****************************************************************************
 *
 * state information (used in GLOBAL_ARRAY_INFO)
 *
 ****************************************************************************/
...
#define STATE_IS_MUST_KILL  0x00000001
#define STATE_IS_MAY_KILL   0x00000002
#define STATE_IS_EUSE       0x00000004
#define STATE_IS_USE        0x00000008
#define STATE_IS_MUST_REDUC 0x00000010
#define STATE_IS_MAY_REDUC  0x00000020
#define STATE_IS_SCALAR     0x00000040
#define STATE_IS_REMOVED    0x00000080
...
/** DAVID CODE BEGIN **/
     ...
     */
    void ShallowCopy(const STATE *other);
...
/*****************************************************************************
 *
 * This class describes the mod/ref sections for a global array. It is the
 * node type of GLOBAL_ARRAY_LIST.
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * A list containing sections for the elements in a common block
 *
...
private:
    ...
    // DAVID COMMENT: is this the common block symbol?
    ST_IDX _st_idx;
    UINT _is_messy : 1;
...
public:
    ...
    GLOBAL_ARRAY_INFO* Append(ST_IDX st_idx, MEM_POOL* pool) {
        GLOBAL_ARRAY_INFO *gai = CXX_NEW(GLOBAL_ARRAY_INFO(st_idx), pool);
        Append(gai);
        return gai;
    }
    ...
    BOOL Merge(const IPA_NODE* caller, const IPA_NODE* callee,
            const SUMMARY_CALLSITE* call, GLOBAL_ARRAY_LIST* caller_list, 
...
public:
    ...
    DECLARE_SLIST_ITER_CLASS(GLOBAL_ARRAY_LIST_ITER,
            GLOBAL_ARRAY_INFO, GLOBAL_ARRAY_LIST);
};


/*****************************************************************************
 *
 * Section annotations for an IPA node
 *
 ****************************************************************************/
...
private:
    ...
    IPA_NODE **_callsite_map;           // from callsite IDs to ipa nodes
...
public:
    ...
    INT Get_formal_count() const {
        return (_formals ? _formals->Elements() : 0);
    }
    ...
    GLOBAL_ARRAY_LIST* Add_Global_Array_List(const SUMMARY_SYMBOL* s) 
    {
        ...
        GLOBAL_ARRAY_LIST *list = CXX_NEW(
                GLOBAL_ARRAY_LIST(base_st_idx), _mem);
        _glob_table->Enter(base_st_idx, list);
    ...
    GLOBAL_ARRAY_INFO* Find_Global_Array_Info(const SUMMARY_SYMBOL* s) 
    {
        ...
        return list->Find_Global_Array_Info(s->St_idx());
    }
    ...
    GLOBAL_ARRAY_INFO* Add_Global_Array_Info(const SUMMARY_SYMBOL* s) 
    {
        ...
        GLOBAL_ARRAY_LIST *list = Find_Global_Array_List(s);
        if (list == NULL) list = Add_Global_Array_List(s);
    ...
    PROJECTED_REGION* Global_Array_Region(const SUMMARY_SYMBOL* s,
            BOOL* is_messy, PROJECTED_REGION* region, BOOL is_mod);
 ...
 * Additional variables are added during the propagation, when mapping callee
 * annotations to caller annotations.
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * Obtain the side effects (on actuals and globals) at a callsite. Array
 * sections accessed by the callee are mapped to the caller space.
 *
 * If the callee has incomplete array summary, no PROJECTED_REGIONs are
 * returned, but scalar accesses are still present, and TRUE is returned.
 * Otherwise, FALSE is returned.
 *
 * <p_actuals> and <p_globals> will point to the result, allocated using the
 * given mempool.
 *
 ****************************************************************************/

extern BOOL IPA_get_call_data_summary(IPA_EDGE *e,
