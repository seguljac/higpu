...
/*****************************************************************************
 * 
 * These functions provide interface for accessing summary information from
 * the inliner and IPA. Since we want to be able to regenerate summary arrays
 * for a specific PU during IPA, it is safer to access them via IPA_NODE
 * instead of IP_FILE_HDR. However, this is not always possible, since some
 * arrays are read early in IPA, before the call graph is built. For those, we
 * provide dual interfaces.
 *
 ****************************************************************************/
...
public:
    ...
    IPL_SUMMARY_PTRS(void* summary, void* array_summary) :
        _summary(summary), _array_summary(array_summary) {}

    void* Summary() const { return _summary; }
    void* Array_Summary() const { return _array_summary; }
};

inline SUMMARY_FILE_HEADER* IPA_get_file_header(char* summary_base)
{
...
/*****************************************************************************
 *
 * Functions that take IP_FILE_HDR parameter also return the size of the
 * summary array from the file. 
 *
 ****************************************************************************/

SUMMARY_PROCEDURE* IPA_get_procedure_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_SYMBOL* IPA_get_symbol_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_COMMON* IPA_get_common_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
IVAR* IPA_get_ivar_file_array(const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_GLOBAL* IPA_get_global_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_VALUE* IPA_get_value_file_array(const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_FORMAL* IPA_get_formal_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_ACTUAL* IPA_get_actual_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_CALLSITE* IPA_get_callsite_file_array(
        const IP_FILE_HDR& hdr, INT32& size);
SUMMARY_STRUCT_ACCESS* IPA_get_struct_access_file_array(
        const IP_FILE_HDR& hdr, INT32& size);

/*****************************************************************************
 *
 * Access to the following SUMMARY arrays will go through the file header,
 * even if the IPA_NODE has been preoptimized. These access functions are
 * provided for convinience and completeness.
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * The following SUMMARY arrays will change when IPA_NODE is preoptimized, and
 * they shouldn't be accessed directly through the file header.
 *
 ****************************************************************************/
