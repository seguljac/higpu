...
/*****************************************************************************
 *
 * Use IPA's MOD information to determine if a symbol has been modified
...
/*****************************************************************************
 *
 * If the summary value is an integer constant, then return TRUE and val
 * should contain the value of the constant
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * Update terms in the linex expressions with constant values if possible and
 * determine if any term has non-constant symbolic values.
 *
 * `modified' will store whether or not some formal or global has been
 * modified in the procedure (so that constant prop becomes complicated?).
 *
 ****************************************************************************/

static LINEX* update_linex_with_constant_mod(IPA_NODE* node,
        LINEX* linex, BOOL* modified)
{
    INT32 size;
    ...
    for (INT i = 0; i <= linex->Num_terms(); ++i)
    {
        ...
        switch(term->Get_type())
        {
            ...
            case LTKIND_IV:
            {
                ...
                if (ivar.Is_Formal()) {
                    INT32 idx = ipa_proc->Get_formal_index()
                    ...
                    *modified |= Is_formal_modified(node, formal);
                    ...
                    Update_term_with_global_constant(
                            node, base_st, offset, mtype, term);
            ...
            default:
                Fail_FmtAssertion(
                        "update_linex_with_constant_mod: LTKIND_NONE");
                break;
        }
    }
...
/*****************************************************************************
 * 
 * Merge regions to compute DMOD after constant propagation and scalar mod
 * information have been propagated.
 *
 * NOTE: an easy extension to this is DUSE.
 *
 * Need to set it to bottom if bad alias and/or messy region. A region is set
 * to messy if it has non-constant symbolic terms.
 *
 ****************************************************************************/
...
static void update_region(IPA_NODE *node, REGION_ARRAYS *r)
{
    ...
    /* Since procedure clones share PROJECTED_REGIONs from summary files, they
     * need to be copied rather than changed in-place.
     */
    PROJECTED_REGION *copy_pr = (PROJECTED_REGION*)MEM_POOL_Alloc(
            &IPA_array_prop_pool, sizeof(PROJECTED_REGION) * count);
    bcopy(IPA_get_proj_region_array(node) + start_idx, copy_pr, 
            count * sizeof(PROJECTED_REGION));
    for (i = 0; i < count; i++) copy_pr[i].Set_Mem_Pool(&IPA_array_prop_pool);
    ...
    if (sym->Is_formal()) {
        ...
        position =
            IPA_get_formal_array(node)[sym->Get_findex()].Get_position();
        is_formal = TRUE;
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        PROJECTED_REGION *p = &copy_pr[i];

        /* Make p totally independent from the file-mapped version and has
         * been simplified through constant propagation.
         */
        if (!p->Is_messy_region()) update_projected_region(node, p);
...
/*****************************************************************************
 *
 * For each formal/global section in the local summary, use the result of
 * constant propogation ot promote bounds to constants, and then merge them.
 * The result is stored in the IPA_NODE_SECTION_INFO of 'node'.
 *
 ****************************************************************************/
...
static void update_mod_const_sections(IPA_NODE *node)
{
    ...
    if (ipa_proc->Has_incomplete_array_info()) return; 
    ...
    for (INT i = start_idx; i < end_idx; ++ i)
    {
        ...
        for (INT j = start_region_idx; j < end_region_idx; ++j) {
...
/*****************************************************************************
 *
 * Walk through the call graph and init each node's section information.
 *
 ****************************************************************************/

static void init_node_section_annot(IPA_NODE *node)
{
    if (node->Section_Annot() != NULL) return;
    ...
    IPA_NODE_SECTION_INFO *asection = CXX_NEW(
            IPA_NODE_SECTION_INFO(&IPA_array_prop_pool),
            &IPA_array_prop_pool);
    ...
    if (count > 0)
    {
        STATE_ARRAY *state = CXX_NEW(STATE_ARRAY(&IPA_array_prop_pool),
                &IPA_array_prop_pool);
        ...
        {
            (*state)[j].Init();
            // The formal is considered to have an array section iff it is a
            // pointer-to-array type.
            TY_IDX formal_ty_idx = formal->Get_ty();
    ...
    if (ipa_cfg != NULL) {
        if (ipa_proc->Get_array_section_count() != 0) {
            asection->Set_cfg_node(
                    &ipa_cfg[ipa_proc->Get_array_section_index()]);
        }
    }
...
/*****************************************************************************
 *
 * Constructor of IPA_ARRAY_DF_FLOW
 *
 ****************************************************************************/

IPA_ARRAY_DF_FLOW::IPA_ARRAY_DF_FLOW(IPA_CALL_GRAPH*, DF_DIRECTION ddf,
        MEM_POOL *m)
: IPA_DATA_FLOW(ddf, m)
{
...
/*****************************************************************************
 *
 * Initialize each NODE in the call graph by mapping all the section data
 * structures into pointers so that manipulating, modifying, deleting etc. is
 * straightforward.
 *
 ****************************************************************************/

void IPA_ARRAY_DF_FLOW::InitializeNode(void *n)
{
    ...
    if (IP_FILE_HDR_section_annot(file_hdr) == NULL)
    {
        ...
        SECTION_FILE_ANNOT *section_annot = CXX_NEW(
                SECTION_FILE_ANNOT(ivar, &IPA_array_prop_pool),
                &IPA_array_prop_pool);
...
/*****************************************************************************
 *
 * The MEET operation does nothing; everything is done in TRANS operation.
 *
 ****************************************************************************/

void* IPA_ARRAY_DF_FLOW::Meet(void*, void*, INT*)
{
    return NULL;
}
...
void* IPA_ARRAY_DF_FLOW::Trans(void*, void*, void *vertex, INT *change)
{
    // This operation simply does both the Union MEET and the TRANSFER.
...
/*****************************************************************************
 *
 * Constructor of IPA_FORMALS_IN_ARRAY_SECTION_DF
 *
 ****************************************************************************/

IPA_FORMALS_IN_ARRAY_SECTION_DF::IPA_FORMALS_IN_ARRAY_SECTION_DF(
        IPA_CALL_GRAPH*, DF_DIRECTION ddf, MEM_POOL* m)
: IPA_DATA_FLOW(ddf,m)
{}

void* IPA_FORMALS_IN_ARRAY_SECTION_DF::Meet(void*, void*, INT*)
{
    // All work is done in Trans.
    return NULL;
}


/* These are made file-statics so that we don't pass 6 arguments into possibly
 * deep recursive calls. They are all from the summary of the caller, i.e.,
 * node in Trans.
 */
...
/*****************************************************************************
 *
 * Check if an expression contains only formal parameters (and optional
 * constants). The formals being used are marked in 'formals_used'.
 *
 ****************************************************************************/

static BOOL Expr_has_only_formals(SUMMARY_EXPR* expr, mBOOL* formals_used)
{
    if (expr->Is_expr_unknown()) return FALSE;
    ...
    if (expr->Has_const_operand())
    {
        ...
        return
            ((expr->Is_expr_value(kid)
              && Value_has_only_formals(values + non_const_idx, formals_used))
             ||
             (expr->Is_expr_expr(kid)
              && Expr_has_only_formals(expressions + non_const_idx,
                  formals_used)));
    }
 ...
 * The formals being used are marked in 'formals_used'.
 *
 ****************************************************************************/
...
{
    if (value->Is_formal()) {
        INT pos = formals[value->Get_formal_index()].Get_position();
        formals_used[pos] = TRUE;
        return TRUE;
    }
    ...
    return (value->Is_int_const() || value->Is_const_st()) ? TRUE : FALSE;
}


/*****************************************************************************
 *
 * Transfer operation, merge sections from callee to caller If change in
 * annotation, set change to TRUE.
 *
 ****************************************************************************/

void* IPA_FORMALS_IN_ARRAY_SECTION_DF::Trans(void*, void*, void* vertex, 
        INT* change)
{
    IPA_NODE* node = (IPA_NODE*)vertex;

    UINT32 num_formals = node->Num_Formals();
    if (num_formals == 0) return NULL;
