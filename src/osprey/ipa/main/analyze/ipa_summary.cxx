...
static SUMMARY_PROCEDURE* get_procedure_file_array(
...
static SUMMARY_PROCEDURE* get_procedure_array(const IPA_NODE* node)
{
    INT32 size;
    return IPA_get_procedure_file_array(node->File_Header(), size);
}

static SUMMARY_SYMBOL* get_symbol_array(const IPA_NODE* node)
{
    INT32 size;
...
static SUMMARY_FORMAL* get_formal_array(const IPA_NODE *node)
{
    if (node->Is_Preoptimized())
    {
        SUMMARY *summary = 
            (SUMMARY*)IPA_Call_Graph->New_Summary_Ptrs(node)->Summary();
        if (summary->Has_formal_entry()) return summary->Get_formal(0);
    }
    else
    {
        const IP_FILE_HDR& hdr = node->File_Header();
        const SUMMARY_FILE_HEADER *summary_header =
            IP_FILE_HDR_file_header(hdr);

        if (summary_header->Get_formal_size()) {
            return (SUMMARY_FORMAL *)(IP_FILE_HDR_summary(hdr)
                    + summary_header->Get_formal_offset());
        }
    }
...
{
    const SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
    size = summary_header->Get_ivar_size();
    return (size == 0) ? NULL :
        (IVAR*)(IP_FILE_HDR_summary(hdr) + summary_header->Get_ivar_offset());
}
...
SUMMARY_PROCEDURE* IPA_get_procedure_file_array(
        const IP_FILE_HDR& hdr, INT32& size)
{
    if (IP_FILE_HDR_file_header(hdr) != NULL) {
    ...
    return (size > 0) ? summary->Get_procedure(0) : NULL;
}
...
{
    if (IP_FILE_HDR_file_header(node->File_Header()) != NULL) {
...
{
    IP_FILE_HDR& header = node->File_Header();
    SECTION_FILE_ANNOT* annot = IP_FILE_HDR_section_annot(header);
    if (annot != NULL) {
    ...
    return IPA_get_ivar_file_array(header, size);
}
...
REGION_ARRAYS* IPA_get_region_array(const IPA_NODE *node)
{
    ...
    else
    {
        ...
        if (summary_header->Get_regions_array_size()) {
            return (REGION_ARRAYS *)(IP_FILE_HDR_summary (hdr)
                    + summary_header->Get_regions_array_offset());
        }
    }
