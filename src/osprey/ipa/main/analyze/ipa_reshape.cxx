...
extern BOOL Mismatched_Types(IPA_NODE* ipan_caller, IPA_NODE* ipan_callee,
        SUMMARY_CALLSITE *sk, INT position, MEM_POOL *mem_pool)
{
    FmtAssert(position >= 0 && position < sk->Get_param_count(),
            ("Translate_Through_Call: 'position' out of range"));
...
/*****************************************************************************
 *
 * Consider the callsite 'sk' which represents a call from 'ipan_caller' to
 * 'ipan_callee'. Return TRUE if the 'position'-th argument in this call has
 * a problem which MAY be resolved by reshaping. Return FALSE otherwise. 
 *
 ****************************************************************************/

extern BOOL Try_Reshape_Callee_Formal(IPA_NODE* ipan_caller,
        IPA_NODE* ipan_callee, SUMMARY_CALLSITE* sk, INT position,
        MEM_POOL* mem_pool)
{
    FmtAssert(position >= 0 && position < sk->Get_param_count(),
            ("Translate_Through_Call: 'position' out of range"));
