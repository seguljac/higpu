...
/*****************************************************************************
 *
 * Print mod/ref information for formals at current node.
 *
 ****************************************************************************/
...
void CG_BROWSER::Mod_Ref_Formals(FILE* fp)
{
    ...
    { 
        ...
        { 
            BOOL indent = FALSE;
            if (ipaa->Is_formal_dmod_elmt(i)
                    || ipaa->Is_formal_imod_elmt(i)) { 
                fprintf(fp, "MOD %s", Formal_Name(i));
                indent = TRUE;
            } 
            if (ipaa->Is_formal_dref_elmt(i)
                    || ipaa->Is_formal_iref_elmt(i)) {
                if (indent) fprintf(fp, "     ");
                fprintf(fp, "REF %s", Formal_Name(i));
                indent = TRUE;
            }
            if (!indent) fprintf(fp, "    %s", Formal_Name(i));
            fprintf(fp, "\n");
        }
    }
    else
    {
        ...
        for (INT i = 0; i < formal_count; i++)
        {
            ...
            else
            {
                ...
                PROJECTED_REGION *pr = ipaf->Get_projected_mod_region();
                if (pr != NULL) {
                    fprintf(fp, "MOD %s", Formal_Name(i));
                    INT cc = Print_Projected_Region(buffer, 0, pr);
                    if (cc >= CGB_MAX_STRING_LENGTH - 1) {
                        fprintf(fp,
                                "Warning: string exceeded buffer length!!");
                    }
                    fprintf(fp, "%s\n", buffer);
                    indent = TRUE; 
                }

                pr = ipaf->Get_projected_ref_region();
                if (pr != NULL) {
                    if (indent) fprintf(fp, "     ");
                    fprintf(fp, "REF %s", Formal_Name(i));
                    INT cc = Print_Projected_Region(buffer, 0, pr);
                ...
                pr = ipaf->Get_projected_dcl_region();
                if (pr != NULL) {
                    if (indent) fprintf(fp, "     ");
                    fprintf(fp, "DCL %s", Formal_Name(i));
                    INT cc = Print_Projected_Region(buffer, 0, pr);
                    if (cc >= CGB_MAX_STRING_LENGTH - 1) {
                        fprintf(fp,
                                "Warning: string exceeded buffer length!!");
