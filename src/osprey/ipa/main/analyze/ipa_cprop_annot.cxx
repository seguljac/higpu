...
//-------------------------------------------------------
static BOOL Update_node(SUMMARY_VALUE& formal, const SUMMARY_VALUE& actual,
        TYPE_ID formal_type, BOOL is_ref_parm)
{
    // should we verify type compatibility here?

    if (formal.Is_not_const()) return FALSE;
...
extern BOOL Edges_Have_Equiv_Cprop_Annots(
        const IPA_EDGE* e1, const IPA_EDGE* e2)
{
...
static IPA_CLONING_ACTION Precise_int_const_node_update(
        SUMMARY_VALUE* formal, SUMMARY_VALUE* actual)
{
    if (formal->Is_unknown()) {
    ...
    if (formal->Is_int_const()
            && formal->Get_int_const_value()
            == actual->Get_int_const_value()) {
        return NO_CHANGE;
    }
...
//------------------------------------------------------------------
static IPA_CLONING_ACTION Precise_const_st_node_update(
        SUMMARY_VALUE* formal, SUMMARY_VALUE* actual)
{
    if (formal->Is_unknown()) {
        // unknown (TOP) is the initial value that the state is set to
...
static IPA_CLONING_ACTION Precise_node_update(
        SUMMARY_VALUE* formal, SUMMARY_VALUE* actual, 
        TYPE_ID formal_type, BOOL is_ref_parm)
{
...
IPA_CLONING_ACTION Union_Formal_Cprop_Annot_With_Cloning(
        IPA_NODE* callee, IPA_EDGE* edge)
{
    VALUE_DYN_ARRAY *formals = callee->Cprop_Annot();
    VALUE_DYN_ARRAY *actuals = edge->Cprop_Annot();

    if (formals == NULL || actuals == NULL) return NO_CHANGE;
    ...
    for (UINT32 pos = 0, formal_idx = ipa_proc->Get_formal_index(); 
            pos < count; ++pos, ++formal_idx)
    {
        const SUMMARY_FORMAL& sum_formal = ipa_formal[formal_idx];
        const SUMMARY_SYMBOL& formal_sym =
            ipa_symbol[sum_formal.Get_symbol_index()];
        ...
        if (Clone_for_this_formal(callee, pos))
        {
            ...
            IPA_CLONING_ACTION node_action = Precise_node_update(
                    &(*formals)[pos], &(*actuals)[pos], mtype, is_ref_param);
        ...
        {
            if (Update_node((*formals)[pos], (*actuals)[pos],
                        mtype, is_ref_param) && action == NO_CHANGE) {
    ...
    return action;
} // Union_Formal_Cprop_Annot_With_Cloning
...
static void Evaluate_actuals(IPA_NODE *caller, IPA_NODE *callee,
        IPA_EDGE *edge)
{
    ...
    if (count > callee->Num_Formals()) count = callee->Num_Formals();
    ...
    if (annot == NULL) {
        ...
        for (i = 0; i < count; i++) (*annot)[i].Init();
    ...
    if (!Need_to_verify_duplicated) return;
...
extern BOOL Intra_PU_Formal_Cprop(IPA_NODE *node)
{
    BOOL updated = FALSE;

    if (IPA_Enable_DFE && PU_is_dead(node, &updated)) return updated;
    ...
    if ((node->Is_Externally_Callable() && IPA_Enable_Cloning)
            || node->Summary_Proc()->Is_alt_entry()
            || node->Summary_Proc()->Has_alt_entry()) formal_value = NULL;
    ...
    eval_hash = CXX_NEW(
            EVAL_HASH(100, eval_hasher(), std::equal_to<const void *>(),
                &Ipa_cprop_pool), &Ipa_cprop_pool);
    ...
    if (IPA_Enable_Flow_Analysis) Process_cond_branches(call_deleted);
