...
//----------------------------------------------------
static BOOL Edge_has_new_cprop_annot(const IPA_EDGE *edge,
        IPA_EDGE **clone_edges, UINT32 num_clone_edges)
{
    if (edge->Cprop_Annot()) {
...
void* IPA_CPROP_DF_FLOW::Meet(void*, void* vertex, INT* change)
{
    IPA_NODE *n = (IPA_NODE*)vertex;
    BOOL call_from_alt_entry = FALSE;

    // Temporary array for storing edges that require cloning
    IPA_EDGE **clone_edges = (IPA_EDGE**)alloca(
            IPA_Max_Node_Clones * sizeof(IPA_EDGE*));
    ...
    for (edge_iter.First(); !edge_iter.Is_Empty(); edge_iter.Next())
    {
        ...
        if (e == NULL || e->Cprop_Annot() == (void*)-1) continue;
        ...
        if (num_clone_edges < IPA_Max_Node_Clones
            && Node_can_be_cloned(n)
#ifdef KEY
            ...
            if (Union_Formal_Cprop_Annot(n, e)) *change = TRUE;
        }
    }
    ...
    if (num_clone_edges > 0 && !call_from_alt_entry)
    {
        for (UINT32 i = 0; i < num_clone_edges; ++i)
        {
            ...
            IPA_NODE* clone_node =
                IPA_Call_Graph->Create_Quasi_Clone(clone_edges[i]);
...
void* IPA_CPROP_DF_FLOW::Trans(void*, void*, void *vertex, INT *change)
{
    IPA_NODE *n = (IPA_NODE*)vertex;

    MEM_POOL_Push(&Ipa_cprop_pool);

    if (Intra_PU_Formal_Cprop(n)) *change = TRUE;

    if (IPA_Enable_Common_Const) {
        if (Intra_PU_Global_Cprop(n)) *change = TRUE;
    }
...
void IPA_CPROP_DF_FLOW::PerformCloning(IPA_NODE *node)
{
    // don't clone alt. entry cause it is not a complete procedure.
    if (node->Summary_Proc()->Is_alt_entry()
            || node->Summary_Proc()->Has_alt_entry()) return;

    VALUE_DYN_ARRAY *cprop_annot = node->Cprop_Annot();

    if (cprop_annot == NULL
            || cprop_annot == (VALUE_DYN_ARRAY*)-1) return;

    BOOL fix_caller = FALSE;

    if (node->Is_Externally_Callable())
    {
        // If the node has constants and is externally callable then it is a
...
void IPA_CPROP_DF_FLOW::PostProcessIO(void *n)
{
    if (n == NULL) return;
    ...
    if (! node->Icall_List().empty()) Connect_indirect_call(node);

    if (IPA_Enable_Common_Const) Map_Global_Constants(node);

    if (Get_Trace(TP_PTRACE1, TP_PTRACE1_IPA_CPROP)) Ipa_cprop_tlog(node);
}
