...
/*****************************************************************************
 *
 * Every dataflow problem must first initialize this structure. Note that the
 * dataflow solver will init the entry and exit nodes, and the
 * depth-first-ordering of vertices is init on a demand-driven fashion. That
 * way it is created only once if several dataflow problems are solved on the
 * same call graph.
 *
 ****************************************************************************/
...
struct DFS
{
    ...
    NODE_INDEX entry, exit; // entry and exit node that connect the graph
    DFN *d;                 // depth first ordering of vertices

    INT change;             // change field, determine if dataflow problem
                            // has settled (or converged)

    void **in;              // array of IN annotations, one for each node in
                            // the graph (hold the result of meet operation)
     
    void **out;             // array of OUT annotations, one for each node in
                            // the graph (hold the result of trans operation)
};
