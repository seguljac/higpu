...
/*****************************************************************************
 *
 * Remap a PU_Info tree: map any global symbol table indices into merged
 * global symbol table indices. Used by IP_READ_pu_infos.
 *
 ****************************************************************************/
...
void IP_READ_remap_pu_infos(PU_Info *base, const IPC_GLOBAL_IDX_MAP *idx_map)
{
    ...
    for ( ; base != NULL ; base = PU_Info_next(base))
    {
        ...
        child = PU_Info_child(base);
        if (child != NULL) IP_READ_remap_pu_infos(child, idx_map);
    }
}
...
/*****************************************************************************
 *
 * Recursively traverse the PU_Infos in depth-first order to match them up
 * with the summaries. Assume that the PU list and the summary section use the
 * same order. Used by IP_READ_pu_infos.
 *
 ****************************************************************************/

INT IPA_process_PUs(IP_FILE_HDR &s, const SUMMARY_PROCEDURE *proc_array,
        PU_Info *pu_tree, INT proc_idx)
{
    ...
    for (PU_Info *pu = pu_tree; pu; pu = PU_Info_next(pu))
    {
        ...
        if ((child = PU_Info_child(pu)) != NULL) {
            proc_idx = IPA_process_PUs(s, proc_array, child, proc_idx);
...
/*****************************************************************************
 *
 * Read the tree of pu_infos, the summary information, and then set up the
 * array of IP_PROC_INFOs. The array contains essentially the same information
...
{
    if (IP_FILE_HDR_pu_list(s) != NULL) return;
    ...
    if (IP_FILE_HDR_summary(s) == NULL) {
        char *base = (char*)WN_get_section_base(
                IP_FILE_HDR_input_map_addr(s), WT_IPA_SUMMARY);
    ...
    // DAVID COMMENT: what if file_header is NULL?
    ...
    INT32 count;    // number of procedures in this file
    ...
    // Allocate the array of IP_PROC_INFOs.
    Set_IP_FILE_HDR_proc_info(s, (IP_PROC_INFO*)MEM_POOL_Alloc(
                Malloc_Mem_Pool, (sizeof(IP_PROC_INFO)*size_proc)));

    if (count != 0)
    {
        INT proc_idx = IPA_process_PUs(s,
                proc_array, IP_FILE_HDR_pu_list(s), 0);
...
/*****************************************************************************
 *
 * There's very little to do. We already have a merged global symbol table,
 * and the FILE_INFO struct is read in when the IP_FILE_HDR is first set up.
 *
 ****************************************************************************/
