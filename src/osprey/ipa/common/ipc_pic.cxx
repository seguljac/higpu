...
struct _NAME_REFCOUNT_t
{
    const char *name;
    UINT32 count;

    struct _NAME_REFCOUNT_t *next;
};

typedef struct _NAME_REFCOUNT_t NAME_REFCOUNT;

static NAME_REFCOUNT *g_name_refcount = NULL;

// returns a unique name, used for promoting static variables to globals
//
// DAVID EDIT:
//
// This function's behavior deviates from the original one in two ways:
// 1) It maintains a ref count for each name, as opposed to a global count.
// 1) It does not gracefully handle the case <name> == NULL.
// 2) It constructs a new name based on the entire <name> as opposed to
//    the part that does not contain "..".
//
static STR_IDX Create_Unique_Name(const char* name)
{
    char *new_name = NULL;

    Is_True(name != NULL, (""));

    // Does <name> exist in the ref count list?
    NAME_REFCOUNT *nrc = NULL;
    if (name != NULL)
    {
        for (nrc = g_name_refcount; nrc != NULL; nrc = nrc->next)
        {
            if (strcmp(nrc->name, name) == 0) break;
        }
    }

    if (nrc == NULL)
    {
        // Add an entry to the front of the list, with ref count 1.
        nrc = (NAME_REFCOUNT*)malloc(sizeof(NAME_REFCOUNT));
        Is_True(nrc != NULL, (""));
        nrc->name = name;       // shallow copy
        nrc->count = 1;
        nrc->next = g_name_refcount;
        g_name_refcount = nrc;

        // Since this is the first time, keep the original name.
        new_name = (char*)name;
    }
    else
    {
        ...
        new_name = (char*)alloca(strlen(name) + 12);
        strcpy(new_name, name);
        char *p = &new_name[strlen(name)];
        *p++ = '.';
        *p++ = '.';
        UINT32 i = nrc->count;
        do {
...
static void HC_free_name_refcount()
{
    while (g_name_refcount != NULL)
    {
        NAME_REFCOUNT *nrc = g_name_refcount;
        g_name_refcount = nrc->next;
        free(nrc);
    }
}
