...
/*****************************************************************************
 *
 * Given a WHIRL file, this routine will merge the global symbol-table in with
 * the current merged global symbol table which should already have been set
 * up as part of the SCOPE array. We also check that file format and revision
 * is as expected.
 *
 * We return a map from old indices into the global symbol table to to new
 * indices into the merged global symbol table. Note that the
 * IPC_GLOBAL_IDX_MAP structure will be allocated in a Malloc_Mem_Pool, and
 * must be freed up by the caller of this routine when appropriate.
 *
 * TODO: this procedure used to also set p_obj->est_const_size, which it no
 * longer does. If necessary, this ought to be calculated by walking the INITO
 * list (Inito_Table for per-file, the merged INITO table for a total number).
 *
 ****************************************************************************/

template <class Shdr, class Sym>
void process_whirl(an_object_file_ptr p_obj, int nsec,
        const Shdr* section_table, BOOL check_whirl_revision,
        const char* file_name, const Sym* elf_symtab) 
{
    int                    i;
    int                    gsymtab_idx = 0;
    int                    strtab_idx = 0;
