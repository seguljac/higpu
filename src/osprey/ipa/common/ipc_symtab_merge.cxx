...
// Anonymous names of un-named types may conflict after merging.
// This routine re-assigns the names of these types, like Get_Name
// in tree_symtab.cxx.
//
static void rename_anonymous_types()
{
    UINT anon_num = 0;
    char buf[32];

    UINT n_types = TY_Table_Size();
    for (UINT i = 1; i < n_types; ++i)
    {
        ...
        if (TY_anonymous(ty))
        {
            ...
            Set_TY_name_idx(ty, Save_Str(buf));
        }
    }
}
...
//
static void Merge_Global_St_Attr(const ST_ATTR* st_attr_tab, UINT32 size)
{
    for (UINT32 idx = 1; idx < size; ++idx)
    {
...
/** DAVID CODE BEGIN **/
            case ST_ATTR_EXTRA_FLAGS:
                ST_ATTR_Init(new_st_attr, st_idx, ST_ATTR_EXTRA_FLAGS,
                        ST_ATTR_flags(old_st_attr));
                break;
/*** DAVID CODE END ***/

            default:
                Fail_FmtAssertion(
                        "invalid ATTR_KIND in Merge_Global_St_Attr");
                break;
        }
    }
