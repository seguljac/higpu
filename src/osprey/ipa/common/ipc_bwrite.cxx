...
{
    Temporary_Error_Phase ephase("Writing WHIRL file");
    Is_True(out_file == NULL, ("ipa output queue: file is already open"));
    ...
    if (new_file == NULL) ErrMsg(EC_IR_Create, new_file_name, errno);
...
extern "C" void IP_WRITE_pu (IP_FILE_HDR *s , INT pindex)
{
    ...
    if (lexical_level == GLOBAL_SYMTAB + 1)
    {
        ...
        else
        {
            ...
            if (Output_Queue.should_flush(node)) IP_flush_output();
        }
    }
}
