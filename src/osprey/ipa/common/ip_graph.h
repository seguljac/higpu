 ...
 *
 * A GRAPH is a set of VERTEX objects, connected by a set of EDGE objects.
 * These sets are implemented as a vector of VERTEXes and a vector of EDGEs,
 * with all references being indices into these vectors. The sets are allowed
 * to grow (by adding VERTEXes or EDGEs) or to shrink (by deleting them).
 * Growth may cause reallocation of the vector. To minimize reallocation cost,
 * it is done in larger chunks than necessary. To avoid rearrangement of the
 * data upon deletion, the deleted VERTEX/EDGE is simply marked invalid.
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * A VERTEX contains two singly-linked lists of EDGEs, one of EDGEs starting
 * at that VERTEX, and one of EDGEs ending at that VERTEX. Each list is
 * represented in the VERTEX by the index of its first element (NODE_from and
 * NODE_to) and a count of its elements (NODE_fcnt and NODE_tcnt). An invalid
 * (i.e. unused) VERTEX is indicated by NODE_fcnt == INVALID_EDGE_INDEX. The
 * VERTEX also contains a pointer NODE_user to additional data required by the
 * client derived graph. The level of a vertex is defined as the (max level of
 * the immediate successors of the vertex) + 1
 *
 ****************************************************************************/
...
/*****************************************************************************
 *
 * An EDGE contains two VERTEXes, its from VERTEX (EDGE_from) and its to
 * vertex (EDGE_to). It contains links (indices) to the next edges in the
 * NODE_from list for its from VERTEX (EDGE_nfrom), and in the NODE_to list
 * for its to VERTEX (EDGE_nto). It contains a flag word containing
 * attributes, and a pointer EDGE_user to additional data required by the
 * client derived graph.
 *
 ****************************************************************************/
...
/*****************************************************************************
 * 
 * A GRAPH is the top-level directed graph data structure. For both vertices
 * and edges, it contains the number of VERTEX/EDGE structures actually
 * allocated (GRAPH_vmax, GRAPH_emax), the number of actual VERTEX/EDGE
 * components of the graph (GRAPH_vcnt, GRAPH_ecnt), the number of free
 * VERTEX/EDGE structures that are allocated and available for use
 * (GRAPH_vfree, GRAPH_efree), and the vector of NODE_EDGE components
 * (GRAPH_v, GRAPH_e).  It also contains the root of the graph (GRAPH_root)
 * and the memory pool to use for allocation (GRAPH_m).
 *
 ****************************************************************************/
...
public:
    ...
    NODE_ITER(GRAPH* graph, NODE_INDEX node_index) :
        ...
        nfrom  (INVALID_EDGE_INDEX),
        tcnt   (NODE_tcnt(&(GRAPH_v(graph)[node_index]))),
        nto    (INVALID_EDGE_INDEX),
 ...
 * control traversal in a particular order. It contains a vector of the graph
 * vertices in the traversal order (DFN_v_list), and the indices of the first
 * useful index in the vector (GRAV_first) and of the index after the last
 * useful index in the vector (DFN_end).
 *
 ****************************************************************************/
