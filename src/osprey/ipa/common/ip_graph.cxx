 ...
 * The free vertices and edges form a linked list respectively.
 *
 ****************************************************************************/
...
void GRAPH::Build()
{
    ...
    /* Allocate and init the vertices. */
    if (vmax != 0)
    {
        v = (GRAPH::NODE_TYPE*)MEM_POOL_Alloc(m,
                sizeof(GRAPH::NODE_TYPE)*vmax);
        bzero(v, sizeof(GRAPH::NODE_TYPE)*vmax);

        for (i = 0; i < vmax; ++i) {
            // The free vertices form a linked list.
            NODE_from(&v[i]) = i+1;
            NODE_fcnt(&v[i]) = -1;
        }
        // The last free vertex points to nowhere. 
        NODE_from(&v[vmax-1]) = INVALID_NODE_INDEX;
    }

    /* Allocate and init the edges. */
    if (emax != 0)
    {
        e = (GRAPH::EDGE_TYPE*)MEM_POOL_Alloc(m,
                sizeof(GRAPH::EDGE_TYPE)*emax);
        bzero(e, sizeof(GRAPH::EDGE_TYPE)*emax);

        for (i = 0; i < emax; ++i) {
            // The free edges form a linked list.
            EDGE_nfrom(&e[i]) = i+1;
            EDGE_from(&e[i]) = INVALID_NODE_INDEX;
            // DAVID COMMENT: how about EDGE_to?
        }
        // The last free edge points to nowhere.
        EDGE_nfrom(&e[emax-1]) = INVALID_EDGE_INDEX;
    }
}
...
/*---------------------------------------------------------------*/
NODE_INDEX NODE_ITER::First_Pred()
{
    // get the first to-edge
    if (to_e == INVALID_EDGE_INDEX) return INVALID_NODE_INDEX;
...
/*****************************************************************************
 *
 * Helper search function for depth first ordering, as described in the dragon
 * book, page 662.
 *
 * This function recursively searches the subgraph rooted at vertex v,
 * depth-first, finally adding v to the beginning of the node order when it
 * has added all its successors. The effect is that a vertex will always
 * precede all of its successors in the list, and will always follow all of
 * its predecessors (ignoring back-edges in the graph). Therefore, both the
 * resulting node order and its reversal are topological sorts.
 *
 * When called, the DFN struct contains the vertexes already added to the
 * order, with DFN_first being the smallest index where a vertex has been
 * added. The search uses a work array of flags (visit) to keep track of
 * whether a vertex has already been visited.
 *
 ****************************************************************************/

#define VISITED TRUE

static void Search(GRAPH *g, NODE_INDEX v, DFN *d, mBOOL *visit)
{
    // We must mark this node visited before visiting the children, even
    // though the recorded traversal order is not so.
    visit[v] = VISITED;

    NODE_ITER v_iter(g, v);
    for (NODE_INDEX vtx = v_iter.First_Succ(); vtx != INVALID_NODE_INDEX;
            vtx = v_iter.Next_Succ()) {
        /* Recursively search from vtx if it has not been visited. */
        if (!visit[vtx]) Search(g, vtx, d, visit); 
    }

    /* Add v to the order at DFN_first. The list grows from right to left. */
...
/*****************************************************************************
 * 
 * Create a topological sort graph traversal order based on a depth first
 * ordering (see the dragon book, page 660). Such an order helps to speed up
 * iterative data flow algorithms and can also be used to detect loops (i.e.
 * back-edges in the graph).
 *
 ****************************************************************************/

DFN* Depth_First_Ordering(GRAPH *g, MEM_POOL *m)
{
    ...
    if (Get_Trace(TP_IPA, 8)) {
        fprintf(TFile,
                "Depth_First_Ordering: vertex count = %d \n", vertex_count);
    }

    /* Allocate the DFN struct: */
    DFN *d = (DFN*)MEM_POOL_Alloc(m, sizeof(DFN));
    if (d == NULL) ErrMsg (EC_No_Mem, "Depth_First_Ordering: d");
    DFN_v_list(d) = (NODE_INDEX*)MEM_POOL_Alloc(m,
            sizeof(NODE_INDEX) * vertex_count);
    if (DFN_v_list(d) == NULL) ErrMsg(EC_No_Mem, "Depth_First_Ordering: list");

    /* Allocate work array to keep track of visited vertices: */
    mBOOL *visit = (mBOOL*)MEM_POOL_Alloc(m, sizeof(mBOOL)*vertex_max);
    if (visit == NULL) ErrMsg(EC_No_Mem, "Depth_First_Ordering: visit");
    bzero(visit, sizeof(mBOOL)*vertex_max);

    /* Initialize the DFN struct -- set end and first to point beyond
     * the first element:
     * The list grows from right to left.
     */
