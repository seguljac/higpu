...
#define HANDLE_PRAGMA_HICUDA
#ifdef HANDLE_PRAGMA_HICUDA
static void handle_pragma_hicuda(cpp_reader*);

static void handle_pragma_hicuda(cpp_reader *pfile)
{
    tree msg;

    if (c_lex(&msg) != CPP_NAME)
    {
        GCC_BAD("Invalid HiCUDA pragma: missing message!\n");
    }
}
