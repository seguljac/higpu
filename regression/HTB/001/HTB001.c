...
#pragma hicuda global alloc d[*] copyin
    ...
    const int limit_b = 900, limit_c = 901, limit_d = 902;

    // Data dependences between the two loops will NOT be respected once
    // turned into CUDA code.
...
#pragma hicuda loop_partition over_tblock(CYCLIC) over_thread
    ...
    // should optimize to a single assignment with guard
#pragma hicuda loop_partition over_tblock(CYCLIC) over_thread
    for (i = 0; i < limit_b; i += 6)
    {
        b[i] += a[i] + i;
    }

    // should optimize to a single assignment
#pragma hicuda loop_partition over_tblock(CYCLIC)
    for (i = 0; i < limit_b; i += 90)
    {
        b[i] += a[i] + i;
    }
